...
r.push(v);return r.join(" ")}var l=this,k=l.$icon.data("color")||"#aaa",h=d.crm.color(k),m=l.$icon.data("hover-color")||h.getTone(20),n=l.$icon.data("stroke-width")||0,p=l.$icon.data("hover")||!1,q=l.svg.append("g").append("polygon").attr("class","is-animated").attr("points",g()).attr("transform","translate("+n/2+","+n/2+")").style("fill",k);0<n&&(h=h.getTone(-20),q.style("stroke",h).style("stroke-width",n));p&&(p=q.node(),p=d(p).closest(".c-state-item "),p.on("mouseenter",function(){q.style("fill",
...
function(){l&&clearTimeout(l);k&&k.abort();k=c.post(h,{process_id:g});k.always(function(){k=null;l=setTimeout(m,e)});k.fail(function(){return!1})};l=setTimeout(m,500+300*d)}catch(n){console.error(["source email worker fail",n])}}()):console.error("Empty url")}})})(jQuery);(function(c,a){c.fn.serializeObject=function(){var b={};c.each(this.serializeArray(),function(d,e){d=e.name;e=e.value;b[d]=b[d]===a?e:c.isArray(b[d])?b[d].concat(e):[b[d],e]});return b}})(jQuery);
...
k.closest(".c-add-contact");h?(l.hide(),k.addClass("is-shown")):(l.show(),k.removeClass("is-shown"))}function e(k,h){function m(n){n=n?n:[];c.each(n,function(p,q){p=q.text;var r=c('<div class="errormsg" />').html(p);l.one("click",function(){r.remove()});g.prepend(r)})}h=h?h:function(){};c.post("?module=deal&action=userAdd",{contact_id:k,deal_id:f.deal_id},function(n){"ok"===n.status?h(n.data.html):n.errors&&m(n.errors)},"json")}var f=this,g=f.$aside.find(".js-add-user-wrapper"),l=g.find(".js-add-contact");
l.on("click",function(k){k.preventDefault();d(c(this),!0)});g.on("click",".js-close-add-contact",function(k){k.preventDefault();d(c(this))});(function(){function k(m,n){var p=f.$aside.find('.c-contact[data-id="'+m+'"]');p.length&&h(p);e(m,function(q){q=c(q);n.before(q);h(q);c(document).trigger("refresh")})}function h(m){m.addClass("is-highlight");setTimeout(function(){c.each(m,function(n,p){c.contains(document,p)&&c(this).removeClass("is-highlight")})},3E3)}f.$aside.find(".js-contact-autocomplete").each(function(){var m=
c(this);m.autocomplete({appendTo:f.$aside,source:c.crm.app_url+"?module=autocomplete&type=user&funnel_id="+f.funnel_id,minLength:0,html:!0,focus:function(){return!1},select:function(n,p){k(p.item.id,m.closest(".c-add-contact"));m.val("");f.$aside.find(".js-close-add-contact").trigger("click");return!1}}).data("ui-autocomplete")._renderItem=function(n,p){var q=c("<li />");q.addClass("ui-menu-item-html").append("<div>"+p.value+"</div>").appendTo(n);p.rights||(q.addClass("is-locked"),q.on("click",function(r){r.preventDefault();
r.stopPropagation()}));return q};m.on("focus",function(){m.data("uiAutocomplete").search(m.val())})})})()};CRMDeal.prototype.initRemoveFiles=function(){function d(g,l){c.post("?module=file&action=delete",{id:g},function(k){"ok"==k.status?l.remove():console&&console.log&&console.log("File Remove Error")})}var e=this,f;e.$wrapper.find(".c-files-wrapper").on("click",".js-remove-file",function(g){g.preventDefault();var l=c(this).closest(".c-file");g=l.find(".c-name").text();var k=parseInt(l.data("id"));
...
g=null;new CrmEditable({$wrapper:d,onSave:function(l){var k=l.$field.val();if(k.length&&l.text!==k){g&&g.abort();k={id:f,name:k};l.$field.attr("disabled",!0);var h=c('<i class="icon16 loading"></i>').css("margin","0 0 0 4px").insertAfter(l.$field);g=c.post(e,k,function(m){l.$field.attr("disabled",!1);h.remove();l.toggle("hide");m&&m.data&&m.data.deal&&(l.text=m.data.deal.name,l.$wrapper.text(m.data.deal.name),c.crm.title.set(m.data.deal.name))})}else k.length||l.$field.val(l.text),l.toggle("hide")}})}};
CRMDeal.prototype.initChangeDate=function(){function d(n){n?(g.addClass("is-shown"),h.focus()):(g.removeClass("is-shown"),h.datepicker("hide"))}function e(n){h.val()||(n="");c.post("?module=deal&action=changeExpectedDate",{id:f.deal_id,expected_date:n?n:""},function(p){"ok"==p.status&&p.data&&p.data.deal&&((p=p.data.deal.expected_date)?(l.hide(),k.show().find(".js-name").text(p)):(k.hide().find(".js-name").text(""),l.show()),d())},"json")}var f=this,g=f.$wrapper.find(".js-date-toggle");if(!g.length)return!1;
var l=g.find(".js-link"),k=g.find(".js-span"),h=g.find(".js-datepicker"),m=g.find(".js-field");g.on("click",".js-set-date",function(n){n.preventDefault();d(!0)});h.on("keydown keypress keyup",function(n){27===n.keyCode&&d()});g.on("click",".js-close-edit-date",function(){d()});(function(){h.datepicker({altField:m,altFormat:"yy-mm-dd",changeMonth:!0,changeYear:!0,onSelect:function(){var n=h.val(),p=m.val(),q=c.datepicker._defaults.dateFormat,r=!1;try{c.datepicker.parseDate(q,n),r=!0}catch(u){}if(r)e(p);
else return!1}});h.val()||h.datepicker("setDate","+0d")})()};CRMDeal.prototype.initChangePrice=function(){function d(p){p?l.addClass("is-shown"):l.removeClass("is-shown")}function e(p,q){c.post("?module=deal&action=changeAmount",{id:g.deal_id,amount:p?p:"",currency_id:q?q:""},function(r){"ok"==r.status&&r.data&&r.data.deal&&((r=r.data.deal.amount)?(k.hide(),h.show().text(r)):(h.hide().text(""),k.show()),d())},"json")}function f(){var p=m.val(),q=n.val(),r=!1;p&&(r=p.replace(",","."));0<=r?e(p,q):
m.addClass("error")}var g=this,l=g.$wrapper.find(".js-price-toggle");if(!l.length)return!1;var k=l.find(".js-link"),h=l.find(".js-span"),m=l.find(".js-price-field"),n=l.find(".js-currency-field");l.on("click",".js-set-price",function(p){p.preventDefault();d(!0)});l.on("click",".js-save-price",function(p){p.preventDefault();f()});l.on("click",".js-cancel",function(p){p.preventDefault();d()});m.on("focus",function(){m.hasClass("error")&&m.removeClass("error")});m.on("keydown",function(p){13===p.keyCode&&
(p.preventDefault(),f())})};CRMDeal.prototype.initChangeField=function(){function d(h,m){var n=c(h).parents(".js-field-toggle");m?(n.find(".c-hidden").css({height:c(h).height()+8}),c(h).closest(".js-field-toggle").hasClass("js-textarea")?(n.addClass("is-shown").find(".c-hidden").css("width","100%"),n.find("textarea").slideDown().next().find(".icon16").removeClass("loading").addClass("disk"),n.css({width:"100%","margin-left":0}).prev().hide()):(n.addClass("is-shown"),n.find(".js-datepicker-field").focus())):
c(document).trigger("mousedown")}function e(h){var m=l.data("deal-id"),n="",p=c(h).data("field-id"),q=c(h).data("field-type"),r=!1;switch(q){case "checkbox":c(h).prop("required")&&!c(h).prop("checked")?(r=!0,c(h).addClass("error")):n=Number(c(h).prop("checked"));break;case "date":var u=c(h).siblings(".js-datepicker-field"),v=c.datepicker._defaults.dateFormat,t=!1;try{c.datepicker.parseDate(v,u.val()),t=!0}catch(x){}""!=u.val()&&""!=u.val().trim()&&t||!c(h).prop("required")?n=u.val():(r=!0,u.addClass("error"));
break;case "text":u=c(h).parents(".c-hidden").find(".js-field-value-text");u.next().find(".icon16").removeClass("disk").addClass("loading");""!=u.val()&&""!=u.val().trim()||!u.prop("required")?n=u.val():(r=!0,c(h).parents(".c-hidden").find(".js-field-value-text").addClass("error"));break;case "select":var w=c(h).val();t=!1;c(h).find("option").each(function(x,y){c(y).val()==w&&(t=!0)});!t||""==w&&c(h).prop("required")?(r=!0,c(h).addClass("error")):n=w;break;case "radio":n=c(h).find(":checked").val();
break;case "number":w=c(h).val().replace(",",".");(""==w||isNaN(Number(w)))&&c(h).prop("required")?(r=!0,c(h).addClass("error")):n=w;break;default:w=c(h).val(),""!=w&&""!=w.trim()||!c(h).prop("required")?n=w:(r=!0,c(h).addClass("error"))}r||f(h,{id:m,field_id:p,value:n,value_type:q})}function f(h,m){c.post("?module=deal&action=changeValue",m,function(n){if("ok"==n.status){if(n.data&&n.data.deal_params){n=n.data.deal_params.value;var p=c(h).parents(".js-field-toggle").find(".js-output-value");n?(p.text(n),
...
c(document).on("mousedown",function(h){0!==g.has(h.target).length||g.is(h.target)||g.each(function(m,n){var p=c(n);m=p.find("textarea");p.hasClass("js-textarea")?m.slideUp(function(){p.removeClass("is-shown").removeAttr("style").prev().show()}):p.removeClass("is-shown")})});k.on("keydown",function(h){13===h.keyCode&&(h.preventDefault(),c(this).hasClass("js-datepicker-field")?e(c(this).siblings(".js-field-value")):e(this))});(function(){var h=l.find(".js-datepicker-field"),m=h.siblings(".js-field-value");
h.datepicker({altField:m,altFormat:"yy-mm-dd",changeMonth:!0,changeYear:!0,onSelect:function(){var n=c(this).siblings(".js-field-value");e(n)}})})()};CRMDeal.prototype.initWYSIWYG=function(){var d=this.$wrapper.find(".js-wysiwyg");if(!d.length)return!1;c.crm.initWYSIWYG(d,{keydownCallback:function(e){}})};CRMDeal.prototype.initChangeDescription=function(){function d(k){k?g.addClass("is-shown"):g.removeClass("is-shown")}function e(k){c.post("?module=deal&action=changeDescription",k,function(h){if("ok"==
...
function(k){k.preventDefault();d(!1)});l.on("focus",function(){l.hasClass("error")&&l.removeClass("error")})};CRMDeal.prototype.initChangeUser=function(){function d(h){if(!h.length)return!1;var m=h.closest(".c-contact");h.autocomplete({appendTo:m,source:"?module=autocomplete&type=user&funnel_id="+l.funnel_id,minLength:0,html:!0,focus:function(){return!1},select:function(n,p){e(p.item.id,m);h.val("");return!1}}).data("ui-autocomplete")._renderItem=function(n,p){var q=c("<li />");q.addClass("ui-menu-item-html").append("<div>"+
p.value+"</div>").appendTo(n);p.rights||(q.addClass("is-locked"),q.on("click",function(r){r.preventDefault();r.stopPropagation()}));return q};h.on("focus",function(){h.data("uiAutocomplete").search(h.val())})}function e(h,m){k?g(m):f(h,function(){k=!0;c.post("?module=deal&action=changeUser",{id:l.deal_id,user_contact_id:h},function(n){if("ok"!==n.status)n.errors?console.error(n.errors):console.error(n),c.crm.alert.show({title:"Error",button:"Error"});else if(n.data=n.data||{},n.data.deal_access_denied)window.location.href=
c.crm.app_url+"deal/";else if(n.data.html){n=c(n.data.html);var p=l.$aside.find('.c-users-section .c-contact[data-id="'+h+'"]').not(m);p.length&&p.remove();m.replaceWith(n);d(n.find(".js-owner-autocomplete"))}},"json").always(function(){k=!1})})}function f(h,m){c.post("?module=deal&action=changeUserConfirm",{id:l.deal_id,user_contact_id:h},"json").done(function(n){n.data&&n.data.need_confirm?new CRMDialog({html:n.data.html||"",onOpen:function(p,q){p.find(".js-confirm-button").on("click",function(){m()})}}):
m()})}function g(h,m){m?h.addClass("is-edit"):h.removeClass("is-edit")}var l=this,k=!1;l.$aside.on("click",".js-show-combobox",function(h){h.preventDefault();h=c(this).closest(".c-contact");g(h,!0)});l.$aside.on("click",".js-hide-combobox",function(h){h.preventDefault();h=c(this).closest(".c-contact");g(h)});l.$aside.find(".js-owner-autocomplete").each(function(){d(c(this))});(function(){function h(q){n||f(q,function(){n=!0;c.post("?module=deal&action=changeUser",{id:l.deal_id,user_contact_id:q},
function(r){if("ok"===r.status)if(r.data.deal_access_denied)window.location.href=c.crm.app_url+"deal/";else{r=c(r.data.html);var u=l.$aside.find('.c-users-section .c-contact[data-id="'+l.user_id+'"]');u.length&&u.remove();l.$aside.find('.c-contact[data-id="'+r.data("id")+'"]').remove();p.after(r).css("display","none");l.$aside.find(".js-add-user-wrapper").show();d(r.find(".js-owner-autocomplete"))}},"json").always(function(){n=!1})})}function m(q){q?p.addClass("is-extended"):p.removeClass("is-extended")}
...
d=b.find(".js-datepicker"),e=b.find('[name="deal[expected_date]"]');d.datepicker({altField:e,altFormat:"yy-mm-dd",changeMonth:!0,changeYear:!0,onSelect:a});d.on("blur",a);d.on("keydown keypress keyup",function(f){13===f.which&&f.preventDefault()});d.on("click",".js-icon",function(){d.focus()})};CRMDealEdit.prototype.initSave=function(){function a(){for(var n={data:[],errors:[]},p=l.serializeArray(),q=0;q<p.length-1;q++)for(var r=q+1;r<p.length;r++)p[q].name==p[r].name&&(""==p[q].value?p.splice(q,
1):""==p[r].value&&p.splice(r,1));c.each(p,function(u,v){u=c('[name="'+v.name+'"]');var t=0<c('[name="'+v.name+'"][type="radio"]').length,w=""==v.value||""==v.value.trim();1==c(u).prop("required")&&w&&n.errors.push({name:v.name,value:g.locales.bad_amount,is_radio:t});"deal[amount]"===v.name&&(v.value.length&&(v.value=c.trim(v.value)),t=v.value,u=!1,t.length&&(t=t.replace(",","."),0<t||"0"===t)&&(u=!0),u||n.errors.push({name:v.name,value:g.locales.bad_amount}));n.data.push(v)});"edit"===g.contact_mode?
g.can_edit_contact&&e(n):f(n);return n}function b(n){n&&n[0]||(n=[]);c.each(n,function(p,q){p=q.name;var r=q.value;if(q=q.is_radio)var u=g.$wrapper.find('[name="'+p+"\"][type='hidden']"),v=g.$wrapper.find('[name="'+p+"\"][type='radio']");else u=g.$wrapper.find('[name="'+p+'"]');"contact[firstname]"!==p||u.is(":visible")||(u=g.$wrapper.find(".js-contact-autocomplete"));var t=c("<span class='c-error' />").addClass("errormsg").text(r);if(u.length&&!u.hasClass("error"))if(u.parent().append(t),u.addClass("error"),
q)v.one("focus click change",function(){u.removeClass("error");t.remove()});else u.one("focus click change",function(){u.removeClass("error");t.remove()});else k.append(t),l.one("submit",function(){t.remove()})})}function d(n){if(!m){m=!0;var p=c.crm.app_url+"?module=deal&action=save";h.attr("disabled","disabled").prop("disabled",!0);var q=g.$wrapper.find(".loading");q.show();c.post(p,n,function(r){"ok"===r.status?c.crm.content.load(c.crm.app_url+"deal/"+r.data.deal.id+"/"):r.errors&&b(r.errors)},
"json").always(function(){q.hide();m=!1;h.removeAttr("disabled").prop("disabled",!1)})}}function e(n){var p=g.$wrapper.find("#c-contact-edit-form").closest("form"),q=p.find(".js-phone-list"),r=p.find(".js-email-list");p=p.serializeArray();var u=["contact[firstname]","contact[middlename]","contact[lastname]","contact[name]"],v=!0;c.each(p,function(t,w){n.data.push({name:w.name,value:w.value});0<c.trim(w.value).length&&0<=u.indexOf(w.name)&&(v=!1)});v&&(n.errors.push({name:"contact[firstname]",value:g.locales.empty_name}),
n.errors.push({name:"contact[name]",value:g.locales.empty_name}));q.find("li").each(function(t){var w=c(this),x=w.find(".js-value");w=w.find(".js-ext");var y=x.val(),z=w.val();if(y){var B="contact[phone]["+t+"][value]";x.attr("name",B);n.data.push({name:B,value:y});t="contact[phone]["+t+"][ext]";w.attr("name",t);n.data.push({name:t,value:z})}});r.find("li").each(function(t){var w=c(this),x=w.find(".js-value");w=w.find(".js-ext");var y=x.val(),z=w.val();if(y){var B="contact[email]["+t+"][value]";x.attr("name",
B);n.data.push({name:B,value:y});t="contact[email]["+t+"][ext]";w.attr("name",t);n.data.push({name:t,value:z})}});return n}function f(n){var p=g.$wrapper.find("#c-contact-add-form").closest("form").serializeArray(),q=!1,r=!1;c.each(p,function(u,v){u=v.name;var t=v.value;"deal[contact_id]"===u&&t&&(q=!0);"contact[firstname]"===u&&t&&(r=!0);"contact[lastname]"===u&&t&&(r=!0);"contact[middlename]"===u&&t&&(r=!0);"contact[name]"===u&&t&&(r=!0);n.data.push(v)});q||r||(n.errors.push({name:"contact[name]",
value:g.locales.empty_name}),n.errors.push({name:"contact[firstname]",value:g.locales.empty_name}))}var g=this,l=g.$form,k=g.$form.find(".js-errors-place"),h=g.$wrapper.find(".js-submit-button"),m=!1;h.on("click",function(){l.trigger("submit")});l.on("submit",function(n){n.preventDefault();n=a();n.errors.length?b(n.errors):d(n.data)})};CRMDealEdit.prototype.initSuggestName=function(){var a=this,b=a.$form,d=a.$deal_name_input;if(!d.data("edited")){d.one("mark-edited.crm-deal-suggestion",function(){d.data("edited",
...
".js-crm-deal-name-suggester-input",function(l){var k=c(this);f&&clearTimeout(f);f=setTimeout(function(){if(k.hasClass("crm-find-contact-input")){if("keyup"===l.type)return;k=b.find('[name="deal[id]"]')}var h=k.attr("name")||"",m=c.trim(k.val());if(h){a.suggester_values[h]=c.trim(a.suggester_values[h]);if(a.suggester_values[h]===m)return;a.suggester_values[h]=m}g()},250)})}};CRMDealEdit.prototype.initChangeCompanyContact=function(){var a=this,b=a.$wrapper.find(".js-contact-block"),d=b.find(".js-view-toggle"),
...
var k=g.data("id"),h=l.data("stage-id"),m=l.data("id");h!==k&&(l.draggable("option","revert",!1),a(m,k,l,g));d.length&&(d.removeClass("is-wrapper"),d=!1)}})};CRMDealsFunnel.prototype.initMassiveActionsHeader=function(){function a(){if(c.contains(document,f[0]))if(d.selected_deals.length){f.hasClass("is-hidden")&&f.removeClass("is-hidden");var n=e.scrollTop();k.top+m>n+e.height()?l||(l=!0,g=c("<div />"),g.height(m).insertAfter(f),f.width(h).addClass("is-fixed-to-bottom")):b()}else b(),f.hasClass("is-hidden")||
f.addClass("is-hidden");else e.off("scroll",a)}function b(){g&&g.length&&g.remove();g=!1;f.removeAttr("style").removeClass("is-fixed-to-bottom");h=f.width();m=f.height();l=!1}var d=this,e=c(window),f=d.$actionsHeader,g=!1,l=!1,k=f.offset(),h=f.width(),m=f.height();e.on("scroll",a);e.on("resize",function(){b();e.trigger("scroll")})};CRMDealsFunnel.prototype.initMassActions=function(){function a(k){k.find(".js-check-deal").is(":checked")?(k.addClass("is-active"),l.push(k[0])):(k.removeClass("is-active"),
k=l.indexOf(k[0]),l.splice(k,1));b();c(window).trigger("scroll")}function b(){function k(){var p=d("open"),q=f.find(".js-deals-close").closest(".c-action"),r=q.find(".js-open-count");p.length?(r.text(p.length),q.show()):q.hide()}function h(){var p=f.find(".js-deals-merge").closest(".c-action");1<l.length?p.show():p.hide()}function m(){var p=!1;c(l).each(function(){if(c(this).data("can-delete"))return p=!0,!1});var q=f.find(".js-deals-delete").closest(".c-action");p?q.show():q.hide()}var n=l.length;
0<l.length&&(f.find(".js-count").text(n),k(),h(),m())}function d(k){var h=[];c.each(l,function(m,n){m=c(n);n=m.data("id");(!k||m.data(k))&&n&&h.push(n)});return h}var e=this,f=e.$actionsHeader,g=f.find(".js-checkbox-all"),l=e.selected_deals;e.$wrapper.on("click",".js-deal-wrapper",function(k){k=c(k.target);var h=!(!k.is("a")&&!k.closest("a").length);k=k.is(":checkbox");if(!h){h=c(this);var m=h.find(".js-check-deal");k||m.attr("checked",!m.is(":checked"));a(h)}});e.$tableContent.find(".js-state-column").shiftSelectable({selector:".js-deal-wrapper",
onSelect:function(k,h){var m=c(h.target);m=!(!m.is("a")&&!m.closest("a").length);var n=h.extra.is_first;h=h.extra.is_last;m||n||h||(k.find(".js-check-deal").attr("checked",!0),a(k))}});g.on("change",function(){var k=c(this).is(":checked");e.$wrapper.find(".js-deal-wrapper").each(function(){var h=c(this),m=h.find(".js-check-deal");m.is(":checked")!==k&&(m.attr("checked",k),a(h))})});f.on("click",".js-deals-merge",function(k){k.preventDefault();k=d();k=c.crm.app_url+"deal/merge/?ids="+k.join(",");c.crm.content.load(k)});
(function(){var k=!1;f.on("click",".js-deals-delete",function(h){h.preventDefault();c.crm.confirm.show({title:e.locales.delete_confirm_title.replace("%s",l.length),text:e.locales.delete_confirm_text,button:e.locales.delete_confirm_button,onConfirm:function(m){function n(){var u=[],v=d();c.each(v,function(t,w){u.push({name:"id[]",value:w})});return u}function p(){var u=l.map(function(v){return v});c.each(u,function(v,t){v=c(t);v.find(".js-check-deal").attr("checked",!1).trigger("change");v.remove()})}
if(!k){k=!0;var q=c.crm.app_url+"?module=deal&action=delete",r=n();c.post(q,r,function(u){"ok"===u.status&&(p(),m.close(),c.crm.content.reload(),c.crm.sidebar.reload())}).always(function(){k=!1})}return!1}})})})();(function(){var k=!1;f.on("click",".js-deals-change-responsible",function(h){h.preventDefault();k||(k=!0,h={ids:d().join(",")},c.post("?module=deal&action=changeResponsible",h,function(m){new CRMDialog({html:m})}).always(function(){k=!1}))})})();(function(){var k=!1;f.on("click",".js-deals-close",
function(h){h.preventDefault();k||(k=!0,h={ids:d("open").join(",")},c.post("?module=deal&action=massClose",h,function(m){new CRMDialog({html:m})}).always(function(){k=!1}))})})();(function(){var k=!1;f.on("click",".js-deals-change-funnel",function(h){h.preventDefault();k||(k=!0,h={deal_ids:d().join(",")},c.post("?module=deal&action=changeFunnel",h,function(m){new CRMDialog({html:m})}).always(function(){k=!1}))})})();(function(){var k=!1;f.on("click",".js-deals-export",function(h){h.preventDefault();
k||(k=!0,h=d(),c.post(c.crm.app_url+"?module=deal&action=export",{ids:h},function(m){new CRMDialog({html:m})}).always(function(){k=!1}))})})();(function(){var k=!1;f.on("click",".js-deals-set-tags",function(h){h.preventDefault();if(!k){k=!0;h=d();var m=c.crm.app_url+"?module=dealOperation&action=assignTags",n={deal_ids:h};1===h.length&&(n.is_assign=1);c.post(m,n,function(p){new CRMDialog({html:p})}).always(function(){k=!1})}})})()};return CRMDealsFunnel}(jQuery),CRMDealsList=function(c){CRMDealsList=
...
function(){this.initLazy();this.initElasticHeader();this.initMassActions()};CRMDealsList.prototype.initLazy=function(){function a(){if(c.contains(document,g[0])){var k=c(window).scrollTop(),h=e.height(),m=g.offset().top;k+h>=m&&(l||b())}else e.off("scroll",a)}function b(){var k={offset:g.data("offset")};d.stage_id&&(k.stage=d.stage_id);d.contact_id&&(k.user=d.contact_id);l=!0;c.post("?module=deal&action=list",k,function(h){var m=c(h).find(".js-lazyload");m.length?(d.limit+=k.offset,m.insertAfter(g),
g.remove(),g=m):g.remove();c(h).find(".c-deal-wrapper").insertAfter(f.find("tr:last"))}).always(function(){l=!1})}var d=this,e=c(window),f=d.$tableBody,g=d.$wrapper.find(".js-lazyload"),l=!1;g.length&&(e.on("scroll",a),g.offset().top<e.height()&&e.trigger("scroll"))};CRMDealsList.prototype.initElasticHeader=function(){function a(){c.contains(document,e[0])?b.scrollTop()>f.top?e.css({left:f.left}).width(d.outerWidth()).addClass("is-fixed"):e.removeAttr("style").removeClass("is-fixed"):b.off("scroll",
a)}var b=c(window),d=this.$tableWrapper,e=this.$tableHeaderW,f=d.offset();b.on("scroll",a)};CRMDealsList.prototype.initMassActions=function(){function a(n){n.find(".js-check-deal").prop("checked",!0).trigger("change",[!0])}function b(n){n.find(".js-check-deal").prop("checked",!1).trigger("change",[!1])}function d(n){n.find(".js-check-deal").is(":checked")?(n.addClass("is-active"),l.push(n[0])):(n.removeClass("is-active"),n=l.indexOf(n[0]),l.splice(n,1));e()}function e(){function n(){var u=f("open"),
...
!0):m.prop("indeterminate",!1).prop("checked",!0)):(k.show(),h.hide(),m.prop("indeterminate",!1).prop("checked",!1))}function f(n){var p=[];c.each(l,function(q,r){q=c(r);r=q.data("id");(!n||q.data(n))&&r&&p.push(r)});return p}var g=this,l=[],k=g.$tableHeader,h=g.$tableActions,m=g.$tableHeaderW.find(".js-checkbox-all");g.$tableBody.on("click",".js-deal-wrapper",function(n){n=c(n.target);var p=c(this),q=p.find(".js-check-deal");q.closest("td");var r=!(!n.is("a")&&!n.closest("a").length),u=!(!n.is(".c-column-checkbox")&&
!n.closest(".c-column-checkbox").length);if(n.is(":checkbox"))return d(p),!0;if(r)return!0;u&&(q.attr("checked",!q.is(":checked")),d(p))});g.$tableBody.shiftSelectable({selector:".js-deal-wrapper",onSelect:function(n,p){var q=c(p.target);q=!(!q.is("a")&&!q.closest("a").length);var r=p.extra.is_first;p=p.extra.is_last;q||r||p||(n.find(".js-check-deal").attr("checked",!0),d(n))}});m.on("change",function(){var n=c(this).is(":checked"),p=g.$wrapper.find(".js-deal-wrapper"),q=p.find(".js-check-deal").filter(":checked").length;
0<q?q<g.limit||q===g.limit?b(p):a(p):n?a(p):b(p);p.each(function(){d(c(this))})});h.on("click",".js-deals-merge",function(n){n.preventDefault();n=f();n=c.crm.app_url+"deal/merge/?ids="+n.join(",");c.crm.content.load(n)});(function(){var n=!1;h.on("click",".js-deals-delete",function(p){p.preventDefault();c.crm.confirm.show({title:g.locales.delete_confirm_title.replace("%s",l.length),text:g.locales.delete_confirm_text,button:g.locales.delete_confirm_button,onConfirm:function(q){function r(){var w=[],
x=f();c.each(x,function(y,z){w.push({name:"id[]",value:z})});return w}function u(){var w=l.map(function(x){return x});c.each(w,function(x,y){x=c(y);x.find(".js-check-deal").attr("checked",!1).trigger("change");x.remove()})}if(!n){n=!0;var v=c.crm.app_url+"?module=deal&action=delete",t=r();c.post(v,t,function(w){"ok"===w.status&&(u(),q.close(),k.show(),h.hide(),m.prop("indeterminate",!1).prop("checked",!1),c.crm.content.reload(),c.crm.sidebar.reload())}).always(function(){n=!1})}return!1}})})})();
(function(){var n=!1;h.on("click",".js-deals-change-responsible",function(p){p.preventDefault();n||(n=!0,p={ids:f().join(",")},c.post("?module=deal&action=changeResponsible",p,function(q){new CRMDialog({html:q})}).always(function(){n=!1}))})})();(function(){var n=!1;h.on("click",".js-deals-close",function(p){p.preventDefault();n||(n=!0,p={ids:f("open").join(",")},c.post("?module=deal&action=massClose",p,function(q){new CRMDialog({html:q})}).always(function(){n=!1}))})})();(function(){var n=!1;h.on("click",
".js-deals-change-funnel",function(p){p.preventDefault();n||(n=!0,p={deal_ids:f().join(",")},c.post("?module=deal&action=changeFunnel",p,function(q){new CRMDialog({html:q})}).always(function(){n=!1}))})})();(function(){var n=!1;h.on("click",".js-deals-export",function(p){p.preventDefault();n||(n=!0,p=f(),c.post(c.crm.app_url+"?module=deal&action=export",{ids:p},function(q){new CRMDialog({html:q})}).always(function(){n=!1}))})})();(function(){var n=!1;h.on("click",".js-deals-set-tags",function(p){p.preventDefault();
if(!n){n=!0;p=f();var q=c.crm.app_url+"?module=dealOperation&action=assignTags",r={deal_ids:p};1===p.length&&(r.is_assign=1);c.post(q,r,function(u){new CRMDialog({html:u})}).always(function(){n=!1})}})})()};return CRMDealsList}(jQuery),CRMDealsMergePage=function(c){CRMDealsMergePage=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.$submitButton=this.$wrapper.find(".js-submit-button");this.$activeDeal=this.is_deal_set=!1;this.initClass()};CRMDealsMergePage.prototype.initClass=
...
l)}var b=this,d=b.$wrapper.find(".js-funnels-list"),e=d.find(".js-visible-link"),f=d.find(".js-field"),g=d.find(".menu-v");g.on("click","a",function(){var l=c(this);e.find(".js-text").html(l.html());g.find(".selected").removeClass("selected");l.closest("li").addClass("selected");g.hide();setTimeout(function(){g.removeAttr("style")},200);l=l.data("id");f.val(l).trigger("change");a(l)})};CRMDealsChangeFunnelDialog.prototype.initChangeStage=function(){function a(k){l.html("");c.each(k,function(h,m){h=
b.stage_template_html;var n=c("<div />").text(m.name).html();h=h.replace("%id%",m.id).replace("%color%",m.color).replace("%name%",n);m=c(h);l.append(m)});c.crm.renderSVG(e);l.find("li:first-child a").trigger("click")}var b=this,d=b.$wrapper.find(".js-funnels-list"),e=b.$wrapper.find(".js-funnel-stages-list"),f=e.find(".js-visible-link"),g=e.find(".js-field"),l=e.find(".menu-v");l.on("click","a",function(){var k=c(this);f.find(".js-text").html(k.html());l.find(".selected").removeClass("selected");
...
this.confirm_dialog_template=d.confirm_dialog_template;this.shop_supported=d.shop_supported;this.tax_set_class="is-changed";this.wa_app_url=d.$wa_app_url;this.currencies=d.currencies;this.supported_currencies=d.supported_currencies;this.company_id=d.company_id;this.tax=d.tax&&d.tax.name?d.tax:!1;this.active_tax_array=[];this.tax_percent=c(this.$tax.find("option")[this.$tax[0].selectedIndex]).data("percent")||"0";this.tax_type=c(this.$tax.find("option")[this.$tax[0].selectedIndex]).data("type");this.initClass()};
CRMInvoiceEdit.prototype.initClass=function(){0==this.$wrapper.find('input[name="invoice[deal_id]"]').val()&&this.initSelectDeal();this.initDatePickers();this.initTableEvents();this.initWYSIWYG();this.initSubmit();this.initChangeContact();this.initDealActions();this.initChangeCurrency();this.shop_supported&&this.initProductName();this.initChangeTax();this.initChangeCompany();this.initToggleButton()};CRMInvoiceEdit.prototype.initTableEvents=function(){function d(f){if(c.contains(document,e.$table[0])){var g=
!0,l=e.$table.find(".js-tax-dropdown.is-active");f=c(f.target).closest(".js-tax-dropdown");f.length&&(g=f.hasClass("is-active"));l.removeClass("is-active");g||"NONE"===e.tax_type||f.addClass("is-active")}else c(document).off("click",d)}var e=this;e.$table.on("focus",".js-name-field",function(){var f=c(this).closest("tr"),g=e.$tableBody.find("tr").length;f.index()+1>=g&&e.addTableRow(g+1)});e.$table.on("keypress",".js-name-field",function(f){13===f.keyCode&&f.preventDefault()});e.$table.on("click",
".js-remove-row",function(f){f.preventDefault();1<e.$tableBody.find("tr").length&&e.removeTableRow(c(this).closest("tr"))});e.$table.on("change",".js-amount-field",function(){e.changeTableRow(c(this))});e.$table.on("change",".js-price-field",function(){var f=c(this).closest("tr").find(".js-amount-field");f.val()||f.val("1");e.changeTableRow(c(this))});e.$table.on("click",".js-set-product-tax",function(){var f=c(this),g=f.closest(".js-tax-dropdown"),l=g.find(".js-product-tax-percent"),k=g.find(".js-product-tax-type");
f=parseInt(f.data("tax-index"));if(f=e.active_tax_array[f])parseInt(e.tax_percent)===parseInt(f.tax_percent)&&e.tax_type===f.tax_type?g.removeClass(e.tax_set_class):g.addClass(e.tax_set_class),g=e.tax_type,"NONE"===f.tax_type&&(g="NONE"),k.val(g).trigger("change"),k=f.tax_percent+"%","NONE"===f.tax_type&&(k=e.companies[e.$companyField.val()],k=e.locales.no_tax.replace("%name",k.tax_name)),l.val(k).trigger("change"),e.refreshTable()});c(document).on("click",d)};CRMInvoiceEdit.prototype.addTableRow=
function(d){var e=c(this.row_template_html);e.find(".js-product-number").text(d);this.renderProductTaxOptions(e);this.$tableBody.append(e);this.renderProductTaxFields();this.shop_supported&&this.$wrapper.trigger("addProduct",e)};CRMInvoiceEdit.prototype.removeTableRow=function(d){d.remove();this.$tableBody.find("tr").each(function(e){c(this).find(".js-product-number").text(e+1)});this.refreshTable()};CRMInvoiceEdit.prototype.changeTableRow=function(d){var e=this.locale,f="price"===d.data("type"),
g=d.val(),l=0;g&&g.length&&(g=g.replace(/,/g,".").replace(/\s/g,""),0<parseFloat(g)&&(l=parseFloat(g)));d.val(f?a(l,e):l);this.refreshTable()};CRMInvoiceEdit.prototype.refreshTable=function(){var d=this,e=function(){var k=[];d.$tableBody.find("tr").each(function(){var h=c(this),m=h.find(".js-amount-field"),n=h.find(".js-price-field"),p=h.find(".js-product-total"),q=h.find(".js-product-tax-percent");m=b(m.val());n=b(n.val());var r=0;0<m&&0<n&&(r=n*m);p.text(a(r,d.locale));(p=parseInt(q.val()))&&(0<
p||0===p)||(p=0);k.push({$tr:h,percent:p,amount:m,price:n,total:r})});return k}(),f=d.tax_type&&"INCLUDE"===d.tax_type,g=0,l=0;c.each(e,function(k,h){g+=h.total;k=h.percent/100;l+=f?k/(1+k)*h.total:h.total*k});d.$table.find(".js-subtotal").text(a(g,d.locale));d.$table.find(".js-tax").text(a(l,d.locale));d.$table.find(".js-total").text(a(g+(f?0:l),d.locale))};CRMInvoiceEdit.prototype.renderProductTaxOptions=function(d){var e=this,f=!1,g;f=d&&d.length?d.find(".js-tax-dropdown"):e.$tableBody.find(".js-tax-dropdown");
var l=f.find(".js-hidden-list").html(""),k=e.companies[e.$companyField.val()],h=[];c.each(e.active_tax_array,function(m,n){g="NONE"===n.tax_type?e.locales.no_tax.replace("%name",k.tax_name):n.tax_percent+"%";0>h.indexOf(g)&&(h.push(g),l.append('<li class="ui-menu-item-html ui-menu-item"><div class="ui-menu-item-wrapper"><div class="js-set-product-tax" data-tax-index="'+m+'">'+g+"</div></div></li>"))})};CRMInvoiceEdit.prototype.renderProductTaxFields=function(d){var e=this,f=e.$tableBody.find(".js-tax-dropdown");
d&&f.removeClass(e.tax_set_class);f.each(function(){var g=c(this),l=g.find(".js-product-tax-percent"),k=g.find(".js-product-tax-type");g.hasClass(e.tax_set_class)&&"NONE"!==e.tax_type||(k.val(e.tax_type).trigger("change"),k=e.tax_percent+"%","NONE"===e.tax_type&&(k="\u2014",g.removeClass(e.tax_set_class)),l.val(k).trigger("change"))})};CRMInvoiceEdit.prototype.initSubmit=function(){function d(){var h={data:[],errors:[]},m=l.serializeArray();c.each(m,function(n,p){"invoice[contact]"!==p.name&&h.data.push(p);
"invoice[contact_id]"===p.name&&(0<p.value||h.errors.push({name:"invoice[contact_id]",value:g.locales.empty_contact}))});(function(n){g.$tableBody.find("tr").each(function(p){var q=c(this),r=q.find(".js-product-field"),u=q.find(".js-amount-field"),v=q.find(".js-price-field"),t=q.find(".js-product-tax-percent"),w=q.find(".js-product-tax-type");q=q.find(".js-name-field");u=b(u.val());v=b(v.val());t=parseInt(t.val());w=w.val();r=r.val();q=q.val();if(u||v||q)n.push({name:"items["+p+"][name]",value:q}),
n.push({name:"items["+p+"][price]",value:v}),n.push({name:"items["+p+"][quantity]",value:u}),n.push({name:"items["+p+"][tax_percent]",value:0<t||0===t?t:""}),n.push({name:"items["+p+"][tax_type]",value:w});r&&n.push({name:"items["+p+"][product_id]",value:r})})})(h.data);return h}function e(h,m){m=m?m:[];if(h){var n=Object.keys(h);c.each(n,function(p,q){m.push({name:q,value:h[q]})})}c.each(m,function(p,q){p=q.value;var r=g.$form.find('[name="'+q.name+'"]');if(r.length&&!r.hasClass("state-error")){var u=
c("<span />").addClass("state-error-hint").text(p);u.insertAfter(r);r.addClass("state-error").one("focus click change",function(){r.removeClass("state-error");u.remove()})}})}function f(h,m){if(!k){k=!0;var n=c.crm.app_url+"?module=invoice&action=save",p=spinnerView(m);p.show();c.post(n,h,function(q){if("ok"===q.status){if(q.data.id){c(document).trigger("unsavedChanges",!1);var r=c.crm.app_url+"invoice/"+q.data.id+"/",u=getIframeIfExists();u?(window.parent.iframe_invoice_is_saved=!0,u.dispatchEvent(new CustomEvent("close",
{detail:{id:q.data.id}}))):c.crm.content.load(r)}}else p.hide(),e(q.errors)},"json").always(function(){k=!1})}}var g=this,l=g.$form,k=!1;l.on("submit",function(h){h.preventDefault();h=d();h.errors.length?e(!1,h.errors):f(h.data,l.find(".js-submit-button"))})};CRMInvoiceEdit.prototype.initDatePickers=function(){var d=this,e=d.$wrapper.find(".js-invoice-datepicker"),f=d.$wrapper.find(".js-invoice-due-datepicker"),g=d.$wrapper.find(".js-due-days-field"),l=d.$wrapper.find(".js-clear-due-date");(function(){var k=
e.parent().find("input[type='hidden']");e.datepicker({altField:k,altFormat:"yy-mm-dd",changeMonth:!0,changeYear:!0});d.invoice_id||e.datepicker("setDate","+0d")})();(function(){var k=f.parent().find("input[type='hidden']");f.datepicker({altField:k,altFormat:"yy-mm-dd",changeMonth:!0,changeYear:!0});f.on("change",function(){var h=c(this).val();c.trim(h).length?l.show():(k.val(""),l.hide())})})();d.$wrapper.on("click",".js-focus-on-field",function(){c(this).parent().find(".js-invoice-due-datepicker").trigger("focus")});
g.on("change",function(){var k=g.val(),h=0;k&&k.length&&0<parseInt(k)&&(h=parseInt(k),g.val(h));0<h?l.show():l.hide();g.val(h);k=h;h=e.datepicker("getDate");k=new Date(h.getTime()+864E5*k);f.datepicker("setDate",k)});e.on("change",function(){var k=g.val();k&&k.length&&0<parseInt(k)&&g.trigger("change")});f.on("change",function(){g.val("")});l.on("click",function(){f.val("").trigger("change")})};CRMInvoiceEdit.prototype.initChangeContact=function(){var d=this,e=d.$wrapper.find(".js-contact-toggle"),
f=e.find(".js-name"),g=e.find(".js-company-name"),l=e.find(".js-profile-image img"),k=e.find(".js-contact-id"),h=!1;e.on("click",".js-change-contact",function(m){m.preventDefault();var n=c(this);c.get(c.crm.app_url+"?module=invoice&action=contactAdd",function(p){c.waDialog({html:p,options:{onAdd:function(q,r,u,v){k.val(q).trigger("change");f.text(r);g.length&&v?g.text(v):g.text("");l.length&&u?l.attr("src",u):l.attr("src",d.wa_app_url+"wa-content/img/userpic.svg");h||(q=n.find(".js-label"),r=q.data("change-text"),
e.find(".c-profile-block").show(),q.find("span").text(r),q.find("svg").addClass("fa-pen"),h=!0)}}})})})};CRMInvoiceEdit.prototype.initDealActions=function(){function d(l,k){var h=null;l.on("click",".js-detach-deal",function(m){m.preventDefault();c.crm.confirm.show({title:e.locales.deal_detach_title,text:e.locales.deal_detach_text.replace(/%s/,k),button:e.locales.deal_detach_confirm_button,onConfirm:function(){h&&h.abort();e.loadDealListByContact(function(n){h=c.post(c.crm.app_url+"?module=invoiceDeal&action=detach",
{invoice_id:e.invoice_id}).done(function(p){p&&"ok"===p.status&&(c.crm.content.reload(),l.data("dialog").close())}).always(function(){h=null})})}})})}var e=this,f=e.$wrapper.find(".js-deal-toggle"),g=e.$wrapper.find(".js-deal-toggle [name='invoice[deal_id]']").val();f.on("click",".js-attach-other-deal, .js-associate-deal",function(l){l.preventDefault();var k=f.find(".js-deal-name").text();e.loadDealListByContact(function(h){c.get(c.crm.app_url+"?module=invoiceDeal&action=attachDialog",{invoice_id:e.invoice_id,
deal_id:g},function(m){var n=c(m);c.waDialog({html:n,onOpen:function(){var p=n.find(".js-invoice-deal"),q=e.initSelectDeal({$wrapper:p,data:h});n.find(".js-save-button").on("click",function(){q.submit()});d(n,k)}})})})});f.on("click",".js-detach-deal",function(l){l.preventDefault();var k=null;l=f.find(".js-deal-name").text();c.crm.confirm.show({title:e.locales.deal_detach_title,text:e.locales.deal_detach_text.replace(/%s/,l),button:e.locales.deal_detach_confirm_button,onConfirm:function(){k&&k.abort();
e.loadDealListByContact(function(h){k=c.post(c.crm.app_url+"?module=invoiceDeal&action=detach",{invoice_id:e.invoice_id}).done(function(m){m&&"ok"===m.status&&c.crm.content.reload()}).always(function(){k=null})})}})})};CRMInvoiceEdit.prototype.initSelectDeal=function(d){function e(){t.html(g.locales.deal_empty);z.val("none");m.val("");h.addClass("c-deal-name-hidden");p.addClass("hidden").removeAttr("title");v.addClass("c-empty-deal-hidden");w.addClass("hidden");y.find("li").removeClass("selected");
0<q.data("deals_count")?(q.removeClass("hidden"),r.addClass("hidden")):(q.addClass("hidden"),r.removeClass("hidden"))}function f(){var C=l.find(".js-created-deal"),A=x.find(".js-visible-link .js-text .funnel-state").clone(),D=c.trim(m.val()),E=k.serializeObject();E.invoice_id=g.invoice_id;E["deal[id]"]=z.val();E["deal[name]"]=m.val();E["deal[funnel_id]"]=k.find(".js-select-deal-funnel").val();E["deal[stage_id]"]=k.find(".js-select-deal-stage").val();if("none"===z.val())return k.addClass("shake animated"),
setTimeout(function(){k.removeClass("shake animated")},500),!1;if(0>=z.val()&&!D)return h.addClass("shake animated"),setTimeout(function(){h.removeClass("shake animated");m.focus()},500),!1;k.addClass("deal-form-hidden");C.removeClass("hidden");0>=z.val()?(C.html(A),C.append(c.crm.escape(D))):(D=q.find(".js-visible-link .js-text"),A=D.find(".funnel-state").clone(),D=D.find(".deal-item--name").text(),C.html(A),C.append(c.crm.escape(D)));c.post(c.crm.app_url+"?module=invoice&action=associateDealSave",
E,function(F){"ok"===F.status?(l.closest(".dialog").data("dialog").close(),c.crm.content.reload()):(C.html(""),C.addClass("hidden"),e(),k.removeClass("deal-form-hidden"))})}d=d||{};var g=this,l=d.$wrapper||g.$wrapper,k=l.find(".js-deal-selector-control-wrapper"),h=k.find(".js-deal-name"),m=k.find(".js-deal-name-input"),n=l.closest(".js-conversation-deal-attach-dialog").find(".js-save-button"),p=k.find(".js-save-deal"),q=k.find(".js-deals-dropdown"),r=k.find(".js-create-new-deal-link"),u=k.find(".js-remove-deal"),
v=q.find(".js-empty-deal"),t=k.find(".js-select-deal .js-visible-link .js-text"),w=k.find(".js-select-funnel-wrapper"),x=k.find(".js-select-stage-wrapper"),y=k.find(".js-deals-list"),z=k.find(".js-deal-id"),B=function(C){C=C||{};var A=C.funnels||{},D=0;c.each(C.deals||{},function(E,F){E=y;var G=E.prepend;var H=A[F.funnel_id];if(!F||0>=F.id)F="";else{var I=F.id,K=F.name||"",J="",L="";H&&H.stages&&H.stages[F.stage_id]&&(J=H.stages[F.stage_id].color||"");c.isEmptyObject(H)&&(L='<span class="hint">'+
g.locales.funnel_deleted+"</span>");F='<li><a href="javascript:void(0);" class="js-deal-item" data-deal-id="'+I+'"><span class="flexbox middle js-text"><i class="fas fa-circle funnel-state" style="color: '+J+'"></i><span class="deal-item--name" style="word-break: break-word;">'+K+"</span></span>"+L+"</a></li>"}G.call(E,F);D++});k.show();q.data("deals_count",D);0<D?(q.removeClass("hidden"),r.addClass("hidden")):(r.removeClass("hidden"),q.addClass("hidden"))};z.val("none");"undefined"===typeof d.data?
g.loadDealListByContact(B):B(d.data);k.on("click",".js-create-new-deal",function(){z.val("0");q.addClass("hidden");r.addClass("hidden");h.removeClass("c-deal-name-hidden");p.attr("title",g.locales.deal_create).removeClass("hidden");w.removeClass("hidden");v.removeClass("c-empty-deal-hidden");m.focus();n.addClass("yellow")});k.on("click",".js-deal-item",function(){var C=c(this).find(".js-text").html();t.html(C);z.val(c(this).data("deal-id"));p.attr("title",g.locales.deal_add).removeClass("hidden");
w.addClass("hidden");v.removeClass("c-empty-deal-hidden");y.find("li").removeClass("selected");c(this).parent().addClass("selected");n.addClass("yellow")});y.on("click",function(){y.hide();setTimeout(function(){y.removeAttr("style")},200)});v.on("click",function(){e()});u.on("click",function(){e()});p.on("click",function(C){C.preventDefault();k.trigger("submit")});k.on("submit",function(C){C.preventDefault();f()});k.on("change",".js-select-deal-funnel",function(){k.find(".js-select-stage-wrapper").load("?module=deal&action=stagesByFunnel&id="+
c(this).val())});return{submit:function(){k.trigger("submit")}}};CRMInvoiceEdit.prototype.loadDealListByContact=function(d){var e=this.$wrapper.find(".js-contact-toggle .js-contact-id").val(),f="?module=deal&action=byContact&id="+e;d=d||function(){};0>=e||this.iframe?d({}):c.get(f,"json").always(function(g){g&&"ok"===g.status?d(g.data||{}):d({})})};CRMInvoiceEdit.prototype.initChangeCompany=function(){function d(l,k,h){l&&l.length&&(e.active_tax_array=l);g.find("option").remove();e.$taxHeader.text(k.length?
k:"\u2014");l&&k?(e.$emptyTax.hide(),e.$tax.parent().show(),c.each(l,function(m,n){m=c('<option value="'+m+'"></option>');if("INCLUDE"===n.tax_type){m.data("type","INCLUDE");var p=e.locales.include_tax}else"APPEND"===n.tax_type?(m.data("type","APPEND"),p=e.locales.append_tax):"NONE"===n.tax_type&&(m.data("type","NONE"),n.tax_percent=0,p=e.locales.no_tax);0<n.tax_percent?(m.data("percent",n.tax_percent),p=p.replace("%value",n.tax_percent+"%")):(m.data("percent",0),p=p.replace("%value",""));if(p=p.replace("%name",
k)){if(e.tax){var q=k===e.tax.name,r=n.tax_type===e.tax.type;n=(+n.tax_percent).toFixed(2)===(+e.tax.percent).toFixed(2);q&&r&&n&&m.attr("selected",!0)}m.text(p);m.data("name",k);g.append(m)}})):(e.$tax.parent().hide(),e.$emptyTax.show(),l=e.$emptyTaxLink.data("href"),e.$emptyTaxLink.attr("href",l+e.$companyField.val().toString()+"/"));g.trigger("change",!h)}var e=this,f=e.$companyField,g=e.$tax;f.on("change",function(l,k){l=c(this).val();var h=e.companies[l];e.active_tax_array=[];h&&(l=h.tax_name||
e.locales.tax,h=JSON.parse(h.tax_options),d(h,l,k))});f.trigger("change",!0)};CRMInvoiceEdit.prototype.initChangeTax=function(){var d=this;d.$tax.on("change",function(e,f){e=c(this).find("option");d.tax_percent=0;d.tax_type="NONE";d.tax_name="";e.length&&(e=e[this.selectedIndex],d.tax_percent=c(e).data("percent")||0,d.tax_type=c(e).data("type"),d.tax_name=c(e).data("name"));d.$taxPercent.val(d.tax_percent);d.$taxType.val(d.tax_type);d.$taxName.val(d.tax_name);d.renderProductTaxOptions();d.renderProductTaxFields(f);
d.refreshTable()})};CRMInvoiceEdit.prototype.initWYSIWYG=function(){var d=this.$wrapper.find(".js-wysiwyg");c.crm.initWYSIWYG(d,{keydownCallback:function(e){}})};CRMInvoiceEdit.prototype.initProductName=function(){function d(g){function l(x){w&&w.abort();var y=c.crm.backend_url+"shop/?action=autocomplete&with_counts=1";x={term:x};h("loading");w=c.get(y,x,function(z){z.length?(r.html(""),c.each(z,function(B,C){B=c("<div />");B.html(C.label).data("product",C).addClass("js-set-product");h(B)})):e.$wrapper.trigger("searchProduct")},
"json").always(function(){w=!1})}function k(x){w&&w.abort();h("loading");var y=e.$currencyField.val();w=c.get(c.crm.backend_url+"shop/?module=orders&action=getProduct",{product_id:x.id,currency:y},function(z){"ok"===z.status?(r.html(""),c.each(z.data.sku_ids,function(B,C){B=c("<div />");C=z.data.product.skus[C];B.text(z.data.product.name+(C.name.length?" ("+C.name+")":"")).data("product",z.data.product).data("sku",C).addClass("js-set-sku");h(B);1===z.data.sku_ids.length&&B.trigger("click")})):e.$wrapper.trigger("searchProduct")},
"json").always(function(){w=!1})}function h(x){if("loading"===x)r.html("").append('<li class="ui-menu-item-html ui-menu-item"><div class="ui-menu-item-wrapper"><i class="fas fa-spinner wa-animation-spin"></i></div></li>');else{var y=c("<li />").addClass("ui-menu-item-html ui-menu-item");c("<div />").addClass("ui-menu-item-wrapper").append(x).appendTo(y);y.appendTo(r)}}function m(){g.removeClass("is-active");r.html("");v=!1}function n(x){var y=r.find("li"),z=y.filter(".is-highlighted");if(!y.length)return!1;
...
q.trigger("blur");break;case 38:n(!1);break;case 40:n(!0)}else{var y=c(this).val();1<y.length?(v||(e.$wrapper.trigger("searchProduct"),g.addClass("is-active"),v=!0),clearTimeout(t),t=setTimeout(function(){l(y)},500)):e.$wrapper.trigger("searchProduct")}});q.on("change",function(){u.val("").trigger("change")});e.$wrapper.on("searchProduct",m);r.on("click",".js-set-product",function(){var x=c(this).data("product");x&&k(x)});r.on("click",".js-set-sku",function(){var x=c(this),y=x.data("product");x=x.data("sku")||
!1;if(y){var z="shop:"+y.id+(x?":"+x.id:""),B=x.price;q.val(y.name+(x.name.length?" ("+x.name+")":"")).trigger("change");g.closest("tr").find(".js-price-field").val(B).trigger("change");u.val(z).trigger("change")}m();q.trigger("blur")})}var e=this,f=e.$wrapper.find(".js-name-autocomplete");f.length&&(f.each(function(){d(c(this))}),e.refreshTable());e.$wrapper.on("addProduct",function(g,l){g=c(l).find(".js-name-autocomplete");g.length&&d(g)})};CRMInvoiceEdit.prototype.initChangeCurrency=function(){function d(g,
l){e.$tableBody.find(".js-price-field").each(function(){var k=c(this),h=k.val(),m=0;h&&h.length&&(h=h.replace(/,/g,".").replace(/\s/g,""),0<parseFloat(h)&&(m=parseFloat(h)),k.val(m*e.currencies[l].rate/e.currencies[g].rate).trigger("change"))})}var e=this,f=e.$currencyField.val();e.$currencyField.on("change",function(){var g=e.$currencyField.val(),l=!1;e.$tableBody.find(".js-price-field").each(function(){if(c(this).val().length)return l=!0,!1});l?(e.$currencyField.val(f),c.waDialog({html:e.confirm_dialog_template,
options:{changeCurrencyWithPrice:function(){e.$currencyField.val(g);d(g,f);f=g},changeCurrency:function(){e.$currencyField.val(g);f=g}}})):f=g})};CRMInvoiceEdit.prototype.initToggleButton=function(){var d=this.$wrapper.find(".js-footer-actions"),e=d.find(".js-submit-button");this.$wrapper.on("change keydown","input, textarea, select",function(){e.removeClass("green").addClass("yellow");d.addClass("is-changed");c(document).trigger("unsavedChanges",!0)})};return CRMInvoiceEdit}(jQuery),CRMInvoiceContactAdd=
function(c){CRMInvoiceContactAdd=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.dialog=this.$wrapper.data("dialog");this.locales=a.locales;this.initClass()};CRMInvoiceContactAdd.prototype.initClass=function(){function a(){var g=e.$form.serializeArray(),l={data:[],errors:[]},k=!1,h=!1;c.each(g,function(m,n){m=n.name;var p=n.value;"deal[contact_id]"===m&&p&&(k=!0);"contact[firstname]"===m&&p&&(h=!0);"contact[lastname]"===m&&p&&(h=!0);"contact[middlename]"===m&&p&&(h=
!0);"contact[name]"===m&&p&&(h=!0);l.data.push(n)});k||h||(l.errors.push({name:"contact[name]",value:e.locales.empty_name}),l.errors.push({name:"contact[firstname]",value:e.locales.empty_name}));return l}function b(g){c.post("?module=invoice&action=contactAddSave",g,function(l){"ok"===l.status?(e.dialog.options.onAdd(l.data.id,l.data.name,!1,!1),e.dialog.close()):d(l.errors)},"json").always(function(){f=!1})}function d(g,l){l=l?l:[];if(g){var k=Object.keys(g);c.each(k,function(h,m){l.push({name:m,
value:g[m]})})}c.each(l,function(h,m){h=m.name;m=m.value;"name"===h&&(h="contact[firstname]");var n=e.$form.find('[name="'+h+'"]');"contact[firstname]"!==h||n.is(":visible")||(n=e.$form.find(".js-contact-autocomplete"));if(n.length&&!n.hasClass("state-error")){var p=c("<div />").addClass("state-error-hint").text(m);p.insertAfter(n);n.addClass("state-error").one("focus click",function(){n.removeClass("state-error");p.remove()});c(document).on("toggleChanged",function(){n.removeClass("state-error");
p.remove()})}})}var e=this,f=!1;e.$form.on("submit",function(g){g.preventDefault();g=e.$wrapper.find(".js-contact-id-field").val();var l=!1;if(g){var k=e.$wrapper.find(".js-contact-autocomplete");var h=k.val();is_company=k.data("is_company");+is_company||(l=e.$wrapper.find(".js-company-autocomplete").val());k=k.data("photo_url");e.dialog.options.onAdd(g,h,k,l);e.dialog.close()}else f||(f=!0,g=a(),g.errors.length?(d(!1,g.errors),f=!1):b(g.data))})};return CRMInvoiceContactAdd}(jQuery),CRMInvoicePage=
function(c){function a(b,d){if(!b)return!1;b=c('.js-invoices-list .c-invoice[data-id="'+b+'"]');b.length&&(d?b.replaceWith(d):b.remove())}CRMInvoicePage=function(b){this.$wrapper=b.$wrapper;this.invoice_id=b.invoice_id;this.locales=b.locales;this.initClass()};CRMInvoicePage.prototype.initClass=function(){var b=this;b.initChangeState();b.initDelete();b.initRefund();b.initRestore();setTimeout(function(){c(document).trigger("viewInvoice",b.invoice_id)},100);b.$wrapper.data("page",b);c(".js-action-link").one("click",
function(d){spinnerView(c(this)).show()})};CRMInvoicePage.prototype.initChangeState=function(){function b(g,l){if(!f){f=!0;var k={invoice_id:e,action:g},h=spinnerView(l);h.show();c.post("?module=invoice&action=handleTransaction",k,function(m){if("ok"===m.status){var n=getIframeIfExists();n?(window.parent.iframe_invoice_is_saved=!0,n.contentWindow.location.reload()):(a(d.invoice_id,"delete"===g?"":m.data.html),d.load(c.crm.app_url+"invoice/"+d.invoice_id+"/"))}else alert(m.errors)},"json").always(function(){h.hide();
...
!0;var g=c.crm.app_url+"?module=invoice&action=refundDialog",l={invoice_id:d.invoice_id},k=spinnerView(f,0);k.show();c.post(g,l,function(h){c.waDialog({html:h,options:{onRefund:function(m){var n=getIframeIfExists();n?(window.parent.iframe_invoice_is_saved=!0,n.contentWindow.location.reload()):(a(d.invoice_id,m.html),d.reload())}}})}).always(function(){k.hide();e=!1})}}var d=this,e=!1;d.$wrapper.on("click",".js-change-refund",function(f){f.preventDefault();b(c(this))})};CRMInvoicePage.prototype.initRestore=
...
b.$wrapper.remove();d.removeClass("is-locked")})};CRMInvoices.prototype.initContentRouter=function(){function b(k){l&&l.abort();c(document).trigger("wa_before_load");return l=c.ajax({method:"GET",url:k,data:{content_only:1},dataType:"html",global:!1,cache:!1,xhr:function(){var h=new window.XMLHttpRequest;h.addEventListener("progress",function(m){c(document).trigger("wa_loading",m)},!1);h.addEventListener("abort",function(m){c(document).trigger("wa_abort")},!1);return h}}).always(function(){d.xhr=
!1}).done(function(h){g&&history.pushState({reload:!0,content_uri:k},"",k);c(document).trigger("wa_before_render");e.html(h);c(document).trigger("wa_loaded")}).fail(function(h){c(document).trigger("wa_abort");h.responseText&&c.waDialog.alert({title:"Error",text:h.responseText,button_title:"Close"})})}var d=this,e=d.$content.find(".js-inner-content"),f=!1,g=!(!history||!history.pushState),l=!1;d.$wrapper.on("click","a",function(k){var h=c(this),m=h.hasClass("js-disable-router")?h.attr("href"):!1;if(!m)return!0;
k.preventDefault();f?c.waDialog.confirm({title:c.crm.locales.unsaved_dialog_title,text:c.crm.locales.unsaved_dialog_text,success_button_title:c.crm.locales.unsaved_dialog_button,success_button_class:"danger",cancel_button_title:c.crm.locales.cancel,cancel_button_class:"light-gray",onSuccess:function(){c(document).trigger("unsavedChanges",!1);b(m)}}):b(m).then(function(){d.$content.is(":hidden")&&d.showContentMobile()})});c(document).on("unsavedChanges",function(k,h){f=!!h})};CRMInvoices.prototype.initLazy=
function(){function b(){if(!g){var l={offset:f.data("offset")},k=a();k&&(l.invoice_id=k);g=!0;c.post("?module=invoice&action=sidebar",l,function(h){var m=c(h);h=m.find(".js-lazyload");m=m.find(".c-invoice");e.unobserve(f[0]);h.length?(h.insertAfter(f),f.remove(),f=h,e.observe(f[0])):f.remove();d.append(m)}).always(function(){g=!1})}}var d=this.$invoiceList,e=null,f=this.$wrapper.find(".js-lazyload"),g=!1;this.$invoiceList.on("touchstart",function(){c("body").css({overflow:"hidden"});c(this).closest(".sidebar-body").css({overflow:"overlay"});
...
g.name})[0].value;e.date_time=a.due_date===b&&a.due_time===f&&a.user_contact.id===d?!0:!1;return e}return this.initialFormValues===b};CRMReminderFormEdit.prototype.initSubmit=function(){function a(){var n={data:[],errors:[]},p=l.serializeArray(),q=!1;type_other=!1;c.each(p,function(r,u){u.value.length&&(n.data.push(u),"data[due_time]"===u.name&&(q=!0),"OTHER"===u.value&&(type_other=!0))});n.data.length<(q?5:type_other?4:3)&&n.errors.push({name:"content",value:f.locales.at_least});type_other&&(p=n.data.filter(function(r){return"data[content]"===
r.name}),p.length&&/\S/.test(p[0].value)||n.errors.push({name:"content",value:f.locales.empty}));return n}function b(n,p){p=p?p:[];if(n){var q=Object.keys(n);c.each(q,function(r,u){p.push({name:u,value:n[u]})})}c.each(p,function(r,u){r=u.value;var v=l.find('[name="data['+u.name+']"');if(!v.hasClass("error")){var t=c("<span />").addClass("errormsg").text(r);h.find(".flexbox.vertical.width-100").append(t);v.addClass("error").one("focus click change clear_error",function(){v.removeClass("error");t.remove()});
$has_errors=!0}})}function d(n,p){if(!g){g=!0;var q=f.app_url+"?module=reminder&action=save";$loader=c('<span class="icon"><i class="fas fa-spinner wa-animation-spin"></i></span>');h.find(".c-actions-button.js-save").append($loader);c.post(q,n,function(r){"ok"===r.status?(p?(k.trigger("reminderIsChanged"),c(document).off("click",f.editClickWatcher),k.off("escKeyPress",f.$clear)):e(n),f.initialFormValues=f.$form.serialize(),f.form_is_change=!1):b(r.errors)},"json").always(function(){$loader.remove();
g=!1})}}function e(n){var p=n.filter(function(r){return"data[content]"===r.name});n=n.filter(function(r){return"data[type]"===r.name})[0].value;f.tempFormData.content!==p&&(p.length?(f.tempFormData.content=p[0].value,f.$textField.html((p[0].value+"").replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g,"$1<br />$2"))):(f.tempFormData.content="",f.$textField.html("")));if(f.tempFormData.type!==n){p=f.$wrapper.find(".js-reminder-type-toggle button");var q=f.$main_wrapper.find("button.no-hover");"OTHER"===n?q.remove():
q.length?q.html(p.html()):(p='<button class="button button-slim smallest rounded light-gray no-hover">'+p.html()+"</button>",f.$main_wrapper.find(".is-view .c-footer").prepend(p));f.tempFormData.type=n}f.tempFormData.contact.id!==f.initialFormData.contact.id&&f.tempFormData.contact.name!==f.initialFormData.contact.name&&(n=f.$main_wrapper.find(".is-view .c-deal-wrapper"),""!==f.tempFormData.contact.id?n.html(f.setDeal(f.tempFormData.contact,!0)):n.html(""));f.initialFormData=jQuery.extend(!0,{},f.tempFormData);
k.addClass("highlighted");k.trigger("reminderNotChanged");k.trigger("updatePadding")}var f=this,g=!1,l=f.$form,k=f.$main_wrapper,h=f.$wrapper,m=l.find(".js-save");$cancel_button=l.find(".js-cancel");$has_errors=!1;m.on("click",function(n){n.stopPropagation();f.$form.trigger("submit")});$cancel_button.on("click",function(n){n.stopPropagation();n.preventDefault();f.$clear()});f.$clear=function(){var n=l.find(".js-textarea"),p=l.find(".js-datepicker"),q=l.find(".js-timepicker"),r=l.find(".c-search-contact-wrapper").hasClass("is-hidden"),
u=l.find(".js-contact-field");if(!f.checkFormChange()){var v=f.initialFormData;n.val(v.content);h.find(".menu a[data-type-id='"+v.type+"']").trigger("click");p.val(v.due_date).trigger("change");q.val(v.due_time).trigger("change");v.contact.id?f.setDeal(v.contact):r&&f.clearDeal();u!==v.user_contact.id&&f.setContact(v.user_contact);$has_errors&&(l.find('[name="data[content]"').trigger("clear_error"),$has_errors=!1);n.css("height",0);n.css("height",n[0].scrollHeight+"px")}f.close_edit()};l.on("submit",
function(n){n.preventDefault();n=f.checkFormChange(!0);n.all?f.close_edit():n.date_time?f.$submit_reminder():f.$submit_reminder(!0)});f.$submit_reminder=function(n){$has_errors&&(l.find('[name="data[content]"').trigger("clear_error"),$has_errors=!1);var p=a();p.errors.length?b(!1,p.errors):d(p.data,n)}};return CRMReminderFormEdit}(jQuery);var CRMReminders=function(c){CRMReminders=function(a){this.$wrapper=a.$wrapper;this.$sidebar=a.$sidebar;this.$iframe=a.iframe;this.$completedSection=this.$wrapper.find(".c-completed-reminders-section");this.user_id=a.is_all_reminders?"all":a.user_id;this.setting_contact_id=a.setting_contact_id;this.setting_deal_id=a.setting_deal_id;this.current_page=a.current_page;this.assign_to_user=a.assign_to_user;this.locales=a.locales;this.click_reminder_event=!1;this.initClass()};CRMReminders.prototype.initClass=
...
y.autocomplete({appendTo:x,source:c.crm.app_url+"?module=autocomplete&type=user",minLength:0,html:!0,focus:function(){return!1},select:function(z,B){n.setContact(B.item);h(!1);y.val("");return!1}}).data("ui-autocomplete")._renderItem=function(z,B){return c("<li />").addClass("ui-menu-item-html").append("<div>"+B.value+"</div>").appendTo(z)};y.on("focus",function(){y.data("uiAutocomplete").search(y.val())})})();n.setContact=function(y){var z=t.find(".js-user");y.photo_url&&z.find(".icon.size-24").css("background-image",
...
p.addClass(extended_class),wrapper_is_open=!0,is_first_click&&(k($wrapperContact),m(),is_first_click=!1),c(document).on("click",b))});r.on("input",l);r.on("keydown",function(t){var w=t.keyCode;wrapper_is_open&&13===w&&!t.shiftKey&&(t.preventDefault(),t.stopPropagation(),a()?d():q.trigger("submit"))});$plus_icon.on("click",function(){wrapper_is_open?a()?d():q.trigger("submit"):r.focus()});p.on("click",".js-save",function(t){t.preventDefault();a()?d():q.trigger("submit")});p.on("click",".js-cancel",
function(t){t.preventDefault();e()});c(document).on("keydown",function(t){t.stopPropagation();if(!n.$wrapper.find(".c-completed-reminders-section").hasClass("is-shown")){var w=c(t.target),x=t.keyCode,y=27===x;13!==x||wrapper_is_open||(w=w.hasClass("c-text-field"),c(".c-step.is-edit.is-shown").length||w||(t.preventDefault(),r.focus()));y&&(t.preventDefault(),t.stopPropagation(),e(),c(".c-reminder-wrapper").trigger("escKeyPress"))}});(function(t){var w=t.find(".js-reminder-type-toggle"),x=w.find(".js-type-field"),
y=w.find(".menu");w.waDropdown();y.on("click","a",function(){var z=c(this);w.find(".js-text").html(z.html());y.find(".selected").removeClass("selected");z.closest("li").addClass("selected");z=z.data("type-id");x.val(z)})})(p);(function(){p.find(".js-datepicker").each(function(){var t=c(this);t.datepicker({changeMonth:!0,changeYear:!0,showOtherMonths:!0,selectOtherMonths:!0,gotoCurrent:!0});var w=t.parent(),x=w.find(".calendar");n.$date_clear=w.find(".js-reset-date");x.on("click",function(){t.focus()});
t.on("change",function(){this.style.width=0<this.value.length?7*this.value.length+"px":8*(this.getAttribute("placeholder").length+1)+"px";""!==t.val()&&w.addClass("is-active")});n.$date_clear.on("click",function(){t.val("");t.trigger("change");w.removeClass("is-active")})})})();(function(){function t(y){y?w.addClass("is-active"):w.removeClass("is-active")}var w=p.find(".js-time-toggle"),x=w.find(".js-timepicker");n.$time_clear=w.find(".js-reset-time");x.on("change",function(){t(!0);""==x.val()&&t()});
w.on("click",".js-show-time",function(){x.focus()});n.$time_clear.on("click",function(){x.val("");t()})})();(function(){p.find(".js-timepicker").each(function(){c(this).timepicker()})})();q.on("submit",function(t,w){t.preventDefault();is_locked||(is_locked=!0,t=f(),data=t.data,a()||(t.errors.length?(g(t.errors),is_locked=!1):($plus_icon.addClass("loading"),$plus_icon.html('<span class="icon size-20"><i class="fas fa-spinner fa-spin"></i></span>'),q.find(".js-save").attr("disabled",!0),c.post("?module=reminder&action=add",
data,function(x){"ok"===x.status?w||(n.$iframe?null:c.crm.sidebar.reload(),c.crm.content.reload()):console.error(x.errors)},"json").always(function(){is_locked=!1}))))})};CRMReminders.prototype.initLazyLoading=function(){function a(){function e(){var h=c.contains(document,$loader[0]);h?f($loader):($loader=$list.find(".js-lazy-load"),(h=c.contains(document,$loader[0]))?f($loader):k.off("scroll",e))}function f(h){var m=c(window).scrollTop(),n=l.height(),p=h.offset().top;m+n>=p&&(d||g(h))}function g(h){var m=
c.crm.app_url+"?module=reminder&action=actual"+((0<b.setting_deal_id?"&deal="+b.setting_deal_id:"")+(0<b.setting_contact_id?"&contact="+b.setting_contact_id:"")+(b.$iframe?"&iframe=1":""));data={user_id:b.user_id,page:++b.current_page};d=!0;c.post(m,data,function(n){b.click_reminder_event||(h&&h.remove(),$list.append(n),a())}).always(function(){d=!1})}var l=c(window),k=l;$list=b.$wrapper.find("#c-main-reminders-list");$loader=$list.find(".js-lazy-load");if($loader.length)if(0<l.height())if(1==b.current_page&&
l.height()>$list.height())e();else k.on("scroll",e);else setTimeout(function(){return a()},100)}var b=this,d=!1;a()};CRMReminders.prototype.initCompletedReminders=function(){function a(){var t=m.width();l.width(t)}function b(){var t="?module=reminder&action=completed"+((0<g.setting_deal_id?"&deal="+g.setting_deal_id:"")+(0<g.setting_contact_id?"&contact="+g.setting_contact_id:"")),w={user_id:g.user_id};v=!0;n.append(q);p.hide();c.post(t,w,function(x){r=!0;k.html(x);f();n.children().last().remove();
p.show();c(window).trigger("scroll");d(!0)}).always(function(){v=!1})}function d(t,w){u?(setTimeout(function(){h.hide();m.css("overflow","unset");g.$iframe||m.css("max-height","unset")},300),c(document).off("click",e)):(m.css("overflow","hidden"),g.$iframe||m.css("max-height","calc(100vh - 4rem)"),h.show(),c(document).on("click",e));w?(l.toggleClass("is-shown"),u=!u,k.slideToggle(300)):t?(l.addClass("is-shown"),u=!0,k.slideDown(300)):(l.removeClass("is-shown"),u=!1,k.slideUp(300))}function e(t){t.preventDefault();
...
C,function(A){z.remove();k.append(A);f()}).always(function(){y=!1})}c(window);var x=k.find(".js-lazy-load"),y=!1;if(x.length)k.on("scroll",t)}var g=this,l=g.$completedSection,k=l.find(".c-reminders-list"),h=g.$wrapper.find(".drawer"),m=g.$wrapper.find(".article-body"),n=l.find(".c-actions"),p=n.find(".sort-down"),q=c("<span/>").addClass("icon size-16").append("<i/>").addClass("fas fa-spinner wa-animation-spin speed-1000 text-gray"),r=!1,u=!1,v=!1;g.$iframe||(a(),c(window).on("resize",function(){a()}));
c(document).on("is_completed_loaded_false",function(t,w){if(w){w.new_text&&l.find(".js-completed-reminders-btn-text").text(w.new_text);l.hasClass("hidden")&&l.removeClass("hidden");t=w.state;var x=g.$sidebar.find("[data-id='"+w.user_id+"'] .details");w=g.$sidebar.find("[data-id='all'] .details");var y=x.find(".count.all"),z=w.find(".count.all");y.text(y.text()-1);z.text(z.text()-1);if("overdue"==t||"burn"==t)y=x.find(".count.overdue"),x=x.find(".count.burn"),y.length?(x=y.data("due-count"),z=+y.text(),
...
!0;d.renderLoading(!0);var h=f.serializeArray();c.post("?module=reminder&action=save",h,function(m){"ok"===m.status&&(l=!1,g.removeClass("is-changed").blur())}).always(function(){k=!1;d.renderLoading(!1)})}}function b(){g.css("min-height",0);g.css("min-height",g[0].scrollHeight+"px")}var d=this,e=d.$wrapper.find(".js-top-content-toggle-wrapper");0===e.height()&&e.css("padding-bottom","0");if(!e.length)return!1;var f=e.find("form"),g=f.find(".js-textarea"),l=!1,k=!1;g.on("keyup",function(h){g.val().length?
...
-864E5));y=parseInt(B.getFullYear());x=parseInt(B.getMonth())+1;B=parseInt(B.getDate());10>B&&(B="0"+B);10>x&&(x="0"+x);return[y,x,B].join("-")}1===t[0].data.length&&d.each(t,function(x){var y=t[x].data[0],z={value:0,date:w(y.date,!1)},B={value:0,date:w(y.date,!0)};t[x].data=[z,y,B]});return t}(k);for(var n=0;n<k.length;n++){for(var p=k[n].data,q=[],r=0;r<p.length;r++){var u=p[r],v=parseInt(u.value);q.push({date:h(u.date),value:v})}m.push(q)}return d3.layout.stack().offset("zero").values(function(t){return t}).x(function(t){return t.date}).y(function(t){return t.value})(m)}
...
this.yDomain));this.svg.selectAll(".axis .y").transition().duration(600).call(this.yAxis);this.xAxis.scale(this.x);this.svg.selectAll(".axis .x").transition().duration(600).call(this.xAxis);this.renderCharts()};b.prototype.showHint=function(k,h,m,n){var p=this;k=d(h);if(!(0<Math.ceil(k.attr("height"))))return!1;var q=m.date;h=p.$hint.find(".c-date");var r=p.$hint.find(".c-app"),u=p.$hint.find(".c-value");q=function(v,t){var w=parseInt(v.getMonth());"months"==t?(v=p.locales.months,v[w]||(v="\u042f\u043d\u0432\u0430\u0440\u044c \u0424\u0435\u0432\u0440\u0430\u043b\u044c \u041c\u0430\u0440\u0442 \u0410\u043f\u0440\u0435\u043b\u044c \u041c\u0430\u0439 \u0418\u044e\u043d\u044c \u0418\u044e\u043b\u044c \u0410\u0432\u0433\u0443\u0441\u0442 \u0421\u0435\u043d\u0442\u044f\u0431\u0440\u044c \u041e\u043a\u0442\u044f\u0431\u0440\u044c \u041d\u043e\u044f\u0431\u0440\u044c \u0414\u0435\u043a\u0430\u0431\u0440\u044c".split(" ")),
w=v[w]):(t=v.getDate(),w+=1,w=(10>t?"0"+t:t)+"."+(10>w?"0"+w:w)+"."+v.getFullYear());return w}(q,p.group_by);h.text(q);r.text(n.name);u.text(m.value);m=function(v){var t=d(window),w=t.width();t.height();t=p.$hint.outerWidth();var x=p.$hint.outerHeight(),y=Math.ceil(v.attr("width")),z=Math.ceil(v.attr("height")),B=p.$wrapper.offset();v=v.offset();x={left:v.left-B.left+y+10,top:v.top-B.top+(z<x?z-x-2:2)};w<v.left+10+t&&(x.left=v.left-B.left-y-t);return x}(k);p.$hint.css(m).addClass(p.show_class)};b.prototype.moveHint=
...
".js-show-create-dialog",function(d){d.preventDefault();d=b.escapeHtml(""+c(d.target).data("number"));a(d)})};CRMCallPage.prototype.initContactUpdateDialog=function(){function a(d,e){b||(b=!0,c.post(c.crm.app_url+"?module=contact&action=updateDialog",{call_id:d},function(f){c.waDialog({html:f,onClose:function(g,l){g=g.$content.find(".js-field-autocomplete");g.data("success")&&(g=g.html(),e.closest(".c-column-phone").find(".c-user-wrapper .flexbox.middle.space-8").html(g))}})}).always(function(){b=
!1}))}var b=!1;this.$wrapper.on("click",".js-show-update-dialog",function(d){d.preventDefault();d=c(d.target);var e=d.data("id");a(e,d)})};CRMCallPage.prototype.initFilterDrawer=function(){function a(){p.show();c("#wa-app").addClass("content-on-top");setTimeout(function(){p.removeClass("is-hide");B?null:c("body").addClass("is-locked").css("padding-right","17px")},50)}function b(){p.addClass("is-hide");setTimeout(function(){p.hide();c("body").removeClass("is-locked").css("padding-right","");c("#wa-app").removeClass("content-on-top")},
300)}function d(A){A=void 0===A?!1:A;var D=q.serializeArray();D.forEach(function(G,H){if(""!==G.value&&"null"!==G.value){if(4===H&&"custom"===G.value){var I=p.find(".js-datepicker");""===I.eq(0).val()&&(D[5].value="");""===I.eq(1).val()&&(D[6].value="")}h.href_prop=h.href_prop+(0==H?"":"&")+G.name+"="+G.value}});c.crm.content.load(c.crm.app_url+"call/?"+h.href_prop);var E=c("<span/>").addClass("icon size-20").append("<i/>").addClass("fas fa-spinner wa-animation-spin speed-1000 ");if(A)m.prop("disabled",
!0).append(E);else{var F=p.find(".c-filter-buttom");F.append(E);F.children().each(function(){c(this).prop("disabled",!0)});c(document).one("wa_loaded",function(){E.remove();F.children().each(function(){c(this).prop("disabled",!1)});b()})}}function e(A){var D=p.find("#dropdown-directions-filter [data-filter='"+A[0].value+"']"),E=p.find("#dropdown-states-filter [data-filter='"+A[1].value+"']"),F=p.find("#dropdown-responsible-filter [data-filter='"+A[7].value+"']"),G=p.find("#dropdown-date-filter [data-timeframe='"+
A[4].value+"']"),H=q.serializeArray(),I=p.find(".js-datepicker"),K=p.find("#switch-deal-off");D.trigger("click");E.trigger("click");F.trigger("click");""===A[2].value?g():f(h.started_contact,A[2].value,w,r);""===A[3].value||"0"===A[3].value?k():l(h.started_deal,A[3].value);A[3].value===H[3].value||"0"!==H[3].value&&"0"!==A[3].value||K.trigger("click");I.eq(0).datepicker("setDate",new Date(A[5].value));I.eq(1).datepicker("setDate",new Date(A[6].value));G.trigger("click")}function f(A,D,E,F){var G=
A.name;A='<div class="c-user-wrapper">\n                    <div class="flexbox space-8">\n                        <div class="c-column c-column-image">\n                            <div class="flexbox middle">\n                                <img src="'+A.userpic+'" alt="'+G+'">\n                            </div>\n                        </div>\n                    <div class="c-column nowrap">\n                            <div class="c-column--name">'+G+'</div>\n                            <div class="c-jobtitle hint">'+
(F.val()&&A.email?A.email:"")+'</div>\n                            <div class="c-jobtitle hint">'+(F.val()&&A.phone?A.phone:"")+'</div>\n                    </div>\n                    </div>\n                </div>\n                <span class="icon cursor-pointer js-call-filter-contact-cancel"><i class="fas fa-times-circle"></i></span>';p.find("#input-contact").val(D);F.val()?F.val(""):null;E.html(A);F.hide()}function g(){r.show();w.html("");p.find("#input-contact").val("")}function l(A,D){var E=
h.escapeHtml(A.name);A='\n                <div class="c-deal-wrapper nowrap">\n                <div class="flexbox space-8 ">\n                    <i class="fas fa-circle" style="color: '+(A.stage?A.stage.color:"")+'"></i>\n                </div>\n                <div>\n                    <div class="c-deal-wrapper--name">'+E+'</div>\n                    <div>\n                        <span class="hint">'+(A.create_datetime?A.create_datetime.split(" ")[0]:"")+"</span>\n                        "+
(A.closed_datetime?"<span> - </span>":"")+'\n                        <span class="hint">'+(A.closed_datetime?A.closed_datetime.split(" ")[0]:"")+'</span>\n                    </div>\n                    <div class="hint">'+(A.amount?Math.ceil(+A.amount):"")+" "+A.currency_id+'</div>   \n                </div>\n            </div>\n                <span class="icon js-call-filter-deal-cancel"><i class="fas fa-times-circle"></i></span>';p.find("#input-deal").val(D);v.val("");x.html(A);v.hide()}function k(){v.show();
x.html("");p.find("#input-deal").val("")}var h=this,m=h.$wrapper.find(".js-filter-button"),n=h.$wrapper.find(".js-reload-filter-button"),p=h.$wrapper.find(".c-call-filter"),q=p.find(".c-call-filter-form"),r=p.find(".js-search-field-contact"),u=r.parent(),v=p.find(".js-search-field-deal"),t=v.parent(),w=p.find(".js-call-filter-contact"),x=p.find(".js-call-filter-deal"),y=q.serializeArray(),z=q.serialize(),B=h.$wrapper.height()<c(".content.blank.c-shadowed-content").height(),C=[{name:"direction",value:"all"},
{name:"status",value:"ALL"},{name:"contact",value:""},{name:"deal",value:""},{name:"timeframe",value:"null"},{name:"start",value:""},{name:"end",value:""},{name:"user",value:""}];m.on("click",function(A){A.preventDefault();a()});n.on("click",function(A){A.preventDefault();n.find("svg").removeClass("fa-times").addClass("fa-spinner fa-spin");c.crm.content.load(c.crm.app_url+"call/?direction=all&status=ALL")});p.on("click",".js-close-drawer",function(A){A.preventDefault();b()});p.on("click",".js-apply-filter",
function(A){A.preventDefault();d()});p.on("click",".js-reset-filter",function(A){A.preventDefault();"direction=all&status=ALL&timeframe=null&deal=&contact=&start=&end="!==q.serialize()&&e(C)});p.on("click",".js-restore-filter",function(A){A.preventDefault();z!==q.serialize()&&e(y);b()});p.on("click",".js-call-filter-contact-cancel",function(A){A.preventDefault();g()});p.on("click",".js-call-filter-deal-cancel",function(A){A.preventDefault();k()});(function(){var A=h.$wrapper.find(".js-search-field-client"),
D=A.parent();$search_contact_wrapper=h.$wrapper.find(".js-call-filter-search");$search_contact_cancel=h.$wrapper.find(".c-call-filter-contact .js-call-filter-contact-cancel");A.autocomplete({appendTo:D,source:c.crm.app_url+"?module=autocompleteContact&with_email=1&with_phone=1",minLength:0,html:!0,focus:function(){return!1},select:function(E,F){f(F.item,F.item.id,$search_contact_wrapper,D);d(!0);return!1}}).data("ui-autocomplete")._renderItem=function(E,F){return c("<li />").addClass("ui-menu-item-html").append("<div><span class='icon userpic custom-mr-4'><i style='background-image: url("+
F.userpic+");'></i></span>"+F.name+"</div>").appendTo(E)};$search_contact_cancel.on("click",function(E){E.preventDefault();D.show();$search_contact_wrapper.html("")})})();r.autocomplete({appendTo:u,source:c.crm.app_url+"?module=autocompleteContact&with_email=1&with_phone=1",minLength:0,html:!0,focus:function(){return!1},select:function(A,D){f(D.item,D.item.id,w,r);return!1}}).data("ui-autocomplete")._renderItem=function(A,D){return c("<li />").addClass("ui-menu-item-html").append("<div class='flexbox'><span class='icon userpic custom-mr-4'><i style='background-image: url("+
D.userpic+");'></i></span>"+D.name+"</div>").appendTo(A)};v.autocomplete({appendTo:t,source:c.crm.app_url+"?module=autocompleteDeal",minLength:0,html:!0,focus:function(){return!1},select:function(A,D){l(D.item,D.item.id);return!1}}).data("ui-autocomplete")._renderItem=function(A,D){return c("<li />").addClass("ui-menu-item-html").append("<div>"+h.escapeHtml(D.name)+"</div>").appendTo(A)};(function(){p.find(".js-datepicker").each(function(){var A=c(this),D=p.find("."+A.data("selector"));A.datepicker({altField:D,
altFormat:"yy-mm-dd",changeMonth:!0,changeYear:!0})})})()};CRMCallPage.prototype.initDownloadLink=function(){$records_array=this.$wrapper.find(".c-call-wrapper .c-call-record-link");setTimeout(function(){$records_array.eq(0).trigger("click")},2E3)};CRMCallPage.prototype.initCustomAudioPlayer=function(){var a=null;this.$wrapper.on("click",".c-call-record-link",function(b,d){var e=c(this);play=e.find(".play").length||null;loading=e.find(".loading").length||null;pause=e.find(".pause").length||null;$call_wrapper=
e.closest(".c-call-wrapper");$call_id=$call_wrapper.data("id");$column_audio=$call_wrapper.find(".c-column-audio");$progressbar_new=$call_wrapper.find("#c-audio-slider");$progressbar_item=$progressbar_new[0];$duration=$call_wrapper.find(".c-audio-duration");$current_time=$call_wrapper.find(".c-audio-current-time");var f=null,g=function(k){var h=Math.floor(k%60);return Math.floor(k/60)+":"+(10>h?"0"+h:""+h)};if(loading||pause){var l=function(k){clearTimeout(a);clearInterval(id);var h=k[0];$column_audio.slideDown(200);
$duration.text(g(h.duration));$progressbar_item.max=Math.floor(h.duration);$progressbar_new.on("input",function(n){$current_time.text(g($progressbar_item.value));n=n.target;$column_audio[0].style.setProperty("--seek-before-width",n.value/n.max*100+"%");cancelAnimationFrame(f)});$progressbar_new.on("input",function(){h.currentTime=$progressbar_item.value;requestAnimationFrame(m)});var m=function(){$progressbar_item.value=Math.floor(h.currentTime);$current_time.text(g($progressbar_item.value));$column_audio[0].style.setProperty("--seek-before-width",
$progressbar_item.value/$progressbar_item.max*100+"%");f=requestAnimationFrame(m);h.currentTime==h.duration&&(cancelAnimationFrame(f),a=setTimeout(function(){return $column_audio.slideUp(200)},3E3))};k=function(){var n=Math.floor(h.buffered.end(h.buffered.length-1));$column_audio[0].style.setProperty("--buffered-width",n/$progressbar_item.max*100+"%")};h.addEventListener("progress",k);k();requestAnimationFrame(m)};id=setInterval(function(){var k=e.find("audio");k.length&&0<k[0].readyState&&l(k)},
50)}play&&(a=setTimeout(function(){return $column_audio.slideUp(200)},3E3),$progressbar_new.off("change"),$progressbar_new.off("input"),cancelAnimationFrame(f))})};CRMCallPage.prototype.initDotsDetail=function(){var a=this;a.$wrapper.on("click",".js-dots",function(b){$detail=c(this).siblings(".c-dots-detail");$detail_wrapper=$detail.closest(".js-dots-wrapper");$detail.toggle();$detail_wrapper.toggleClass("open");var d=$detail.height();b=$detail.width();var e=$detail_wrapper[0].getBoundingClientRect();
window.innerHeight-e.bottom<d+30?$detail.css("bottom","30px"):$detail.css("bottom","unset");d=$detail_wrapper[0].getBoundingClientRect();window.innerWidth-d.left>b+30?$detail.css("left","0"):$detail.css("right","0")});c(document).on("click",function(b){var d=c(b.target),e=a.$wrapper.find(".js-dots-wrapper.open");d=!!d.closest(".c-dots-detail").length;e.length&&!d&&e.each(function(){c.contains(c(this)[0],b.target)||(c(this).removeClass("open"),c(this).find(".c-dots-detail").hide())})})};CRMCallPage.prototype.initFinishCallLinks=
function(){this.$wrapper.on("click",".js-finish-call",function(a){var b=c(this);a=c.crm.app_url+"?module=call&action=finish";var d=b.closest(".c-call-wrapper"),e=d.data("id");0>=e||b.data("loading")||(b.data("loading",1),b.find(".loading").show(),b.find(".yes-bw").hide(),c.post(a,{id:e},function(f){if("ok"===f.status){f=c("<div>").html(f.data.html);var g=f.find('.c-call-wrapper[data-id="'+e+'"]').find(".c-column-state");d.find(".c-column-state").replaceWith(g);f.remove()}else b.data("loading",0),
b.find(".loading").hide(),b.find(".yes-bw").show()}).error(function(){b.data("loading",0);b.find(".loading").hide();b.find(".yes-bw").show()}))})};CRMCallPage.prototype.initRedirectCall=function(){this.$wrapper.on("click",".js-redirect-call",function(a){a=c(this).parents(".c-call-wrapper").data("id");a=c.crm.app_url+"?module=call&action=redirectDialog&id="+a;var b=c(this);b.prop("disabled",!0).find(".svg-inline--fa").removeClass("fa-exchange-alt").addClass("fa-spinner fa-spin");c.get(a,function(d){c.waDialog({html:d,
onOpen:function(){b.prop("disabled",!1).find(".svg-inline--fa").removeClass("fa-spinner fa-spin").addClass("fa-exchange-alt")}})})})};CRMCallPage.prototype.initDeleteCall=function(){function a(e){var f=e.find(".c-delete-call"),g=f.closest(".c-call-wrapper"),l=g.data("id");e=f.data("title");c.waDialog.confirm({title:'<i class="fas fa-exclamation-triangle smaller state-error"></i> '+e+"?",text:b.locales.delete_confirm_text,success_button_title:b.locales.delete_confirm_button,success_button_class:"danger custom-mr-4",
cancel_button_title:""+b.locales.delete_cancel_button,cancel_button_class:"light-gray",onSuccess:function(){if(!d){d=!0;var k={id:l},h=f.find(".delete");h.removeClass("delete").addClass("loading");c.post("?module=call&action=delete",k,function(m){"ok"===m.status&&g.remove()}).always(function(){h.removeClass("loading").addClass("delete");d=!1})}}})}var b=this,d=!1;b.$wrapper.on("click",".js-delete-call",function(e){e.preventDefault();a(c(this))})};CRMCallPage.prototype.initAssociateDeal=function(){this.$wrapper.on("click",
".js-associate-deal",function(a){a=c(this).data("dialog-url");var b=c(this);b.prop("disabled",!0);c.get(a,function(d){c.waDialog({html:d,onOpen:function(e,f){b.prop("disabled",!1)}})})})};CRMCallPage.prototype.initBackgroundReload=function(){function a(){clearTimeout(f);f=setTimeout(b,1E4)}function b(){var g=!1,l=d.$wrapper.find("audio");c.each(l,function(k,h){c(h)[0].ended||(g=!0)});if(g)return a(),!1;e||(e=!0,c.post("?module=call&action=ts&background_process=1",{call_ts:d.call_ts},function(k){"ok"==
k.status&&c.contains(document,d.$wrapper[0])&&(k.data!==d.call_ts?c.crm.content.reload():a())}).always(function(){e=!1}))}var d=this,e=!1,f=0;a()};CRMCallPage.prototype.initLazyLoading=function(){function a(){function e(){if(c.contains(document,k[0])){var h=k,m=c(window).scrollTop(),n=g.height(),p=h.offset().top;m+n>=p&&(d||f(h))}else g.off("scroll",e)}function f(h){var m=window.location.href.split("/crm/call/")[1];m=c.crm.app_url+"?module=call"+(m?"&"+m.slice(1):"");data={lazy:1,page:++b.current_page};
d=!0;console.log(m);c.post(m,data,function(n){n=c(n).find("#js-calls-table").html();l.append(n);c(document).trigger("wa_loaded");h&&h.remove();a()}).always(function(){d=!1})}var g=c(window),l=b.$wrapper.find("#js-calls-table"),k=b.$wrapper.find(".js-lazy-load");if(k.length)g.on("scroll",e)}var b=this,d=!1;a()};return CRMCallPage}(jQuery);var CRMCallAssociateDealDialog=function(c){CRMCallAssociateDealDialog=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.$footer=this.$wrapper.find(".js-dialog-footer");this.$submit=this.$form.find(".js-submit");this.$deal_name=this.$form.find(".js-deal-name");this.$deal_funnel=this.$form.find(".js-select-deal-funnel");this.$deal_stage=this.$form.find(".js-select-deal-stage");this.$deal_id=this.$form.find(".js-deal-id");this.initClass()};CRMCallAssociateDealDialog.prototype.initClass=
...
var d=this,e=d.$wrapper.find(".deal-form"),f=e.find(".js-deal-name"),g=e.find(".js-deal-name-input"),l=e.find(".js-save-deal"),k=e.find(".js-deals-dropdown"),h=e.find(".js-remove-deal"),m=k.find(".js-empty-deal"),n=e.find(".js-select-deal .js-visible-link .js-text"),p=e.find(".js-select-funnel-wrapper"),q=e.find(".js-select-stage-wrapper"),r=e.find(".js-deals-list"),u=e.find(".js-deal-id"),v=d.contact_id;u.val("none");v&&c.get("?module=deal&action=byContact&id="+v,function(t){"ok"===t.status&&c.each(t.data.deals,
...
g.focus()});e.on("click",".js-deal-item",function(){var t=c(this).find(".js-text").html();n.html(t);u.val(c(this).data("deal-id"));l.attr("title",d.locales.deal_add).removeClass("hidden");p.addClass("hidden");m.removeClass("c-empty-deal-hidden");r.find("li").removeClass("selected");c(this).parent().addClass("selected")});r.on("click",function(){r.hide();setTimeout(function(){r.removeAttr("style")},200)});m.on("click",function(){a()});h.on("click",function(){a()});l.on("click",function(t){t.preventDefault();
...
this.settings=a;this.iframe=a.iframe;this.noemail=a.noemail;this.initClass()};CRMMessagesSidebar.prototype.initClass=function(){var a=this;a.initLazyLoading();a.initSearchHandler();a.conversationListHandler();new CRMPageMessagesOperations(a.settings);a.no_messages&&a.$wrapper.closest("#c-messages-sidebar").addClass("no-messages-sidebar");a.$wrapper.on("click",".js-associate-deal",function(d){d.preventDefault();d=c(this).data("dialog-url");c.get(d,function(e){c.waDialog({html:e})})});var b=!1;a.$wrapper.on("click",
".js-write-message",function(d){d.preventDefault();b||(b=!0,d=c.crm.app_url+"?module=message&action=writeNewDialog",data={contact_id:a.$settings_contact_id?a.$settings_contact_id:null,deal_id:a.$settings_deal_id?a.$settings_deal_id:null},c.get(d,data,function(e){c.waDialog({html:e,onOpen:function(){b=!1}})}))});a.$wrapper.on("click",".js-filter-open",function(){a.$wrapper.find(".c-message-filter-wrapper").toggle()});a.current_page&&a.initBackgroundUpdate()};CRMMessagesSidebar.prototype.initSearchHandler=
function(){var a=this,b=a.$sidebar;a.$search_wrapper=b.find(".js-search-wrapper");var d=a.$search_wrapper;$page_name=b.find(".js-page-name-header");$autocomplete=d.find(".js-search-field");$search_block=d.find(".state-with-inner-icon");a.iframe||($autocomplete.autocomplete({appendTo:d,source:c.crm.app_url+"?module=autocompleteContact",minLength:0,html:!0,focus:function(){return!1},select:function(e,f){c.crm.content.load(c.crm.app_url+"message/?contact="+f.item.id);a.$skeleton.show();$autocomplete.val("");
...
$search_block.removeClass("desktop-and-tablet-only");d.find(".js-search-contact-hide").show()});d.on("click",".js-search-contact-hide",function(e){d.find(".js-search-contact-hide").hide();d.find(".js-search-mobile-show").add("mobile-only").show();$page_name.addClass("mobile-only").show();$search_block.addClass("desktop-and-tablet-only")})};CRMMessagesSidebar.prototype.initBackgroundUpdate=function(){function a(){c(document).off("msg_sidebar_upd_needed");c(document).off("change_active_id")}function b(h){h=
void 0===h?!1:h;clearTimeout(k);h?d():k=setTimeout(d,12E4)}function d(){l||(l=!0,c.contains(document,f.$wrapper[0])&&c.post("?module=message&action=listByConversation",{check:1},function(h){"ok"===h.status&&(h=h.data!==g,c.contains(document,f.$wrapper[0])&&(h&&!c(".dialog-background:visible").length?e():b()))},"json").always(function(){l=!1}))}function e(){var h=c.crm.app_url+"message/"+(f.active_id?"conversation/"+f.active_id+"/":"")+"?reload=1"+(f.$settings_contact_id?"&contact="+f.$settings_contact_id:
f.$settings_deal_id?"&deal="+f.$settings_deal_id:"")+(f.iframe?"&iframe="+f.iframe:"")+(f.noemail?"&noemail=1":""),m={ui:f.$ui,id:+f.active_id};a();f.$wrapper.off("click");f.$search_wrapper.off("click");f.$list.off("click");clearTimeout(k);c.get(h,m,function(n){n=c(n).find("#c-messages-conversation-list").html();f.$wrapper.html(n)})}var f=this,g=f.last_message_id,l=!1,k=0;a();c(document).on("change_active_id",function(h,m){m.target_id&&(f.active_id=m.target_id)});c(document).on("msg_sidebar_upd_needed",
function(){b(!0)});b()};CRMMessagesSidebar.prototype.conversationListHandler=function(){function a(g){g.preventDefault();g.stopPropagation();g=c(this);var l=e.next(),k=g.prop("href"),h=g.parent().data("id");target_props=h+(b.iframe?"&iframe=1&view=chat":"&view=chat")+(b.$settings_contact_id?"&contact="+b.$settings_contact_id:b.$settings_deal_id?"&deal="+b.$settings_deal_id:"");c.message.content.load(k,!1,target_props,h,g);d.find(".selected").removeClass("selected");g.parent().addClass("selected").removeClass("unread");
l.is(":visible")||(e.addClass("desktop-and-tablet-only"),l.removeClass("desktop-and-tablet-only"))}var b=this,d=b.$list,e=b.$sidebar,f=c(window);d.on("click","a",a);f.on("beforeunload",function(){d.off("click","a",a)});c(document).ready(function(){b.scrollToSelected();b.$skeleton.hide()})};CRMMessagesSidebar.prototype.scrollToSelected=function(){var a=this.$sidebar,b=this.$list.find(".js-message-wrapper.selected");if(b.length){var d=b.offset().top;target_h=b.height();viewportHeight=c(window).height();
scrollIt=d-(viewportHeight-target_h)/2;a.scrollTop(scrollIt)}};CRMMessagesSidebar.prototype.initLazyLoading=function(){function a(){function m(){if(c.contains(document,n[0])){var q=n,r=f.scrollTop(),u=f.height(),v=q.offset().top;r+u>=v&&(e||b(q))}else g.off("scroll touchmove",m)}var n=k.find(".js-lazy-load");e=!1;if(h&&h>d.current_page)b(n);else if(h&&h==d.current_page&&d.scrollToSelected(),n.length){var p=1==d.current_page&&f.height()-64>l?!0:!1;if(p)m();else g.on("scroll touchmove",m)}}function b(m){var n=
c.crm.app_url+"?module=messageListByConversation",p=d.$settings_contact_id?+d.$settings_contact_id:void 0,q=d.$settings_deal_id?+d.$settings_deal_id:void 0;data={id:+d.active_id,page:++d.current_page,contact:p,deal:q,reload:1,iframe:d.iframe};e=!0;c.post(n,data,function(r){m&&m.remove();r=c(r).find(".c-messages-table-section").html();k.append(r);a()}).always(function(){})}var d=this,e=!1,f=c(window),g=d.$sidebar,l=g.find(".c-messages-conversation-list").height(),k=d.$list,h=d.page_of_item;a()};return CRMMessagesSidebar}(jQuery);var CRMMessageConversationPage=function(c){CRMMessageConversationPage=function(a){this.$wrapper=a.$wrapper;this.locales=a.locales;this.conversation_id=a.conversation_id;this.contact_id=a.contact_id;this.filter_contact_id=a.filter_contact_id;this.filter_deal_id=a.filter_deal_id;this.funnel_id=a.funnel_id;this.last_message_id=a.last_message_id;this.check_interval=a.check_interval;this.$replySection=this.$wrapper.find(".c-reply-section");this.iframe=a.iframe;this.old_message_id=a.old_message_id;this.$dropdown_name=
...
e=a.$wrapper[0].getElementsByTagName("video"),f=d.length+e.length;b();for(var g=0;g<e.length;g++)3<=e[g].readyState?(f--,b()):(e[g].addEventListener("loadeddata",function(){f--;b()}),e[g].addEventListener("error",function(){f--;b()}));for(g=0;g<d.length;g++)d[g].complete?(f--,b()):(d[g].addEventListener("load",function(){f--;b()}),d[g].addEventListener("error",function(){f--;b()}))});a.initSingleMessage();a.initToggleSidebar();a.initLazyLoading();a.initBlockquotToggle();a.initScrollToBottom();a.initAuxDropdown()};
CRMMessageConversationPage.prototype.setLinkWithId=function(){var a=window.location.href;a.slice(-8).includes("message")&&(a=a+"conversation/"+this.conversation_id+"/",history.replaceState({reload:!0,content_uri:a},"",a))};CRMMessageConversationPage.prototype.initAuxDropdown=function(){function a(d){c.post("?module=message&action=verification",{message_id:b.last_message_id,no_more_confirmation:(void 0===d?0:d)?1:0},function(e){"ok"==e.status?c(document).trigger("msg_conversation_update"):c.waDialog.alert({text:e.errors.message||
"Unknown error",button_title:b.locales.close,button_class:"gray"})})}var b=this;b.$wrapper.find(".js-request-auth").click(function(){b.do_confirm_verification?c.waDialog.confirm({title:b.locales.verify_confirm_title,text:b.locales.verify_confirm_text+'<div class="custom-mt-24"><label><span class="wa-checkbox"><input type="checkbox" value="1" class="js-verification-not-show-confirm-checkbox"><span><span class="icon"><i class="fas fa-check"></i></span></span></span> '+b.locales.verify_no_more_confirmation_label+
"</label></div>",success_button_title:b.locales.verify_confirm_button,cancel_button_title:c.crm.locales.cancel,cancel_button_class:"light-gray",onSuccess:function(){var d=c(".js-verification-not-show-confirm-checkbox").is(":checked");a(d)}}):a()})};CRMMessageConversationPage.prototype.initBlockquotToggle=function(){var a=this.$wrapper.find(".c-email-messages-list").find(".c-message-body");c.each(a,function(b,d){c(d).find(".js-blockquote-toggle").length||(b=c(d).children("blockquote"),c.each(b,function(e,
f){e=c('<span class="button light-gray custom-mb-4 size-14 js-blockquote-toggle"><i class="fas fa-ellipsis-h"></i></span>');e.insertBefore(f);e.on("click",function(g){c(f).slideToggle(400)})}))})};CRMMessageConversationPage.prototype.initLazyLoading=function(){function a(){function m(){if(c.contains(document,n[0])){var p=n;0>=f.scrollTop()&&!e&&(f.off(".lazy"),b(p))}else f.off("scroll.lazy touchmove.lazy",m)}var n=f.find(".js-lazy-load");e=!1;1==d.current_page&&10>k?n.remove():n.length&&(f.on("scroll.lazy touchmove.lazy",
m),d.$wrapper.one("bigWindowEvent.lazy",m))}function b(m){var n=c.crm.app_url+"?module=messageConversationId&id="+d.conversation_id+"&old_message_id="+d.old_message_id;e=!0;c.ajax({url:n,type:"POST",dataType:"html",beforeSend:function(){l.show();h=f.prop("scrollHeight")},complete:function(){},data:{delay:1},success:function(p){var q=c(p).find(".js-messages-list");p=c(p).find(".js-message-wrapper");var r=p.length;if(r){var u=function(){m&&m.remove();f.prepend(g.html());g.html("");f.scrollTop(f.prop("scrollHeight")-
h);h=f.prop("scrollHeight");l.hide();a()};d.old_message_id=p.eq(0).data("id");10>r&&q.find(".js-lazy-load").remove();q=q.html();g.append(q);(function(){var v=g[0].getElementsByTagName("img"),t=v.length;if(0==t)u();else for(var w=0;w<v.length;w++)v[w].complete?t--:(v[w].addEventListener("load",function(){t--;0==t&&u()}),v[w].addEventListener("error",function(){t--;0==t&&u()})),0==t&&u()})()}else m&&m.remove();d.current_page++;a()}})}var d=this,e=!1,f=d.$wrapper.find(".js-messages-list"),g=d.$wrapper.find(".c-conversation-body--blank"),
l=f.find(".js-messages-list--transparent-layer"),k=f.find(".js-message-wrapper").length,h=d.$wrapper.prop("scrollHeight");d.current_page=1;a();c(document).on("new_message_lazy",function(m){a()})};CRMMessageConversationPage.prototype.initVisiblityWatcher=function(){function a(e){e?(d.addClass("is-extended").trigger("resize"),b.$wrapper.addClass("scroll-lock")):(d.removeClass("is-extended").trigger("resize"),b.$wrapper.removeClass("scroll-lock"))}var b=this,d=b.$replySection;d.on("click",".js-revert",
function(){a(!1)});d.on("click",".js-message-resize",function(){a(!1)});d.on("click",".js-message-resize-open",function(){a(!0)})};CRMMessageConversationPage.prototype.loadDealListByContact=function(a){a=a||function(){};var b=this.contact_id;0>=b||this.iframe?a({}):c.get("?module=deal&action=byContact&id="+b,"json").always(function(d){d&&"ok"===d.status?a(d.data||{}):a({})})};CRMMessageConversationPage.prototype.initSelectDeal=function(a){function b(){t.html(e.locales.deal_empty);z.val("none");m.val("");
h.addClass("c-deal-name-hidden");p.addClass("hidden").removeAttr("title");v.addClass("c-empty-deal-hidden");w.addClass("hidden");y.find("li").removeClass("selected");0<q.data("deals_count")?(q.removeClass("hidden"),r.addClass("hidden")):(q.addClass("hidden"),r.removeClass("hidden"))}function d(){var B=f.find(".js-created-deal"),C=x.find(".js-visible-link .js-text .funnel-state").clone(),A=c.trim(m.val()),D=k.serializeObject();D.conversation_id=e.conversation_id;if("none"===z.val())return k.addClass("shake animated"),
setTimeout(function(){k.removeClass("shake animated")},500),!1;if(0>=z.val()&&!A)return h.addClass("shake animated"),setTimeout(function(){h.removeClass("shake animated");m.focus()},500),!1;k.addClass("deal-form-hidden");B.removeClass("hidden");0>=z.val()?(B.html(C),B.append(c.crm.escape(A))):(A=q.find(".js-visible-link .js-text"),C=A.find(".funnel-state").clone(),A=A.find(".deal-item--name").text(),B.html(C),B.append(c.crm.escape(A)));c.post(c.crm.app_url+"?module=message&action=conversationAssociateDealSave",
D,function(E){"ok"===E.status?(f.closest(".dialog").data("dialog").close(),c.crm.content.reload()):(B.html(""),B.addClass("hidden"),b(),k.removeClass("deal-form-hidden"))})}var e=this;a=a||{};var f=a.$wrapper||e.$wrapper,g=function(B){B=B||{};var C=B.funnels||{},A=0;c.each(B.deals||{},function(D,E){D=y;var F=D.prepend;var G=C[E.funnel_id];if(!E||0>=E.id)E="";else{var H=E.id,I=E.name||"",K="",J="";G&&G.stages&&G.stages[E.stage_id]&&(K=G.stages[E.stage_id].color||"");c.isEmptyObject(G)&&(J='<span class="hint">'+
e.locales.funnel_deleted+"</span>");E='<li><a href="javascript:void(0);" class="js-deal-item" data-deal-id="'+H+'"><span class="flexbox js-text"><i class="fas fa-circle funnel-state custom-mr-4" style="color: '+K+'"></i><span class="deal-item--name" style="word-break: break-word;">'+I+"</span></span>"+J+"</a></li>"}F.call(D,E);A++});l.show();q.data("deals_count",A);0<A?(q.removeClass("hidden"),r.addClass("hidden")):(r.removeClass("hidden"),q.addClass("hidden"))},l=f.find(".js-deal-selector-control-wrapper"),
k=l.find(".deal-form"),h=k.find(".js-deal-name"),m=k.find(".js-deal-name-input"),n=f.closest(".js-conversation-deal-attach-dialog").find(".js-save-button"),p=k.find(".js-save-deal"),q=k.find(".js-deals-dropdown"),r=k.find(".js-create-new-deal-link"),u=k.find(".js-remove-deal"),v=q.find(".js-empty-deal"),t=k.find(".js-select-deal .js-visible-link .js-text"),w=k.find(".js-select-funnel-wrapper"),x=k.find(".js-select-stage-wrapper"),y=k.find(".js-deals-list"),z=k.find(".js-deal-id");z.val("none");"undefined"===
typeof a.data?e.loadDealListByContact(g):g(a.data);k.on("click",".js-create-new-deal",function(){z.val("0");q.addClass("hidden");r.addClass("hidden");h.removeClass("c-deal-name-hidden");p.attr("title",e.locales.deal_create).removeClass("hidden");w.removeClass("hidden");v.removeClass("c-empty-deal-hidden");m.focus();n.addClass("yellow")});k.on("click",".js-deal-item",function(){var B=c(this).find(".js-text").html();t.html(B);z.val(c(this).data("deal-id"));p.attr("title",e.locales.deal_add).removeClass("hidden");
w.addClass("hidden");v.removeClass("c-empty-deal-hidden");y.find("li").removeClass("selected");c(this).parent().addClass("selected");n.addClass("yellow")});y.on("click",function(){y.hide();setTimeout(function(){y.removeAttr("style")},200)});v.on("click",function(){b()});u.on("click",function(){b()});p.on("click",function(B){B.preventDefault();k.trigger("submit")});k.on("submit",function(B){B.preventDefault();d()});k.on("change",".js-select-deal-funnel",function(){k.find(".js-select-stage-wrapper").load("?module=deal&action=stagesByFunnel&id="+
c(this).val())});return{submit:function(){k.trigger("submit")}}};CRMMessageConversationPage.prototype.initDealActions=function(){function a(f,g){var l=null;f.on("click",".js-detach-deal",function(k){k.preventDefault();c.crm.confirm.show({title:b.locales.deal_detach_title,text:b.locales.deal_detach_text.replace(/%s/,g),button:b.locales.deal_detach_confirm_button,onConfirm:function(){l&&l.abort();b.loadDealListByContact(function(h){l=c.post(c.crm.app_url+"?module=messageConversationDeal&action=detach",
{id:b.conversation_id}).done(function(m){m&&"ok"===m.status&&(c.crm.content.reload(),f.data("dialog").close())}).always(function(){l=null})})}})})}var b=this,d=b.$wrapper,e=d.find(".js-conversation-deal");$deal_second=d.find("#dropdown-c-deal");e.on("click",".js-associate-deal",function(f){f.preventDefault();f=c(this).data("dialog-url");c.get(f,function(g){c.waDialog({html:g})})});e.on("click",".js-attach-other-deal",function(f){setTimeout(function(){$deal_second.find(".dropdown-toggle").trigger("click")},
50)});$deal_second.on("click",".js-return-back",function(f){setTimeout(function(){b.$dropdown_name.find(".dropdown-toggle.c-name").trigger("click")},50)});$deal_second.on("click",".js-attach-other-deal",function(f){f.preventDefault();var g=e.find(".js-deal-link").text();b.loadDealListByContact(function(l){c.get(c.crm.app_url+"?module=messageConversationDeal&action=attachDialog",{id:b.conversation_id},function(k){var h=c(k);c.waDialog({html:h,onOpen:function(){var m=h.find(".js-conversation-deal"),
n=b.initSelectDeal({$wrapper:m,data:l});h.find(".js-save-button").on("click",function(){n.submit()});a(h,g)}})})})});$deal_second.on("click",".js-detach-deal",function(f){f.preventDefault();var g=null;f=e.find(".js-deal-link--name").text();c.crm.confirm.show({title:b.locales.deal_detach_title,text:b.locales.deal_detach_text.replace(/%s/,f),button:b.locales.deal_detach_confirm_button,onConfirm:function(){g&&g.abort();b.loadDealListByContact(function(l){g=c.post(c.crm.app_url+"?module=messageConversationDeal&action=detach",
{id:b.conversation_id}).done(function(k){k&&"ok"===k.status&&c.crm.content.reload()}).always(function(){g=null})})}})})};CRMMessageConversationPage.prototype.initChangeResponsible=function(){function a(){e||(e=!0,c.post("?module=message&action=changeConversationUser",{action:"remove",id:d.conversation_id},function(h){"ok"===h.status&&(e=!1)},"json").always(function(){e=!1;c.crm.content.reload()}))}function b(h,m){function n(w){var x="?module=autocomplete&type=user";x=0<d.funnel_id?x+("&funnel_id="+
d.funnel_id):x+("&contact_id="+d.contact_id);w.autocomplete({appendTo:w.parent(),source:x,minLength:0,html:!0,focus:function(){return!1},select:function(y,z){p(z.item.id);w.val("");m.close();c.crm.content.reload();return!1}}).data("ui-autocomplete")._renderItem=function(y,z){var B=c("<li />");B.addClass("ui-menu-item-html").append("<div>"+z.value+"</div>").appendTo(y);z.rights||(B.addClass("is-locked"),B.on("click",function(C){C.preventDefault();C.stopPropagation()}));return B};w.on("focus",function(){w.data("uiAutocomplete").search(w.val())})}
function p(w){e||(e=!0,c.post("?module=message&action=changeConversationUser",{action:"set",id:d.conversation_id,user_contact_id:w},function(x){"ok"===x.status&&(x=c(x.data.html),r.html(x),v.addClass("hidden"),x=r.find(".js-owner-autocomplete"),x.length&&n(x))},"json").always(function(){e=!1}))}function q(){e||(e=!0,c.post("?module=message&action=changeConversationUser",{action:"remove",id:d.conversation_id},function(w){"ok"===w.status&&(r.html(""),v.removeClass("hidden"),e=!1)},"json").always(function(){e=
!1;m.close();c.crm.content.reload()}))}h.on("click",".js-cancel",function(w){w.preventDefault();m.close()});var r=h.find(".js-responsible-wrapper"),u=r.find(".js-owner-autocomplete"),v=h.find(".js-responsible-empty-wrapper"),t=v.find(".js-responsible-empty-autocomplete");t.length&&n(t);u.length&&n(u);v.on("click",".js-set-extended, .js-unset-extended",function(){v.toggleClass("is-extended");v.hasClass("is-extended")&&t.focus()});r.on("click",".js-show-combobox",function(){r.find(".c-conversation-member").addClass("is-edit");
r.find(".js-owner-autocomplete").focus()});r.on("click",".c-conversation-member .profile",function(){m.close()});r.on("click",".js-hide-combobox",function(){r.find(".c-conversation-member").removeClass("is-edit")});r.on("click",".js-remove-owner",function(w){w.preventDefault();w=c.trim(r.find(".c-name").html());c.crm.confirm.show({title:d.locales.remove_owner_title,text:d.locales.remove_owner_text.replace(/%s/,w),button:d.locales.remove_owner_button,onConfirm:function(){q()}})})}var d=this,e=!1,f=
d.$wrapper.find(".js-conversation-contact"),g=d.$wrapper.find(".js-open-responsible-dialog"),l=d.$wrapper.find("#dropdown-c-contact");$responsible_wrapper_dialog=d.$wrapper.find(".js-conversation-responsible-wrapper");html_content=$responsible_wrapper_dialog.html();f.on("click",".js-open-second-menu",function(h){setTimeout(function(){l.find(".dropdown-toggle").trigger("click")},50)});l.on("click",".js-return-back",function(h){setTimeout(function(){d.$dropdown_name.find(".dropdown-toggle.c-name").trigger("click")},
50)});l.on("click",".js-remove-owner",function(h){h.preventDefault();h=c.trim($responsible_wrapper_dialog.find(".c-name").html());c.crm.confirm.show({title:d.locales.remove_owner_title,text:d.locales.remove_owner_text.replace(/%s/,h),button:d.locales.remove_owner_button,onConfirm:function(){a()}})});var k='<div class="dialog dialog-conversation-responsible" id="my-dialog">\n        <div class="dialog-background"></div>\n        <div class="dialog-body">\n        <div class="dialog-header"><h3><i class="fas fa-cogs text-blue smaller"></i> '+
d.locales.owner+'</h3></div>\n        <div class="dialog-content">\n        '+html_content+'\n        </div>\n        <div class="dialog-footer">\n            <button class="button light-gray js-cancel" type="button">'+d.locales.close+"</button>\n        </div>\n        </div>";g.on("click",function(){c.waDialog({html:k,onOpen:b})})};CRMMessageConversationPage.prototype.triggerChatUpdate=function(){c(document).trigger("msg_conversation_update")};CRMMessageConversationPage.prototype.initBackgroundActions=
function(){function a(l){l=void 0===l?!1:l;clearTimeout(g);l?b():g=setTimeout(b,e.check_interval)}function b(){f||(f=!0,c.post("?module=message&action=conversationIdCheck",{id:e.conversation_id},function(l){"ok"===l.status&&(+l.data!==e.last_message_id?d().then(function(k){k.length&&e.scroll2bottom(!0,!0);a()}):a())},"json").always(function(){f=!1}))}function d(){var l=c.Deferred();c.get(location.href,function(k){function h(q){function r(){q.removeClass("is-new");q.off("hover",r);u.off("unmark-new-messages",
r)}var u=c(document);q.addClass("is-new");q.on("hover",r);u.on("unmark-new-messages",r);setTimeout(function(){c.contains(document,q[0])&&r()},1E4)}var m=e.last_message_id,n=e.$wrapper.find(".js-messages-list").first();k=c(k).find(".js-messages-list .js-message-wrapper");var p=[];n.length&&(k.each(function(){var q=c(this),r=!1;message_id=q.data("id");q.hasClass("verification_message")&&(r=q.data("verification"));message_id>e.last_message_id&&(n.append(q),h(q),r&&c(document).trigger("updateProfile",
{contact_id:r}),p.push({id:message_id,$message:q}),m=message_id)}),e.last_message_id=m);l.resolve(p)});return l.promise()}var e=this,f=!1,g=0;c(document).on("wa_before_load",function(){clearTimeout(g);c(document).off("wa_before_load")});c(document).on("msg_conversation_update",function(){d().then(function(l){l.length&&e.scroll2bottom(!0,!0);a()})});a()};CRMMessageConversationPage.prototype.initDelete=function(){function a(){var e=b.filter_contact_id?"contact="+b.filter_contact_id:b.filter_deal_id?
"deal="+b.filter_deal_id:null,f=b.iframe?"?iframe=1":"";e=e?(b.iframe?"&":"?")+e:"";var g=b.$wrapper.closest("#c-messages-page").find("#c-messages-sidebar"),l=c.crm.app_url+"message/"+f+e;g.is(":visible")&&(f=g.find(".c-message-wrapper[data-id='"+b.conversation_id+"']").next(".c-message-wrapper"),l=f.length?f.find("a.item").prop("href")+e:l);c.post(c.crm.app_url+"?module=message&action=conversationIdDelete",{id:b.conversation_id},function(k){"ok"===k.status&&c.crm.content.load(l)}).always(function(){d=
!1},"json")}var b=this,d=!1;b.$wrapper.on("click",".js-delete-conversation",function(e){e.preventDefault();d||c.crm.confirm.show({title:b.locales.delete_conversation_title,button:b.locales.delete_conversation_button,onConfirm:a})})};CRMMessageConversationPage.prototype.initSendEmail=function(){var a=!1;this.$wrapper.on("click",".js-show-message-dialog",function(b){b.preventDefault();var d=c(this);b=d.data("id");d=d.data("email");!a&&b&&(a=!0,c.post("?module=message&action=writeNewDialog",{contact_id:b,
email:d},function(e){c.waDialog({html:e})}).always(function(){a=!1}))})};CRMMessageConversationPage.prototype.initSingleMessage=function(){var a=!1;this.$wrapper.on("click",".js-open-detail-message",function(b){b.preventDefault();a||(a=!0,b=c(this).data("dialog-url"),c.get(b,function(d){c.waDialog({html:d,onClose:function(){a=!1}})}))})};CRMMessageConversationPage.prototype.initScrollToBottom=function(){function a(){if(c.contains(document,e[0])){var f=d.get(0).scrollHeight-d.scrollTop()-d.height();
50>=f&&e.is(":visible")?e.hide():50<f&&e.is(":hidden")&&e.show()}else d.off("scroll touchmove",a)}var b=this,d=b.$wrapper.find(".js-messages-list"),e=b.$wrapper.find(".js-messages-list--to-bottom-button");d.on("scroll touchmove",a);e.on("click",function(){return b.scroll2bottom(!0,!0)});c(document).on("scroll_button_event",function(f,g){b.old_message_id=g.old_message_id;b.last_message_id=g.last_message_id;b.scroll2bottom(!0,!0)})};CRMMessageConversationPage.prototype.scroll2bottom=function(a,b){function d(){setTimeout(function(){c.contains(document,
f.$wrapper[0])&&l&&e()},0)}function e(){var k=f.$wrapper.find(".js-messages-list"),h=k.height(),m=k.get(0).scrollHeight;console.log(h,m);0<m?(h<m?b?k.animate({scrollTop:m},500):k.scrollTop(m):f.$wrapper.trigger("bigWindowEvent.lazy"),c.message.content.animate(!1)):setTimeout(function(){return e()},100)}var f=this,g=f.iframe?c(top.window):c(window);g.height();var l=!0;if(a)e();else if(c.crm.is_page_loaded||f.iframe)d();else g.one("scroll",function(){l=!1}).one("load",d)};CRMMessageConversationPage.prototype.initToggleSidebar=
function(){var a=this.$wrapper.parent().parent(),b=this.$wrapper.find(".js-expand-sidebar");if(b.length)b.on("click",function(d){d.preventDefault();d=a.prev();if(a.is(":visible")){a.addClass("desktop-and-tablet-only");d.removeClass("desktop-and-tablet-only");d=window.history.state;var e=d.content_uri.replace("?view=chat","").replace("&view=chat","");d.content_uri=e;window.history.replaceState(d,d.title,e)}})};return CRMMessageConversationPage}(jQuery),CRMMessagesProfileAdditional=function(c){CRMMessagesProfileAdditional=
function(a){this.$main_wrapper=a.$wrapper;this.contact_id=a.contact_id;this.$wrapper=this.$main_wrapper.find("#c-messages-info-additional");this.initClass()};CRMMessagesProfileAdditional.prototype.initClass=function(){function a(g){d.$main_wrapper.prepend('<iframe frameborder="0" src="'+c.crm.app_url+"frame/contact/"+g+'" style="width:100%; background-color: var(--background-color);"></iframe>');d.$main_wrapper.find("iframe").on("load",b)}function b(){var g=c(this),l=g.contents().find("#app");l.length?
(new ResizeObserver(function(k){k=k[0].target.scrollHeight;g.css("height",k);300<k&&d.$wrapper.removeClass("hidden")})).observe(l[0]):setTimeout(function(){return b()},100);g.on("beforeShowModal",function(){g.addClass("height-full");d.$main_wrapper.addClass("freeze-sidebar")});g.on("beforeCloseModal",function(){g.removeClass("height-full");d.$main_wrapper.removeClass("freeze-sidebar")})}var d=this,e=!1,f=c(window).width();if(1024<=f)a(d.contact_id),e=!0;else c(window).on("resize",function(){e||c(this).width()===
f||(f=c(this).width(),1024<=f&&(a(d.contact_id),e=!0))});c(document).on("updateProfile",function(g,l){d.$main_wrapper.find("iframe").remove();a(l.contact_id)});d.initTopInfoFields();d.initAdditionalContentLink()};CRMMessagesProfileAdditional.prototype.initTopInfoFields=function(){function a(){var g=c.crm.app_url+"?module=contact&action=tabs&id="+b.contact_id;f=!0;b.item_html={};c.get(g).done(function(l){c.each(l.data.tabs,function(k,h){e.append('<li class="custom-mb-16 bold">\n                    <a href="javascript:void(0);" class="js-drawer-list-link js-drawer-list--'+
h.app_id+' additional" data-dialog-url="'+(h.url||"")+'" data-class-name="custom" data-id="'+h.app_id+'">\n                    <span class="js-drawer-list-link--header" >'+h.title+"</span>"+(h.count?'<span class="hint custom-ml-4">'+h.count+"</span>":"")+"</a></li>");b.item_html[h.app_id]=h.html});c.each(l.data.counters,function(k,h){h.value&&e.find(".js-drawer-list-counter--"+h.name).text(h.value)});e.find(".skeleton").hide();b.$main_wrapper.animate({scrollTop:b.$main_wrapper[0].scrollHeight},450)})}
var b=this,d=b.$wrapper.find(".js-info-list"),e=b.$wrapper.find(".js-drawer-list"),f=!1;b.$wrapper.on("click",".js-info-toggle",function(g){g.preventDefault();g=c(this);f||a();g.toggleClass("is-active");d.slideToggle();g.hasClass("is-active")&&b.$main_wrapper.animate({scrollTop:b.$main_wrapper[0].scrollHeight},450)})};CRMMessagesProfileAdditional.prototype.initAdditionalContentLink=function(){function a(e,f,g){function l(){m.append("\n                    <script>\n                    (() => {\n                        setTimeout( function() {\n                            Array.from(document.getElementsByTagName('a')).forEach(l => {\n                                l.setAttribute('target', '_top');\n                            });\n                        }, 200);\n                    })();\n                    \x3c/script>")}
...
h=k.find("a").length;l.length||l.push({id:!1,name:b.locales.no_segments});c.each(l,function(m,n){m="javascript:void(0);";var p="",q='<span class="hint">'+n.name+"</span>";n.id&&(m=c.crm.app_url+"contact/segment/"+n.id+"/",p=b.icons_array[n.icon]?b.icons_array[n.icon]:b.icons_array.noname,q="<span>"+n.name+"</span>");n='<a href="'+m+'">'+p+q+"</a>";h&&(n=", "+n);k.append(n);h=!0})}var b=this,d=b.$wrapper.find(".crm-contact-assign-segments"),e=c.crm.app_url+"?module=contactOperation&action=addToSegments&is_assign=1",
...
z.show()))},!1);return E},url:m,data:A,cache:!1,contentType:!1,processData:!1,type:"POST",success:function(E){setTimeout(function(){c.contains(document,x[0])&&(--n,0>=n&&v())},2E3)}}).always(function(){d.is_locked=!1})}var x=t.$file,y=x.find(".js-bar"),z=y.find(".js-progress-success-icon"),B=x.find(".js-status");x.addClass("is-upload");if(d.max_upload_size>t.file.size)w();else{var C=d.$form.find(".js-submit-button");B.addClass("errormsg");x.removeClass("is-upload");d.is_locked=!1;f.find("svg").removeClass("fa-spinner fa-spin").addClass("fa-arrow-up");
C.attr("disabled",!1);d.$textarea_small.attr("disabled",!1);l.prop("disabled",!1);d.$form.find("button").prop("disabled",!1)}}var v=r?r:function(){};d.files_storage.length?(n=d.files_storage.length,c.each(d.files_storage,function(t,w){u(w)})):v()}}};CRMEmailConversationEmailSender.prototype.initWYSIWYG=function(){var a=this,b=a.$textarea;c.crm.initWYSIWYG(b,{maxHeight:50,allowedAttr:[["section","data-role"]],callbacks:{change:function(){a.disableResizer(!0)}}});a.body&&b.redactor("code.set",a.body);
a.$body_redactor_p=a.$wrapper.find(".redactor-layer.redactor-styles.redactor-layer-img-edit section p").eq(0)};CRMEmailConversationEmailSender.prototype.initSave=function(){function a(){l.css("min-height","auto");var p=l[0].scrollHeight;250>p?l.css("min-height",p+"px"):l.css("min-height","250px")}function b(){var p=g.send_action_url,q=g.$form.serializeArray();m.prop("disabled",!0);l.attr("disabled",!0);c.ajax({method:"POST",url:p,data:q,dataType:"json"}).done(function(r){var u=jQuery.parseJSON(r.data.html);
d(r.data.id?r.data.id:r.data.message_id);f();k.attr("disabled",!0);h.attr("disabled",!0);g.body=g.body_old;g.$replySection.find(".js-revert").trigger("click");a();c("#c-messages-page #c-messages-sidebar").length&&c(document).trigger("msg_sidebar_upd_needed");var v=g.$wrapper.find(".redactor-layer.redactor-styles.redactor-layer-img-edit");r=v.find("blockquote").eq(0);v=v.find("> p").eq(1);var t=c(u).find(".c-message-body").html(),w=c(u).find(".c-contact-link--name").html();u=c(u).find(".c-date.hint").data("time");
r.html(t);v.html(u+", "+w+" "+g.locales.wrote+":")}).fail(function(r){var u=g.$form.find("#tooltip-error-message");r=Object.values(response.errors);u.addClass("errormsg");c.each(r,function(v,t){u.append('<span><i class="fas fa-exclamation-triangle state-error"></i> <span class="small">'+t+"</span></span>")});f();k.attr("disabled",!0);h.attr("disabled",!0);g.body=g.body_old;g.$replySection.find(".js-revert").trigger("click");a();g.is_locked=!1;response.errors&&console.error(response.errors);g.$textarea.on("focus",
e);l.on("focus",e);g.$form.on("change",e)}).always(function(){g.is_locked=!1})}function d(p){var q=n.find("#js-message-conversation-page"),r=q.data("conv-id"),u=q.find(".c-conversation-body--blank");q.find(".js-messages-list--transparent-layer");c.ajax({url:c.crm.app_url+"?module=messageConversationId&id="+r,type:"POST",dataType:"html",beforeSend:function(){},complete:function(){},data:{delay:1},success:function(v){var t=c(v).find(".js-messages-list");v=c(v).find(".js-message-wrapper");var w=v.eq(0).data("id");
if(v=v.length)10>v&&t.find(".js-lazy-load").remove(),t=t.html(),u.append(t),function(){function x(){0==B&&(g.$message_list.html(u.html()),u.html(""),c(document).trigger("scroll_button_event",{old_message_id:w,last_message_id:p}),c(document).trigger("new_message_lazy"))}var y=u[0].getElementsByTagName("img"),z=u[0].getElementsByTagName("video"),B=y.length+z.length;x();for(var C=0;C<z.length;C++)3<=z[C].readyState?(B--,x()):(z[C].addEventListener("loadeddata",function(){B--;x()}),z[C].addEventListener("error",
function(){B--;x()}));for(C=0;C<y.length;C++)y[C].complete?(B--,x()):(y[C].addEventListener("load",function(){B--;x()}),y[C].addEventListener("error",function(){B--;x()}))}()}})}function e(){g.$form.find("#tooltip-error-message").removeClass("errormsg").html("");l.off("focus",e);g.$textarea.off("focus",e);g.$form.off("change",e)}function f(){k.find("svg").removeClass("fa-spinner fa-spin").addClass("fa-arrow-up");h.attr("disabled",!1);l.attr("disabled",!1);m.prop("disabled",!1);g.$form.find("button").prop("disabled",
!1);l.val("");760<c(window).width()&&l.focus();if(g.files_storage.length){var p=g.$form.find(".c-upload-item");c.each(p,function(q,r){r.remove()});g.files_storage=[]}}var g=this,l=g.$textarea_small,k=g.$wrapper.find(".c-visible .js-save-button"),h=g.$form.find(".js-submit-button"),m=g.$form.find(".js-drop-field"),n=c(".c-messages-page-content-wrapper");g.$message_list=n.find(".js-messages-list");c(document).on("wa_before_load",function(){c(document).off("wa_before_load")});l.on("keydown",function(p){13!==
p.keyCode&&10!==p.keyCode||p.ctrlKey||p.metaKey||p.shiftKey||(p.preventDefault(),""!==l.val()||g.files_storage.length?g.$form.submit():(l.addClass("shake animated"),setTimeout(function(){l.removeClass("shake animated")},500)))});l.on("focus",function(p){g.$replySection.addClass("focused")});l.on("blur",function(p){g.$replySection.removeClass("focused")});l.on("keyup",function(p){if(13!==p.keyCode&&10!==p.keyCode||!(p.ctrlKey||p.metaKey||p.shiftKey))""!==l.val()||g.files_storage.length?k.prop("disabled")&&
k.prop("disabled",!1):k.prop("disabled")||k.prop("disabled",!0);else if(!p.shiftKey){var q=l.val(),r=l.prop("selectionStart");p=q.slice(0,r);q=q.slice(r);l.val(p+"\n"+q)}a();g.$body_redactor_p.html(l.val().replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/\n/g,"<br>"))});g.$form.on("submit",function(p){p.preventDefault();g.is_locked||(g.is_locked=!0,p=c('<span class="icon size-16"><i class="fas fa-spinner wa-animation-spin custom-mr-4"></i></span>'),
k.find("svg").removeClass("fa-arrow-up").addClass("fa-spinner fa-spin"),h.attr("disabled",!0),p.insertBefore(h),g.$form.find("button").prop("disabled",!0),g.filesController.uploadFiles([{name:"hash",value:g.hash}],b))})};CRMEmailConversationEmailSender.prototype.initPersonalSettingsDialog=function(){function a(){d||(d=!0,c.post(c.crm.app_url+"?module=email&action=personalSettingsDialog",{},function(e){c.waDialog({html:e,options:{onSave:function(f){b.$textarea.redactor("core.editor").find('[data-role="c-email-signature"]').html(f.email_signature||
"");b.$wrapper.find(".js-sender-name").text(f.sender_name||"")}}})}).always(function(){d=!1}))}var b=this,d=!1;b.$wrapper.on("click",".js-show-personal-settings-dialog",function(e){e.preventDefault();a()})};CRMEmailConversationEmailSender.prototype.initEmailCopy=function(){function a(){var n=c.trim(k.val()).split(/[,:;]/);if(n[0].length)return c.each(n,function(p,q){p=c.trim(q).split(/\s+/);var r=!1,u=q=null;c.each(p,function(v,t){c.crm.check.email(t)&&!r&&(q=t.replace(/<|>/g,""),r=v)});!1!==r&&p.splice(r,
1);u=p.join(" ");q&&u&&d(0,q,u);q&&!u&&b(0,q,q);u&&!q&&(k.addClass("shake animated"),setTimeout(function(){k.removeClass("shake animated")},500));r=!1;u=q=null}),!1}function b(n,p,q){l.children('[data-email="'+p+'"]').length&&"0"!==n||!p.length||(e.$wrapper.find(".email-copy-input-div").before('<div class="email-copy-user" title="'+c.crm.escape(p)+'" data-contact-id="'+c.crm.escape(n)+'" data-email="'+c.crm.escape(p)+'">'+q+' <a title="'+e.locales.remove_form_cc+'" class="remove-cc js-remove-cc">x</a> <input name="cc['+
c.crm.escape(p)+'][email]" type="hidden" value="'+c.crm.escape(p)+'" /><input name="cc['+c.crm.escape(p)+'][id]" type="hidden" value="'+c.crm.escape(n)+'" /></div>'),e.$wrapper.find(".email-copy-text-collapsed").append('<div class="email-copy-user" title="'+c.crm.escape(p)+'" data-email="'+c.crm.escape(p)+'">'+q+"</div>"));k.val("").focus()}function d(n,p,q){l.children('[data-email="'+p+'"]').length&&"0"!==n||!p.length||(e.$wrapper.find(".email-copy-input-div").before('<div class="email-copy-user" title="'+
c.crm.escape(p)+'" data-contact-id="'+n+'" data-email="'+c.crm.escape(p)+'">'+c.crm.escape(q)+' <a title="'+e.locales.remove_form_cc+'" class="remove-cc js-remove-cc">x</a> <input name="cc['+c.crm.escape(p)+'][email]" type="hidden" value="'+c.crm.escape(p)+'" /><input name="cc['+c.crm.escape(p)+'][id]" type="hidden" value="'+c.crm.escape(n)+'" /><input name="cc['+c.crm.escape(p)+'][name]" type="hidden" value="'+c.crm.escape(q)+'" /></div>'),e.$wrapper.find(".email-copy-text-collapsed").append('<div class="email-copy-user" title="'+
c.crm.escape(p)+'" data-email="'+c.crm.escape(p)+'">'+c.crm.escape(q)+"</div>"));k.val("").focus()}var e=this,f=e.$wrapper.find(".email-copy-wrapper"),g=f.find(".email-copy-area"),l=g.find(".email-copy-text"),k=g.find(".email-copy-input"),h=f.find(".deal-participants-area"),m=e.$wrapper.find(".email-copy-wrapper-collapsed");k.autocomplete({source:c.crm.app_url+"?module=autocomplete&emailcomplete=true",appendTo:e.$wrapper,minLength:0,focus:function(){return!1},select:function(n,p){b(p.item.id,p.item.email,
'<i class="icon userpic" style="background-image: url('+p.item.photo_url+');"></i><b>'+p.item.name+"</b>");k.val("");return!1}}).data("ui-autocomplete")._renderItem=function(n,p){return c('<li class="ui-menu-item-html"><div>'+p.value+"</div></li>").appendTo(n)};k.on("focus",function(){k.data("uiAutocomplete").search(k.val())});e.$wrapper.on("click",".js-email-copy-link, .email-copy-text-collapsed",function(n){n.preventDefault();n=e.$wrapper.find(".js-email-copy-link-icon");f.toggleClass("email-copy-wrapper-block");
f.is(":visible")?(n.removeClass("fa-caret-right").addClass("fa-caret-down"),k.focus(),m.removeClass("email-copy-wrapper-collapsed-block")):(n.removeClass("fa-caret-down").addClass("fa-caret-right"),e.$wrapper.find(".email-copy-text-collapsed").children().length&&m.addClass("email-copy-wrapper-collapsed-block"));e.$wrapper.closest(".js-reply-wrapper").trigger("resize")});g.on("click",function(){k.focus()});h.on("click",".email-copy-user",function(){var n=c(this).data("cc-contact-id"),p=c(this).data("cc-email"),
q=c(this).html();b(n,p,q)});k.on("focusout",function(n){a()});k.on("keydown",function(n){13==n.keyCode&&(n.preventDefault(),a())});l.on("click",".js-remove-cc",function(n){n.preventDefault();n=c(this).parent(".email-copy-user");var p=n.data("email");n.remove();e.$wrapper.find(".email-copy-text-collapsed").children('[data-email="'+p+'"]').remove()});k.on("keydown",function(n){if(8==n.keyCode&&0==k.val().length){n=e.$wrapper.find(".email-copy-text .email-copy-user:last");var p=n.data("email");n.remove();
e.$wrapper.find(".email-copy-text-collapsed").children('[data-email="'+p+'"]').remove();k.focus()}})};return CRMEmailConversationEmailSender}(jQuery),CRMImConversationSection=function(c){CRMImConversationSection=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.$textarea=this.$form.find(".js-textarea");this.$replySection=this.$wrapper.closest(".c-reply-section");this.$submitButton=this.$form.find(".js-save-button");this.$attachment_icon=this.$form.find(".js-message-attachment");
this.$skeleton_wrapper=this.$wrapper.closest(".c-messages-conversation-wrapper").find(".skeleton-wrapper");this.send_action_url=a.send_action_url;this.file_template_html=a.file_template_html;this.hash=a.hash;this.max_upload_size=a.max_upload_size;this.is_images_enabled=a.is_images_enabled;this.is_files_enabled=a.is_files_enabled;this.hover_class="is-hover";this.files_controller=this.getFilesController();this.is_locked=!1;this.initClass()};CRMImConversationSection.prototype.initClass=function(){this.initSubmit()};
CRMImConversationSection.prototype.getFilesController=function(){function a(n){n.length&&(c.each(n,function(p,q){p=d.files_storage;var r=p.push,u=c(l);u.find(".js-name").text(q.name);f.prepend(u);d.$submitButton.prop("disabled")&&d.$submitButton.prop("disabled",!1);r.call(p,{$file:u,file:q})}),g.val(""))}function b(n){var p=[];c.each(d.files_storage,function(q,r){n[0]!==r.$file[0]?p.push(r):n.remove()});d.files_storage=p}var d=this,e=d.$form.find(".js-default-drop-area"),f=d.$form.find(".js-upload-list"),
g=d.$form.find(".js-drop-field"),l=d.file_template_html,k=c.crm.app_url+"?module=file&action=uploadTmp";d.files_storage=[];var h=0,m=0;d.$attachment_icon.on("click",function(){g.click()});g.on("change",function(n){n.preventDefault();a(this.files)});e.on("drop",function(n){n.preventDefault();a(n.originalEvent.dataTransfer.files)});e.on("dragover",function(n){n.preventDefault();e.addClass(d.hover_class);clearTimeout(m);m=setTimeout(function(){e.removeClass(d.hover_class)},100)});document.onpaste=function(n){a((n.clipboardData||
n.originalEvent.clipboardData).files)};d.$wrapper.on("click",".js-file-delete",function(n){n.preventDefault();b(c(this).closest(".c-upload-item"));d.files_storage.length||""!==d.$textarea.val()||d.$submitButton.prop("disabled")||d.$submitButton.prop("disabled",!0)});return{uploadFiles:function(n,p,q){function r(v){function t(){var C=new FormData,A=document.cookie.match(/(?:^|; )_csrf=([^;]*)/);(A=A?decodeURIComponent(A[1]):"")&&C.append("_csrf",A);n&&n.length&&c.each(n,function(D,E){E.name&&E.value&&
C.append(E.name,E.value)});C.append("file_size",v.file.size);C.append("files",v.file);C.append("file_end",1);c.ajax({xhr:function(){var D=new window.XMLHttpRequest;D.upload.addEventListener("progress",function(E){E.lengthComputable&&(E=parseInt(E.loaded/E.total*100),B.css("--progress-value",E),100===E&&(B.css("background","transparent"),z.show()))},!1);return D},url:k,data:C,cache:!1,contentType:!1,processData:!1,type:"POST",success:function(D){setTimeout(function(){c.contains(document,x[0])&&(--h,
0>=h&&u())},250)}}).always(function(){is_locked=!1})}var w="image"===v.file.type.split("/")[0]?"image":"file";w=p?w===p:!0;var x=v.$file;x.find(".js-file-delete");var y=x.find(".js-status"),z=x.find(".js-progress-success-icon"),B=x.find(".js-bar");x.addClass("is-upload");d.max_upload_size>v.file.size?w?t():(--h,0>=h&&u()):(y.addClass("errormsg"),x.removeClass("is-upload"),d.is_locked=!1,d.$submitButton.prop("disabled",!1),d.$submitButton.find("svg").removeClass("fa-spinner fa-spin").addClass("fa-arrow-up"),
d.$textarea.attr("disabled",!1),g.prop("disabled",!1),d.$form.find("button").prop("disabled",!1))}var u=q?q:function(){};d.files_storage.length?(h=d.files_storage.length,c.each(d.files_storage,function(v,t){r(t)})):u()}}};CRMImConversationSection.prototype.initSubmit=function(){function a(){d.css("min-height","auto");var k=d[0].scrollHeight;250>k?d.css("min-height",k+"px"):d.css("min-height","250px")}var b=this,d=b.$textarea,e=b.$submitButton;b.$form.find(".js-upload-list");var f=b.is_images_enabled?
"files-":"",g=b.$form.find(".js-drop-field"),l=b.is_images_enabled?"file":!1;$main_wrapper=c(".c-messages-page-content-wrapper");b.$message_list=$main_wrapper.find(".js-messages-list");b.is_locked=!1;c(document).on("wa_before_load",function(){c(document).off("wa_before_load")});d.on("keydown",function(k){13!==k.keyCode&&10!==k.keyCode||k.ctrlKey||k.metaKey||k.shiftKey||(k.preventDefault(),""!==d.val()||b.files_storage.length?b.$form.submit():(d.addClass("shake animated"),setTimeout(function(){d.removeClass("shake animated")},
500)))});d.on("focus",function(k){b.$replySection.addClass("focused")});d.on("blur",function(k){b.$replySection.removeClass("focused")});d.on("keyup",function(k){if(13!==k.keyCode&&10!==k.keyCode||!(k.ctrlKey||k.metaKey||k.shiftKey))""!==d.val()||b.files_storage.length?e.prop("disabled")&&e.prop("disabled",!1):e.prop("disabled")||e.prop("disabled",!0);else if(!k.shiftKey){var h=d.val(),m=d.prop("selectionStart");k=h.slice(0,m);h=h.slice(m);d.val(k+"\n"+h)}a()});b.$form.on("submit",function(k){function h(r){var u=
$main_wrapper.find("#js-message-conversation-page"),v=u.data("conv-id"),t=u.find(".c-conversation-body--blank");u.find(".js-messages-list--transparent-layer");c.ajax({url:c.crm.app_url+"?module=messageConversationId&id="+v,type:"POST",dataType:"html",beforeSend:function(){},complete:function(){},data:{delay:1},success:function(w){var x=c(w).find(".js-messages-list");w=c(w).find(".js-message-wrapper");var y=w.eq(0).data("id");if(w=w.length)10>w&&x.find(".js-lazy-load").remove(),x=x.html(),t.append(x),
function(){function z(){0==A&&(b.$message_list.html(t.html()),t.html(""),c(document).trigger("scroll_button_event",{old_message_id:y,last_message_id:r}),c(document).trigger("new_message_lazy"))}var B=t[0].getElementsByTagName("img"),C=t[0].getElementsByTagName("video"),A=B.length+C.length;z();for(var D=0;D<C.length;D++)3<=C[D].readyState?(A--,z()):(C[D].addEventListener("loadeddata",function(){A--;z()}),C[D].addEventListener("error",function(){A--;z()}));for(D=0;D<B.length;D++)B[D].complete?(A--,
z()):(B[D].addEventListener("load",function(){A--;z()}),B[D].addEventListener("error",function(){A--;z()}))}()}})}function m(){b.$form.find("#tooltip-error-message").removeClass("errormsg").html("");b.$form.off("change",m);d.off("focus",m)}function n(){e.prop("disabled",!1);e.find("svg").removeClass("fa-spinner fa-spin").addClass("fa-arrow-up");d.attr("disabled",!1);g.prop("disabled",!1);b.$form.find("button").prop("disabled",!1);d.val("");760<c(window).width()&&d.focus();if(b.files_storage.length){var r=
b.$form.find(".c-upload-item");c.each(r,function(u,v){v.remove()});b.files_storage=[]}}k.preventDefault();if(!b.is_locked){var p=function(){var r=b.send_action_url,u=b.$form.serializeArray();d.attr("disabled",!0);g.prop("disabled",!0);c.post(r,u,function(v){if("ok"===v.status)h(v.data.message_id),n(),e.prop("disabled",!0),a(),c("#c-messages-page #c-messages-sidebar").length&&c(document).trigger("msg_sidebar_upd_needed");else{console.error(v);var t=b.$form.find("#tooltip-error-message");v=Object.values(v.errors);
t.addClass("errormsg");c.each(v,function(w,x){t.append('<span><i class="fas fa-exclamation-triangle state-error"></i> <span class="small">'+x+"</span></span>")});d.addClass("shake animated");setTimeout(function(){d.removeClass("shake animated")},500);b.is_locked=!1;n();e.prop("disabled",!0);a();d.on("focus",m);b.$form.on("change",m)}}).always(function(){b.is_locked=!1})};b.is_locked=!0;e.prop("disabled",!0);e.find("svg").removeClass("fa-arrow-up").addClass("fa-spinner fa-spin");b.$form.find("button").prop("disabled",
!0);if(b.is_images_enabled||b.is_files_enabled){k=function(){b.files_controller.uploadFiles(q,l,p)};var q=[{name:"hash",value:f+b.hash}];b.is_images_enabled?b.files_controller.uploadFiles([{name:"hash",value:"photos-"+b.hash}],"image",k):k()}else p()}})};return CRMImConversationSection}(jQuery),CRMConversationAssociateDealDialog=function(c){CRMConversationAssociateDealDialog=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.$footer=this.$wrapper.find(".js-dialog-footer");
...
b=a.$form.find(".js-select-deal .js-visible-link .js-text"),d=a.$form.find(".js-select-funnel"),e=a.$form.find(".js-deals-list"),f=a.$form.find(".js-deal-name-field");a.$form.on("click",".js-create-new-deal",function(){a.$submit.addClass("yellow").removeAttr("disabled");var g=c(this).find(".js-text").html();a.deal_selected=!0;d.removeClass("hidden");f.removeClass("hidden");a.$deal_name.focus();b.html(g);a.$deal_id.val("0")});a.$form.on("click",".js-deal-item",function(){a.$submit.addClass("yellow").removeAttr("disabled");
var g=c(this).find(".js-text").html();a.deal_selected=!0;e.find("li").removeClass("selected");c(this).parent().addClass("selected");b.html(g);d.addClass("hidden");f.addClass("hidden");a.$deal_name.val("");a.$deal_id.val(c(this).data("deal-id"))});e.on("click",function(){e.hide();setTimeout(function(){e.removeAttr("style")},200)});a.$form.on("change",".js-select-deal-funnel",function(){a.$form.find(".js-select-stage-wrapper").load("?module=deal&action=stagesByFunnel&id="+c(this).val())})};CRMConversationAssociateDealDialog.prototype.initSubmit=
...
CRMPageMessagesOperations.prototype.initCheckboxAll=function(){var a=this;a.$checkbox_all.click(function(){var b=c(this),d=b.is(":checked");0<a.checked_count?a.checked_count<=a.getTotalItemsAtPage()?a.unselectAll():a.selectAll():(b.prop("checked",d),d?a.selectAll():a.unselectAll());a.formBar()})};CRMPageMessagesOperations.prototype.initOperations=function(){var a=this;a.$bar.on("click",".crm-operation-link",function(b){b.preventDefault();switch(c(this).data("id")){case "associate":a.associateWithDeal();
...
a.locales.ban_text+"\n            </label>":"",k=function(h,m){m='<div class="flexbox vertical space-16">'+l+' <span class="text-red">'+c.trim(m)+"</span></div>";c.waDialog.confirm({title:'<i class="fas fa-exclamation-triangle smaller state-error"></i> '+h,text:m,success_button_title:a.locales.delete_button,success_button_class:"danger",cancel_button_title:a.locales.cancel_button,cancel_button_class:"light-gray",onSuccess:function(n,p){n=n.$content.find("#ban_checkbox").prop("checked");var q=e;n&&
...
"reply"!=b.action||b.deal_id||b.initSelectDeal();!b.iframe&&b.dialog.resize();b.iframe&&(760>c(window.parent).width()&&c(".dialog .dialog-content",window.parent.document).css("padding","0 0.75rem"),b.$wrapper.on("click",".js-close-dialog",function(d){d.preventDefault();c(".dialog iframe",window.parent.document)[0].dispatchEvent(new Event("close"));a(b.$textarea)}))};CRMSendEmailDialog.prototype.getFilesController=function(){function b(r){r.length&&c.each(r,function(u,v){u=n;var t=u.push,w=c(h);w.find(".js-name").text(v.name);
k.prepend(w);!e.iframe&&e.dialog.resize();t.call(u,{$file:w,file:v})})}function d(r){var u=[];c.each(n,function(v,t){r[0]!==t.$file[0]?u.push(t):r.remove()});n=u}var e=this,f=e.$wrapper.find(".js-files-wrapper");if(!f.length)return!1;var g=e.$wrapper.find(".js-drop-area"),l=e.$wrapper.find(".js-drop-field"),k=f.find(".c-upload-list"),h=e.file_template_html,m=e.app_url+"?module=file&action=uploadTmp",n=[],p=0,q=0;l.on("change",function(r){r.preventDefault();b(this.files)});g.on("drop",function(r){r.preventDefault();
...
!1)}}var t=u?u:function(){};n.length?(p=n.length,c.each(n,function(w,x){v(x)})):t()}}};CRMSendEmailDialog.prototype.initWYSIWYG=function(){var b=this,d=b.$textarea,e=0;b.iframe||(b.dialog.onClose=function(){a(d)});b.crm.initWYSIWYG(d,{allowedAttr:[["section","data-role"]],callbacks:{change:function(){var f=b.iframe?0:b.dialog.$block.height();e&&e===f||(e=f,!b.iframe&&b.dialog.resize())}}});b.body&&d.redactor("code.set",b.body)};CRMSendEmailDialog.prototype.initSendMessage=function(){var b=this,d=
b.$wrapper.find(".js-to-email"),e=b.$to_id,f=b.$wrapper.find(".js-to-new-name");b.is_locked=!1;b.$form.on("submit",function(l){l.preventDefault();if(!(0!=e.val()||c.trim(f.val())&&c.trim(d.val())||"new"!=b.action&&"forward"!=b.action))return c(".js-to-input, .js-to-add-name, .js-to-add-email").addClass("shake animated").focus(),setTimeout(function(){c(".js-to-input, .js-to-add-name, .js-to-add-email").removeClass("shake animated").focus()},500),!1;if(!c.trim(d.val()))return b.$to_input.addClass("shake animated").focus(),
setTimeout(function(){b.$to_input.removeClass("shake animated").focus()},500),!1;if(!b.is_locked){b.is_locked=!0;l=b.$form.find(".js-submit-button");var k=b.$form.find(".js-drop-field"),h=c('<span class="icon size-16 submit-loader"><i class="fas fa-spinner wa-animation-spin custom-mr-4"></i></span>');l.attr("disabled",!0);h.insertAfter(l);k.attr("disabled",!0);b.$form.find("button").prop("disabled",!0);b.filesController.uploadFiles([{name:"hash",value:b.hash}],g)}});var g=function(){var l=b.send_action_url,
k=b.$form.serializeArray(),h=function(m,n){if(!c.isEmptyObject(m)){m=c.extend(!0,{},m);var p=b.locales.send_error_title||"Send error",q=b.locales.send_error_text||"Can't send email message";q=c(b.error_html).find(".js-error-text").text(q).end();var r=m.common;b.is_admin&&(r?(q.find(".js-technical-info-block.js-known-error").show(),q.find(".js-technical-info-text").text(r)):q.find(".js-technical-info-block.js-unknown-error").show());c.crm.alert.show({title:p,text:q.get(0).outerHTML,button:b.locales.close||
"close",button_class:"red",onOpen:function(u){u.find(".js-technical-info-link").one("click",function(){c(this).remove();u.find(".js-technical-info-text").show()})}});delete m.common;c.isEmptyObject(m)||console.log.error(m)}n&&console.log.error(n)};c.post(l,k,"json").done(function(m){if("ok"===m.status){a(b.$textarea);b.$wrapper.find(".dialog-body").html(b.success_html);!b.iframe&&b.dialog.resize();var n=null;b.iframe||(b.dialog.onClose=function(){n&&clearTimeout(n);n=null;c.crm?c.crm.content.reload():
b.crm.content.reload()});n=setTimeout(function(){b.iframe?c(".dialog iframe",window.parent.document)[0].dispatchEvent(new Event("close")):b.dialog.close()},2500)}else!b.iframe&&b.dialog.close(),c.isEmptyObject(m.errors)?h({common:"Send error"},m):h(m.errors)}).always(function(){b.is_locked=!1}).fail(function(m){h({},m);!b.iframe&&b.dialog.close()})}};CRMSendEmailDialog.prototype.initPersonalSettingsDialog=function(){function b(){d.is_locked||(d.is_locked=!0,c.post(d.crm.app_url+"?module=email&action=personalSettingsDialog",
{},function(e){c.waDialog({html:e,options:{onSave:function(f){d.$textarea.redactor("core.editor").find('[data-role="c-email-signature"]').html(f.email_signature||"");d.$wrapper.find(".js-sender-name").text(f.sender_name||"")}}})}).always(function(){d.is_locked=!1}))}var d=this;d.$wrapper.on("click",".js-show-personal-settings-dialog",function(e){e.preventDefault();b()})};CRMSendEmailDialog.prototype.senderEmailSelect=function(){var b=this;b.$wrapper.on("change",b.$sender_email_select,function(d){d.preventDefault();
b.$sender_email.val(b.$sender_email_select.val())})};CRMSendEmailDialog.prototype.initEmailTo=function(){function b(){var t=c.trim(l.val()).split(/\s+/),w=!1;c.each(t,function(x,y){c.crm.check.email(y)&&!w&&(g=y.replace(/<|>/g,""),w=x)});!1!==w&&t.splice(w,1);f=t.join(" ");g&&f&&(t='<span class="js-edit-recipient" title="'+e.locales.edit_recipient+'"><b>'+c.crm.escape(f)+'</b> <span style="color: #999;">&lt;'+c.crm.escape(g)+"&gt;</span></span>",d(0,g,t),h.val(c.crm.escape(f)));if(f&&!g)return t=
'<span class="js-edit-recipient" title="'+e.locales.edit_recipient+'"><b>'+c.crm.escape(f)+'</b></span> <input class="c-to-input js-to-add-email" type="text" autocomplete="off" placeholder="'+e.locales.type_email+'" style="margin-left: 8px; width: 150px;" />',d(0,null,t),h.val(c.crm.escape(f)),c(".js-to-add-email").focus(),!1;if(g&&!f)return t='<input class="c-to-input js-to-add-name" type="text" autocomplete="off" placeholder="'+e.locales.type_name+'" style="margin-right: 8px; width: 150px;" > <span class="js-edit-recipient" title="'+
e.locales.edit_recipient+'" style="color: #999;">&lt;'+c.crm.escape(g)+"&gt;</span>",d(0,g,t),c(".js-to-add-name").focus(),!1}function d(t,w,x){n.prepend('<div class="c-email-to-user js-email-to-user">'+x+' <a title="'+e.locales.change_recipient+'" class="c-remove-to-email js-remove-to-email text-gray"><i class="icon fas fa-times-circle delete"></i></a></div>');p.removeClass("hidden");l.addClass("hidden").val("");k.val(w);m.val(t).trigger("change")}var e=this,f=null,g=null,l=e.$to_input,k=e.$wrapper.find(".js-to-email"),
h=e.$wrapper.find(".js-to-new-name"),m=e.$to_id,n=e.$wrapper.find(".js-to-name"),p=e.$wrapper.find(".js-deal-field"),q=e.$wrapper.find(".js-select-funnel"),r=e.$wrapper.find(".js-select-deal .js-visible-link .js-text"),u=e.$wrapper.find(".js-deals-list"),v=e.$wrapper.find(".js-create-new-deal");l.autocomplete({source:c.crm.app_url+"?module=autocomplete&emailcomplete=true",appendTo:e.$wrapper,minLength:0,focus:function(){return!1},select:function(t,w){t=w.item;w=(t.criteria||{}).email||t.email;d(t.id,
w,'<i class="icon userpic" style="background-image: url('+t.photo_url+');"></i>'+t.name+' <span style="color: #999;">&lt;'+w+"&gt;</span>");return!1}}).data("ui-autocomplete")._renderItem=function(t,w){return c('<li class="ui-menu-item-html"><div>'+w.value+"</div></li>").appendTo(t)};l.on("focus",function(){l.data("uiAutocomplete").search(l.val())});l.on("keydown",function(t){if(13==t.keyCode)return t.preventDefault(),l.blur(),!1});l.on("focusout",function(t){t.preventDefault();b();return!1});n.on("click",
".js-remove-to-email",function(){c(this).parent(".js-email-to-user").remove();l.removeClass("hidden").focus();p.addClass("hidden");q.addClass("hidden");u.html(v);r.html(""+e.locales.deal_select);k.val("");h.val("");m.val("").trigger("change");g=f=null});e.$wrapper.on("click",".js-edit-recipient",function(){var t=[f,g].join(" ");l.val(c.trim(t)).removeClass("hidden").focus();c(".js-email-to-user").remove();g=f=null;k.val("");h.val("")});e.$wrapper.on("keydown",".js-to-add-email",function(t){13==t.keyCode&&
(t.preventDefault(),c(this).blur())});e.$wrapper.on("focusout",".js-to-add-email",function(){var t=c(".js-to-add-email"),w=c.trim(t.val());w.length&&!0===c.crm.check.email(w)&&(g=w,k.val(c.crm.escape(w)),t.before(c('<span class="js-edit-recipient" title="'+e.locales.edit_recipient+'" style="color: #999;"></span>').text("<"+w+">")).remove())});e.$wrapper.on("keydown",".js-to-add-name",function(t){13==t.keyCode&&(t.preventDefault(),c(this).blur())});e.$wrapper.on("focusout",".js-to-add-name",function(){var t=
c.trim(c(".js-to-add-name").val());t.length&&(f=t,h.val(c.crm.escape(t)),c(".js-to-add-name:first-child").before(c('<span class="js-edit-recipient" title="'+e.locales.edit_recipient+'"><b></b></span>').text(t)).remove())})};CRMSendEmailDialog.prototype.initEmailCopy=function(){function b(){var p=c.trim(h.val()).split(/[,:;]/);if(p[0].length)return c.each(p,function(q,r){q=c.trim(r).split(/\s+/);var u=!1,v=r=null;c.each(q,function(t,w){c.crm.check.email(w)&&!u&&(r=w.replace(/<|>/g,""),u=t)});!1!==
u&&q.splice(u,1);v=q.join(" ");r&&v&&e(0,r,v);r&&!v&&d(0,r,r);v&&!r&&(h.addClass("shake animated"),setTimeout(function(){h.removeClass("shake animated")},500));u=!1;v=r=null}),!1}function d(p,q,r){k.children('[data-email="'+q+'"]').length&&"0"!==p||!q.length||(c(".email-copy-input-div").before('<div class="email-copy-user" title="'+c.crm.escape(q)+'" data-contact-id="'+c.crm.escape(p)+'" data-email="'+c.crm.escape(q)+'">'+r+' <a title="'+f.locales.remove_form_cc+'" class="remove-cc js-remove-cc">x</a> <input name="cc['+
c.crm.escape(q)+'][email]" type="hidden" value="'+c.crm.escape(q)+'" /><input name="cc['+c.crm.escape(q)+'][id]" type="hidden" value="'+c.crm.escape(p)+'" /></div>'),c(".email-copy-text-collapsed").append('<div class="email-copy-user" title="'+c.crm.escape(q)+'" data-email="'+c.crm.escape(q)+'">'+r+"</div>"));h.val("").focus()}function e(p,q,r){k.children('[data-email="'+q+'"]').length&&"0"!==p||!q.length||(c(".email-copy-input-div").before('<div class="email-copy-user" title="'+c.crm.escape(q)+'" data-contact-id="'+
p+'" data-email="'+c.crm.escape(q)+'">'+c.crm.escape(r)+' <a title="'+f.locales.remove_form_cc+'" class="remove-cc js-remove-cc">x</a> <input name="cc['+c.crm.escape(q)+'][email]" type="hidden" value="'+c.crm.escape(q)+'" /><input name="cc['+c.crm.escape(q)+'][id]" type="hidden" value="'+c.crm.escape(p)+'" /><input name="cc['+c.crm.escape(q)+'][name]" type="hidden" value="'+c.crm.escape(r)+'" /></div>'),c(".email-copy-text-collapsed").append('<div class="email-copy-user" title="'+c.crm.escape(q)+
'" data-email="'+c.crm.escape(q)+'">'+c.crm.escape(r)+"</div>"));h.val("").focus()}var f=this,g=f.$wrapper.find(".email-copy-wrapper"),l=g.find(".email-copy-area"),k=l.find(".email-copy-text"),h=l.find(".email-copy-input"),m=g.find(".deal-participants-area"),n=f.$wrapper.find(".email-copy-wrapper-collapsed");h.autocomplete({source:c.crm.app_url+"?module=autocomplete&emailcomplete=true",appendTo:f.$wrapper,minLength:0,focus:function(){return!1},select:function(p,q){p=q.item;d(p.id,(p.criteria||{}).email||
p.email,'<i class="icon userpic" style="background-image: url('+p.photo_url+');"></i>'+p.name);h.val("");return!1}}).data("ui-autocomplete")._renderItem=function(p,q){return c('<li class="ui-menu-item-html"><div>'+q.value+"</div></li>").appendTo(p)};h.on("focus",function(){h.data("uiAutocomplete").search(h.val())});f.$wrapper.on("click",".js-email-copy-link, .email-copy-text-collapsed",function(p){p.preventDefault();p=f.$wrapper.find(".js-email-copy-link-icon");g.toggleClass("email-copy-wrapper-block");
g.is(":visible")?(p.removeClass("fa-caret-right").addClass("fa-caret-down"),h.focus(),n.removeClass("email-copy-wrapper-collapsed-block")):(p.removeClass("fa-caret-down").addClass("fa-caret-right"),c(".email-copy-text-collapsed").children().length&&n.addClass("email-copy-wrapper-collapsed-block"))});l.on("click",function(){h.focus()});m.on("click",".email-copy-user",function(){var p=c(this).data("cc-contact-id"),q=c(this).data("cc-email"),r=c(this).html();d(p,q,r)});h.on("focusout",function(p){b()});
h.on("keydown",function(p){13==p.keyCode&&(p.preventDefault(),b())});k.on("click",".js-remove-cc",function(p){p.preventDefault();p=c(this).parent(".email-copy-user");var q=p.data("email");p.remove();c(".email-copy-text-collapsed").children('[data-email="'+q+'"]').remove()});h.on("keydown",function(p){if(8==p.keyCode&&0==h.val().length){p=c(".email-copy-text .email-copy-user:last");var q=p.data("email");p.remove();c(".email-copy-text-collapsed").children('[data-email="'+q+'"]').remove();h.focus()}})};
CRMSendEmailDialog.prototype.initSelectDeal=function(){function b(){var n=h.val();k.val("none");n&&c.get("?module=deal&action=byContact&id="+n,function(p){"ok"===p.status&&c.each(p.data.deals,function(q,r){l.prepend('<li><a href="javascript:void(0);" class="js-deal-item" data-deal-id="'+r.id+'"><span class="js-text flexbox"><i class="fas fa-circle funnel-state" style="color: '+p.data.funnels[r.funnel_id].stages[r.stage_id].color+'"></i><span class="flexbox">'+r.name+"</span></span></a></li>")})},
"json")}var d=this,e=d.$form.find(".js-select-deal .js-visible-link .js-text"),f=d.$form.find(".js-select-funnel"),g=d.$wrapper.find(".js-deal-field"),l=d.$form.find(".js-deals-list"),k=d.$form.find(".js-deal-id"),h=d.$to_id,m=d.$form.find(".js-empty-deal");d.$form.on("click",".js-create-new-deal",function(){var n=c(this).find(".js-text").html();f.removeClass("hidden");m.removeClass("c-empty-deal-hidden");e.html(n);k.val("0")});m.on("click",function(){c(this).addClass("c-empty-deal-hidden");f.addClass("hidden");
l.find("li").removeClass("selected");e.html(d.locales.deal_empty);k.val("none")});d.$form.on("click",".js-deal-item",function(){var n=c(this).find(".js-text").html();l.find("li").removeClass("selected");c(this).parent().addClass("selected");e.html(n);m.removeClass("c-empty-deal-hidden");f.addClass("hidden");k.val(c(this).data("deal-id"))});"reply"==d.action&&b();"new"==d.action&&0<h.val()&&(b(),g.removeClass("hidden"));h.on("change",b);l.on("click",function(){l.hide();setTimeout(function(){l.removeAttr("style")},
200)});d.$form.on("change",".js-select-deal-funnel",function(){d.$form.find(".js-select-stage-wrapper").load("?module=deal&action=stagesByFunnel&id="+c(this).val())})};return CRMSendEmailDialog}(jQuery);var CRMSendSmsDialog=function(c){CRMSendSmsDialog=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.$textarea=this.$wrapper.find(".js-send-sms-textarea");this.dialog=this.$wrapper.data("dialog");this.success_html=a.success_html;this.error_html=a.error_html;this.hash=a.hash;this.send_action_url=a.send_action_url;var b=!1;window&&window.parent&&window.parent.$&&window.parent.$.crm?b=window.parent.$.crm:window.$&&window.$.crm&&(b=window.$.crm);this.crm=b;this.action=a.action;
...
h=k.find("a").length;l.length||l.push({id:!1,name:b.locales.no_segments});c.each(l,function(m,n){m="javascript:void(0);";var p="",q='<span class="hint">'+n.name+"</span>";n.id&&(m=c.crm.app_url+"contact/segment/"+n.id+"/",p='<i class="icon16 '+(n.icon?n.icon:"folder-dynamic")+'"></i>',q="<span>"+n.name+"</span>");n='<a href="'+m+'">'+p+q+"</a>";h&&(n=","+n);k.append(n);h=!0})}var b=this,d=b.$wrapper.find(".crm-contact-assign-segments"),e=c.crm.app_url+"?module=contactOperation&action=addToSegments&is_assign=1",
...
f.hide();setTimeout(function(){f.removeAttr("style")},200);g=g.data("id");e.val(g).trigger("change");(g=a.funnels[g]||!1)&&b.trigger("changeFunnel",g)});c.crm.renderSVG(b)})();(function(){function d(h){k.html("");c.each(h,function(m,n){m=a.stage_template_html;var p=c("<div />").text(n.name).html();m=m.replace("%id%",n.id).replace("%color%",n.color).replace("%name%",p);n=c(m);k.append(n)});c.crm.renderSVG(f);k.find("li:first-child a").trigger("click")}var e=a.$wrapper.find(".js-funnels-list"),f=a.$wrapper.find(".js-funnel-stages-list"),
g=f.find(".js-visible-link"),l=f.find(".js-field"),k=f.find(".menu-v");k.on("click","a",function(){var h=c(this);g.find(".js-text").html(h.html());k.find(".selected").removeClass("selected");h.closest("li").addClass("selected");k.hide();setTimeout(function(){k.removeAttr("style")},200);h=h.data("id");l.val(h)});e.on("changeFunnel",function(h,m){d(m.stages)})})();(function(){function d(){var l=c.datepicker._defaults.dateFormat,k=!1;try{c.datepicker.parseDate(l,f.val()),k=!0}catch(h){}k?(f.data("last-correct-value",
...
e=d.find(".js-view-toggle"),f=e.find(".is-active");e.on("click",".c-toggle",function(g){g.preventDefault();g=c(this);var l=g.data("id");if(g.hasClass("is-active"))return!1;a.contact_mode=l;f.length&&f.removeClass("is-active");g.addClass("is-active");f=g;d.find(".c-hidden.is-active").removeClass("is-active");d.find(".c-hidden-"+l).addClass("is-active")})})()};CRMNewContactPage.prototype.initCreate=function(){function a(){function g(){var n=e.$wrapper.find(".js-option-field:checked"),p=e.$wrapper.find(".js-vault-toggle"),
q=e.$wrapper.find(".js-owners-list");n.length&&(n=n.val(),"vaults"===n?(p=p.find(".js-field").val(),m.data.push({name:"vault_id",value:p})):"owners"===n&&(p=q.find(".c-owner"),p.length||m.errors.push({name:"owner[autocomplete]"}),p.each(function(){var r=c(this).data("id");0<r&&m.data.push({name:"owners[]",value:r})})))}function l(){var n=e.$segmentsSection.find("form").serializeArray();c.each(n,function(p,q){m.data.push(q)})}function k(){var n=e.$dealSection.find("form").serializeArray(),p=!0;c.each(n,
function(q,r){m.data.push(r);0<c.trim(r.value).length&&"deal[name]"===r.name&&(p=!1)});p&&m.errors.push({name:"deal[name]",value:e.locales.empty_deal_name})}function h(){var n=e.$extendedFieldsW.data("getFormData")();c.each(n.data,function(p,q){m.data.push(q)});c.each(n.errors,function(p,q){m.errors.push(q)})}var m={data:[],errors:[]};(function(){var n=e.$contactSection.find("form.c-general-form"),p=n.find(".js-phone-list"),q=n.find(".js-email-list");p.find("li .js-value, li .js-ext").removeAttr("name");
q.find("li .js-value, li .js-ext").removeAttr("name");n=n.serializeArray();var r=["contact[firstname]","contact[middlename]","contact[lastname]","contact[name]"],u=!0;c.each(n,function(v,t){t.value=c.trim(t.value);m.data.push(t);0<c.trim(t.value).length&&0<=r.indexOf(t.name)&&(u=!1)});u&&(n=e.$contactSection.find('[name="contact[firstname]"]'),n.length?m.errors.push({name:"contact[firstname]",value:e.locales.empty_contact_name}):(n=e.$contactSection.find('[name="contact[name]"]'),n.length&&m.errors.push({name:"contact[name]",
value:e.locales.empty_contact_name})));p.find("li").each(function(v){var t=c(this),w=t.find(".js-value");t=t.find(".js-ext");var x=w.val(),y=t.val();if(x){var z="contact[phone]["+v+"][value]";w.attr("name",z);m.data.push({name:z,value:x});v="contact[phone]["+v+"][ext]";t.attr("name",v);m.data.push({name:v,value:y})}});q.find("li").each(function(v){var t=c(this),w=t.find(".js-value");t=t.find(".js-ext");var x=w.val(),y=t.val();if(x){var z="contact[email]["+v+"][value]";w.attr("name",z);m.data.push({name:z,
value:x});v="contact[email]["+v+"][ext]";t.attr("name",v);m.data.push({name:v,value:y})}})})();e.$extendedFieldsW.length&&e.is_extended&&h();e.selected_access&&g();e.selected_segments&&l();e.create_deal&&k();return m}function b(g,l){l=l?l:[];g&&(l=g);c.each(l,function(k,h){k=h.value;var m=e.$wrapper.find(':input[name="'+h.name+'"]'),n=c("<div />").addClass("errormsg").text(k);m.length?m.hasClass("error")||(m.parent().append(n),m.addClass("error").one("focus click change",function(){m.removeClass("error");
n.remove()})):e.$errorsSection.append(n)})}function d(g){f||(f=!0,c.post("?module=contact&action=newSave",g,function(l){"ok"===l.status?c.crm.content.load(c.crm.app_url+"contact/"+l.data.contact.id+"/"):b(l.errors)},"json").always(function(){f=!1}))}var e=this,f=!1;e.$wrapper.on("click",".js-submit-form",function(g){g.preventDefault();e.$errorsSection.html("");g=a();g.errors.length?b(!1,g.errors):d(g.data)})};CRMNewContactPage.prototype.initExtendedFields=function(){function a(f){f=f||{};var g=b.$wrapper,
l="c-conditional-field-"+(""+Math.random()).slice(2),k=f.input_name;f.parent_field=f.parent_field||"";f.parent_options=f.parent_options||{};f.index=f.index||0;if(f.parent_field&&!c.isEmptyObject(f.parent_options)){var h=f.parent_field.split(":");if(!(1>=h.length)){h='[name="contact['+h[0]+"]["+f.index+"]["+h[1]+']"]';var m=g.find(h);if(!(0>=m.length)){f.show_empty_option=f.required;var n=f.parent_options,p=g.find(':input[name="'+k+'"]').first();if(!(0>=p.length)){if(p.is("input"))var q=p.next();else q=
p,p=q.prev();var r=function(){p[0].hasAttribute("name")||(p.attr("name",q.attr("name")),q[0].removeAttribute("name"));p.show().val("");q.hide()};g=function(){var u=p.is(":visible")?p.val():q.val();var v=c(this).val().toLowerCase();f.hide_unmatched&&p.closest(".field").show();if(n&&n[v]){v=n[v];p.hide();q.show().children().remove();f.show_empty_option&&q.append(c('<option value=""></option>'));for(var t=0;t<v.length;t++)q.append(c("<option></option>").attr("value",v[t]).text(v[t]));q.val(u);p[0].hasAttribute("name")&&
...
g.on("click",".js-remove",function(){c(this).closest(".c-item").remove()});l.on("change",".js-ext-toggle",function(){var h=c(this),m=h.closest(".c-item").find(".js-ext-field");"custom"===h.val()?m.attr("disabled",!1).show():m.attr("disabled",!0).hide()})});d.on("prepareSave",function(){f.each(function(){c(this).find(".js-list").find(".c-item").each(function(g){c(this).find("[data-name-pattern]").each(function(){var l=c(this),k=l.data("name-pattern");k&&(k=k.replace("%index%",g),l.attr("name",k))})})})})})();
(function(){function f(g){function l(){h.each(function(){c(this).find(".js-address-wrapper").each(function(n){c(this).find("[data-name-pattern]").each(function(){var p=c(this),q=p.data("name-pattern");if(q&&(q=q.replace("%index%",n),p.attr("name",q),p.is("input"))){var r=p.closest(".js-custom-sub-field-line");r.is('[data-type="Conditional"]')&&(p=r.data("field-id"),r=r.data("subfield-id"),(p=k[p].fields[r])&&a(c.extend(p,{input_name:q,index:n})))}})})})}(function(){g.find(".js-custom-sub-field-line").each(function(){var n=
c(this),p=n.data("name-pattern");n.find(":input").each(function(){var q=c(this);q.attr("data-name-pattern",p);q.removeAttr("name")})});g.find('.js-custom-sub-field-line[data-type="Select"]').each(function(){var n=c(this);n.data("sub-type")||n.find("select").find('option[value=""]').text(n.data("label")+"...")});g.find('.js-custom-sub-field-line[data-type="Checkbox"]').each(function(){var n=c(this),p=n.find(":checkbox");p.wrap("<label>");p=p.parent();p.append("<span>");p.find("span").append(" "+n.data("label"))})})();
var k=b.extended_fields||{},h=g.find(".js-address-list"),m=g.find(".js-address-wrapper")[0].outerHTML;d.on("prepareSave",l);l();g.on("click",".js-add",function(){h.append(m);l()});g.on("click",".js-remove",function(){c(this).closest(".js-address-wrapper").remove()});g.on("click",".js-clear-selected-radio-value",function(){c(this).closest(".js-custom-sub-field-line").find(":radio:checked").attr("checked",!1)});g.on("change",".js-ext-toggle",function(){var n=c(this),p=n.closest(".js-address-wrapper").find(".js-ext-field");
...
function(){function a(l){function k(m){var n=c('<div class="line errormsg" />').html(m);f.append(n);d.$form.one("submit",function(){n.remove()})}if(l[0])c.each(l,function(m,n){k(n.value)});else{var h=Object.keys(l);c.each(h,function(m,n){k(l[n])})}}function b(l){g||(g=!0,c.post("?module=contact&action=addEmployeeSave",l,function(k){"ok"===k.status?(c.crm.content.reload(),d.dialog.close()):k.errors&&a(k.errors)}).always(function(){g=!1}))}var d=this,e=d.$wrapper.find("#c-contact-add-employee-form"),
...
function(){this.initAutocomplete();this.initSubmit()};CRMContactUpdateDialog.prototype.initAutocomplete=function(){var a=this,b=a.$wrapper.find(".js-autocomplete-wrapper"),d=b.find(".js-autocomplete"),e=b.find(".js-field"),f=a.$wrapper.find(".js-field-autocomplete");d.autocomplete({appendTo:b,source:c.crm.app_url+"?module=autocomplete",minLength:2,html:!0,focus:function(){return!1},select:function(g,l){g=c("<div />").text(l.item.name).text();d.val(g).blur();e.val(l.item.id).trigger("change");g=f.html;
l=l.item;l=contactHtml='        \n            <div class="c-column c-column-image">\n                <a class="flexbox middle" href="'+c.crm.app_url+l.link+'" target="_top" data-link="top" >\n                    <img src="'+l.photo_url+'" alt="'+l.name+'">\n                </a>\n            </div>\n            <div class="c-column middle nowrap">\n                <a href="'+c.crm.app_url+l.link+'" target="_top" data-link="top" title="'+l.name+'">'+l.name+"</a>\n            </div>";g.call(f,l);return!1}}).data("ui-autocomplete")._renderItem=
function(g,l){return c("<li />").addClass("ui-menu-item-html").append("<div>"+l.value+"</div>").appendTo(g)};e.on("change",function(){a.$submitButton.attr("disabled",!1)})};CRMContactUpdateDialog.prototype.initSubmit=function(){function a(){var k={data:[],errors:[]},h=f.serializeArray();c.each(h,function(m,n){k.data.push(n)});return k}function b(k){k=k?k:[];c.each(k,function(h,m){h=m.value;var n=f.find('[name="'+m.name+'"]'),p=c("<span />").addClass("errormsg").text(h);n.length?n.hasClass("error")||
(n.parent().append(p),n.addClass("error").one("focus click",function(){n.removeClass("error");p.remove()})):(g.append(p),f.one("submit",function(){p.remove()}))})}function d(k){l||(l=!0,c.post("?module=contact&action=update",k,function(h){"ok"===h.status?(e.$wrapper.find(".js-field-autocomplete").data("success",!0),e.$dialog.close()):b(h.errors)},"json").always(function(){l=!1}))}var e=this,f=e.$form,g=e.$wrapper.find(".js-errors-place"),l=!1;f.on("submit",function(k){k.preventDefault();k=a();k.errors.length?
b(k.errors):d(k.data)})};return CRMContactUpdateDialog}(jQuery),CRMContactAddCompanyContactDialog=function(c){CRMContactAddCompanyContactDialog=function(a){this.$wrapper=a.$wrapper;this.$form=this.$wrapper.find("form");this.$company_value=this.$wrapper.find(".js-company-value");this.$submit=this.$wrapper.find(".js-submit");this.$footer=this.$wrapper.find(".js-dialog-footer");this.$company_id_input=this.$wrapper.find(".js-company-id");this.$company_name_input=this.$wrapper.find(".js-autocomplete-company");
this.dialog=this.$wrapper.data("dialog");this.initClass()};CRMContactAddCompanyContactDialog.prototype.initClass=function(){var a=this;a.$form.on("click",".js-company-item",function(){a.$submit.prop("disabled",!1)});a.initAutoComplete();a.initSubmit()};CRMContactAddCompanyContactDialog.prototype.initAutoComplete=function(){var a=this,b=a.$wrapper.find(".js-selected-company"),d=a.$wrapper.find(".js-change-company"),e=a.$wrapper.find(".js-label-new-company");a.$company_name_input.autocomplete({source:function(f,
g){c.post("?module=autocomplete&type=company",f,function(l){c.isEmptyObject(l)&&f.term?e.css({color:"#999"}):e.css({color:"#FFF"});g(l)},"json")},minLength:0,focus:function(){return!1},select:function(f,g){a.$company_id_input.val(g.item.id);b.html('<i class="icon userpic size-20" style="background-image: url('+g.item.photo_url+');"></i>').append(c("<span />").text(g.item.name)).removeClass("hidden");a.$company_name_input.addClass("hidden");return!1}}).data("ui-autocomplete")._renderItem=function(f,
g){return c("<li />").addClass("ui-menu-item-html").append("<div>"+g.value+"</div>").appendTo(f)};a.$company_name_input.on("focus",function(){a.$company_name_input.data("uiAutocomplete").search(a.company_name)});d.on("click",function(){a.$company_id_input.val(0);b.html("").addClass("hidden");a.$company_name_input.removeClass("hidden").focus()})};CRMContactAddCompanyContactDialog.prototype.initSubmit=function(){function a(){var d=c('<span class="icon loading"><i class="fas fa-spinner wa-animation-spin"></i></span>'),
e=c.crm.app_url+"?module=contact&action=addCompanyContactSave",f=b.$form.serializeArray();b.$submit.prop("disabled",!0);b.$footer.append(d);c.post(e,f,function(g){"ok"===g.status?c.crm.content.reload():(b.$submit.prop("disabled",!1),d.remove())})}var b=this;b.$form.on("submit",function(d){d.preventDefault();d=b.$company_id_input.val();var e=b.$company_name_input.val();if(0==d&&!c.trim(e))return b.$company_value.addClass("shake animated"),b.$company_name_input.focus(),setTimeout(function(){b.$company_value.removeClass("shake animated");
...
m=g.find(".js-arrows-wrapper"),n=m.find(".js-arrow-left"),p=m.find(".js-arrow-right"),q=g.outerWidth(),r=l.outerWidth(),u=!1,v=0;b();(function(){function t(){if(c.contains(document,x[0])){var z=w.scrollTop()>y.top,B=f.$content.height()>w.height();z&&B?(k.css({left:y.left}).width(g.outerWidth()).addClass("is-fixed"),x.addClass("is-fixed")):(k.removeAttr("style").removeClass("is-fixed"),x.removeClass("is-fixed"))}else w.off("scroll",t)}var w=c(window),x=m,y=g.offset();w.on("scroll",t)})();n.on("click",
...
f=[],g=c.extend({},a.context,!0);g.is_assign=a.is_assign?1:0;var l=function(h){d.hide();b.attr("disabled",!1);a.sidebar&&c.each(h.segments||{},function(m,n){a.sidebar.updateItem({type:"segment",info:{id:n.id}},n)});g.is_assign?c.crm.content.reload():1==f.length?c.crm.content.load(c.crm.app_url+"contact/segment/"+f[0]+"/"):c.crm.content.load(c.crm.app_url)},k=function(h){h=c.extend({offset:h||0},g);c.post(e,h).done(function(m){"ok"==m.status&&(m.data.done?l(m.data):k(m.data.offset||0))})};b.click(function(h){h.preventDefault();
...
h=k?c.crm.app_url+"contact/responsible/"+k+"/":c.crm.app_url+"contact/";c.post(f,g,function(m){"ok"===m.data.result?c.crm.content.reload().then(function(){d.dialog.close();c.crm.content.load(h)}):"no_vault_access"===m.data.result&&(c(".crm-dialog-header h1").html(d.$errorh1),c(".crm-dialog-content p, .js-responsible-list").html(""),c(".crm-dialog-footer").html('<a class="button js-close-dialog" href="javascript:void(0);">'+d.$close_dialog+"</a>"),c(".no-access-error").html(m.data.message),c.each(m.data.users,
...
g.find("li .js-value, li .js-ext").removeAttr("name");var l=e.serializeArray(),k={data:[],errors:[]},h=["contact[firstname]","contact[middlename]","contact[lastname]","contact[name]"],m=!0;c.each(l,function(n,p){k.data.push({name:p.name,value:p.value});0<c.trim(p.value).length&&0<=h.indexOf(p.name)&&(m=!1)});m&&(k.errors.push({name:"contact[firstname]",value:"Name is empty"}),k.errors.push({name:"contact[name]",value:"Name is empty"}));f.find("li").each(function(n){var p=c(this),q=p.find(".js-value");
p=p.find(".js-ext");var r=q.val(),u=p.val();if(r){var v="contact[phone]["+n+"][value]";q.attr("name",v);k.data.push({name:v,value:r});n="contact[phone]["+n+"][ext]";p.attr("name",n);k.data.push({name:n,value:u})}});g.find("li").each(function(n){var p=c(this),q=p.find(".js-value");p=p.find(".js-ext");var r=q.val(),u=p.val();if(r){var v="contact[email]["+n+"][value]";q.attr("name",v);k.data.push({name:v,value:r});n="contact[email]["+n+"][ext]";p.attr("name",n);k.data.push({name:n,value:u})}});return k}
var b=this,d=b.$wrapper.find('.js-toggle-content[data-content="contact"]'),e=d.find("form");(function(){function f(h){c.post(b.contact_id?"?module=deal&action=contactUpdate":"?module=contact&action=save",h,function(m){"ok"==m.status?(b.dialog.close(),c.crm.content.load(c.crm.app_url+"contact/"+m.data.contact.id+"/")):m.errors&&g(m.errors)},"json").always(function(){k=!1})}function g(h){h=h?h:[];c.each(h,function(m,n){m=n.name;n=n.value;"name"===m&&(m="contact[firstname]");var p=e.find('[name="'+m+
...
m=!0;c.each(l,function(n,p){k.data.push({name:p.name,value:p.value});0<c.trim(p.value).length&&0<=h.indexOf(p.name)&&(m=!1)});m&&(k.errors.push({name:"contact[firstname]",value:"Name is empty"}),k.errors.push({name:"contact[name]",value:"Name is empty"}));f.find("li").each(function(n){var p=c(this),q=p.find(".js-value");p=p.find(".js-ext");var r=q.val(),u=p.val();if(r){var v="contact[phone]["+n+"][value]";q.attr("name",v);k.data.push({name:v,value:r});n="contact[phone]["+n+"][ext]";p.attr("name",
n);k.data.push({name:n,value:u})}});g.find("li").each(function(n){var p=c(this),q=p.find(".js-value");p=p.find(".js-ext");var r=q.val(),u=p.val();if(r){var v="contact[email]["+n+"][value]";q.attr("name",v);k.data.push({name:v,value:r});n="contact[email]["+n+"][ext]";p.attr("name",n);k.data.push({name:n,value:u})}});return k}var b=this,d=b.$wrapper.find('.js-toggle-content[data-content="company"]'),e=d.find("form");(function(){function f(){var m={data:[],errors:[]},n=e.serializeArray();c.each(n,function(p,
q){q.value=c.trim(q.value);"contact[company]"===q.name&&m.data.push({name:"contact[name]",value:q.value});m.data.push(q)});n=d.find(".js-phone-list");n.find("li .js-value, li .js-ext").removeAttr("name");n.find("li").each(function(p){var q=c(this),r=q.find(".js-value");q=q.find(".js-ext");var u=r.val(),v=q.val();if(u){var t="contact[phone]["+p+"][value]";r.attr("name",t);m.data.push({name:t,value:u});p="contact[phone]["+p+"][ext]";q.attr("name",p);m.data.push({name:p,value:v})}});n=d.find(".js-email-list");
n.find("li .js-value, li .js-ext").removeAttr("name");n.find("li").each(function(p){var q=c(this),r=q.find(".js-value");q=q.find(".js-ext");var u=r.val(),v=q.val();if(u){var t="contact[email]["+p+"][value]";r.attr("name",t);m.data.push({name:t,value:u});p="contact[email]["+p+"][ext]";q.attr("name",p);m.data.push({name:p,value:v})}});m.data.push({name:"contact[is_company]",value:1});return m}function g(m){m=m?m:[];c.each(m,function(n,p){n=p.name;p=p.value;var q=d.find('[name="'+n+'"]');"contact[firstname]"!==
n||q.is(":visible")||(q=d.find(".js-contact-autocomplete"));var r=c("<span class='c-error' />").addClass("errormsg").text(p);q.length&&!q.hasClass("error")?(q.parent().append(r),q.addClass("error").one("focus click change",function(){q.removeClass("error");r.remove()})):(h.append(r),e.one("submit",function(){remove(r)}))})}function l(m){k||(k=!0,c.post(b.contact_id?"?module=deal&action=contactUpdate":"?module=contact&action=save",m,function(n){"ok"===n.status?(b.dialog.close(),c.crm.content.load(c.crm.app_url+
"contact/"+n.data.contact.id+"/")):n.errors&&g(n.errors)},"json").always(function(){k=!1}))}var k=!1,h=d.find(".js-errors-place");e.on("submit",function(m){m.preventDefault();m=f();m.errors.length?g(errors):l(m.data)})})();(function(){d.on("click",".js-extended-form",function(){var f=a(),g=[];g.push("extended=true");g.push("type=company");c.each(f.data,function(l,k){(l=c.trim(k.value))&&g.push(k.name+"="+l)});f=c.crm.app_url+"contact/new/?"+g.join("&");c.crm.content.load(f)})})()};return CRMContactAddDialog}(jQuery),
...
m.photo_url),e.data("is_company",m.data.is_company),g.val(m.id),b(m.data);else if(0>m.id){g.val("");d.$toggleW.find(".js-show-fio").trigger("click",!0);f.val("");var n=0<=m.name.indexOf("@"),p=!!m.name.match(/^\+?\d/);n?d.$wrapper.find(".js-email-field").val(m.name).trigger("change"):p?d.$wrapper.find(".js-phone-field").val(m.name).trigger("change"):d.$wrapper.find(".js-firstname,.js-name").val(m.name).trigger("change")}}function b(m){c.each(l,function(n,p){n=m[p];var q="phone"===p||"email"===p,r;
if(q){n="";for(r=(m[p]||[]).reverse();""==n&&0<r.length;)n=r.pop();c.isPlainObject(n)&&(n=n.value)}n=c.trim(n||"");""!=n&&(p=q?d.$wrapper.find('[name="contact['+p+'][0][value]"]'):d.$wrapper.find('[name="contact['+p+']"]'),p.val(n))});f.attr("disabled",!0)}var d=this,e=d.$wrapper.find(".js-contact-autocomplete"),f=d.$wrapper.find(".js-field"),g=d.$wrapper.find(".js-contact-id-field"),l=["firstname","company","jobtitle","phone","email"],k="?module=autocomplete&add_new=true&join="+l.join(","),h=!1;
e.autocomplete({appendTo:e.closest(".value"),source:k,minLength:2,html:!0,focus:function(){return!1},select:function(m,n){h=!0;a(n.item);e.blur();return!1},search:function(){g.val("")},open:function(){h=!1},change:function(){h||a({id:-1,name:e.val()})}}).data("ui-autocomplete")._renderItem=function(m,n){return c("<li />").addClass("ui-menu-item-html").append("<div>"+n.value+"</div>").appendTo(m)};e.on("keydown",function(m){13!==m.keyCode&&(f.attr("disabled",!1),f.val(""))})};CRMContactAddForm.prototype.initCompanyAutocomplete=
function(){var a=this.$wrapper.find(".js-company-autocomplete"),b=this.$wrapper.find(".js-company-id-field");if(!a.length||!b.length)return!1;a.autocomplete({appendTo:this.$wrapper,source:"?module=autocomplete&type=company",minLength:2,html:!0,focus:function(){return!1},select:function(d,e){a.val(e.item.name);b.val(e.item.id);return!1},search:function(){b.val("")}}).data("ui-autocomplete")._renderItem=function(d,e){return c("<li />").addClass("ui-menu-item-html").append("<div>"+e.value+"</div>").appendTo(d)}};
CRMContactAddForm.prototype.initToggle=function(){function a(h,m){m&&b.$wrapper.find("input").val("").trigger("change");"fio"===h?(l.show(),k.hide(),e.val("new"),f.val(b.$locales.add),g.attr("disabled",!1)):(l.hide(),k.show(),e.val("search"),f.val(b.$locales.select));c(document).trigger("toggleChanged");c(document).trigger("resizeDialog")}var b=this,d=b.$toggleW;d.find(".is-active");var e=b.$wrapper.find(".js-action-field"),f=b.$wrapper.closest(".dialog-body").find(".js-some-action"),g=b.$wrapper.find(".js-field"),
l=b.$wrapper.find(".js-fio"),k=b.$wrapper.find(".js-combo-name");d.waSwitch({ready:function(h){var m=h.$wrapper.siblings("label");h.$label=m;h.active_text=m.data("active-text");h.inactive_text=m.data("inactive-text")},change:function(h,m){h?(a("fio",!1),m.$label.text(m.active_text)):(a("name",!1),m.$label.text(m.inactive_text))}})};return CRMContactAddForm}(jQuery),CRMCompanyAddForm=function(c){CRMCompanyAddForm=function(a){this.$wrapper=a.$wrapper;this.phone_template_html=a.phone_template_html;this.email_template_html=
a.email_template_html;this.initClass()};CRMCompanyAddForm.prototype.initClass=function(){this.initAddPhone();this.initAddEmail()};CRMCompanyAddForm.prototype.initAddPhone=function(){var a=this,b=a.$wrapper.find(".js-phone-list");a.$wrapper.on("click",".js-add-phone",function(d){d.preventDefault();d=c(a.phone_template_html);b.append(d)});a.$wrapper.on("click",".js-remove-phone",function(d){d.preventDefault();c(this).closest("li").remove()})};CRMCompanyAddForm.prototype.initAddEmail=function(){function a(e,
f){e.preventDefault();e=c(b.email_template_html);f&&(e.find(".js-value").val(f.name).trigger("change"),e.find(".js-ext").val(f.ext).trigger("change"));d.append(e)}var b=this,d=b.$wrapper.find(".js-email-list");b.$wrapper.on("click",".js-add-email",a);b.$wrapper.on("click",".js-remove-email",function(e){e.preventDefault();c(this).closest("li").remove()});c(document).on("addContactEmail",function(e,f){a(e,f)})};return CRMCompanyAddForm}(jQuery);var CRMContactImportPage=function(c){CRMContactImportPage=function(a){this.$wrapper=a.$wrapper;this.$iframe=this.$wrapper.find("iframe");this.initClass()};CRMContactImportPage.prototype.initClass=function(){this.initViewToggle();this.initSubmit()};CRMContactImportPage.prototype.initSubmit=function(){function a(f){f.on("change keydown","input, select, textarea",function(){f.find("input:submit").attr("disabled",!1)});f.on("submit",function(){f.find(".loading").show();e=!0})}var b=this,d=b.$iframe,e=
...
var f=c(this).data("content"),g=f===d;console.log(e.target);if(g)return!1;d=f;a.$wrapper.find(".js-toggle-content.is-active").removeClass("is-active");e=a.$wrapper.find('.js-toggle-content[data-content="'+f+'"]');e.length&&e.addClass("is-active")})};return CRMContactImportPage}(jQuery);var crmContactImportUpload=function(c){var a=function(d){var e=d.$wrapper,f=e.find("form"),g=e.find(".crm-import-upload-table"),l=e.find(".crm-import-upload-button"),k=e.find(".crm-total-count-column .crm-count");this.$wrapper=e;this.$form=f;var h=d.fieldInfo,m=/[\s~!@#\$%\^&\*\(\)_\+\|\\=\-`\{\}:"<>\?\[\];',\.\/]+/g,n=function(u,v){u+=1;v?(g.find("td:nth-child("+u+")").addClass("crm-highlight-cell"),l.attr("disabled",!1)):g.find("td:nth-child("+u+")").removeClass("crm-highlight-cell")};c("select",
g).each(function(u){c(this).change(function(){n(u,c(this).val())})});var p=function(){c(this).is(":checked")?(g.find("tbody tr:first").removeClass("crm-ignored-row"),k.html((parseInt(k.html(),10)||0)+1)):(g.find("tbody tr:first").addClass("crm-ignored-row"),k.html(parseInt(k.html(),10)-1||""))};c("input[name='first_line']",f).change(p);var q=!1,r=g.find("select");g.find("tbody tr:first-child td").each(function(u,v){var t=null,w=0,x=!1;v=c(v).text().toLowerCase().replace(m,"");for(var y in h){var z=
...
c.get(c.crm.app_url+"?module=contactOperation&action=AddToSegments",function(e){c.waDialog({html:e,onOpen:function(f){new CRMContactsOperationAddToSegments({$wrapper:f,onSave:function(g,l){var k=l.data("dialog"),h=[],m=[];l=c(".c-segment-item",l);for(var n=0;n<g.segment_ids.length;n+=1){var p=g.segment_ids[n],q=l.filter('[data-id="'+p+'"]');h.push(q.find(".c-name").text());m.push('<input type="hidden" name="segment_id[]" value="'+p+'">')}h="<span>"+h.join(",")+"</span>";m=m.join(" ");c(".c-add-to-segments-names",
d).html(h+m);k.close();return!1}})}})}):c(".c-add-to-segments-names",d).html("")})};var b=function(d){this.$wrapper=d.$wrapper;this.$form=this.$wrapper.find("form");this.processId=null;this.requests=0;this.$loading=this.$wrapper.find(".crm-loading");this.timer=null;this.messages=d.messages||{};this.url=c.crm.app_url+"?module=contact&action=importProcess"+(d.need_validation?"&need_validation=1":"");this.$progress_bar_dialog=this.$wrapper.find(".crm-import-progressbar-dialog");this.$progress_bar=this.$progress_bar_dialog.find(".crm-progressbar")};
...
m.resolve()},onError:function(){m.resolve()}})},y=function(){n?setTimeout(y,1E3):z()},z=function(){var C=b.find(".crm-merge:not(.crm-finished):first");C.length?(C.parent().find(".crm-loading").css("opacity",1),m=new c.Deferred,m.fail(function(){k.hide();h.show()}),m.done(function(){u+=1;p(u,v);n?y():z()}),w(C)):u<v?t():B()},B=function(){l.hide();h.hide();k.hide();f.hide();a.$wrapper.find(".crm-attention-message").hide();a.$wrapper.find(".crm-done-message").show()};z()};l.click(function(r){r.preventDefault();
...
r.top+(0<q?0-q-2:2)};p<h.left+n&&(h.left=m-(n+10));return h}d=function(h){this.$wrapper=h.$wrapper;this.$hint=this.$wrapper.find(".c-hint-wrapper");this.node=this.$wrapper.find(".js-graph")[0];this.d3node=d3.select(this.node);this.show_class="is-shown";this.charts=h.data;this.data=g(this.charts);this.color=h.color;this.group_by=h.group_by;this.locales=h.locales;this.margin={top:14,right:10,bottom:20,left:34};var m=this.node;h=this.margin;var n=m.offsetWidth;m=m.offsetHeight;this.area={outer_width:n,
outer_height:m,inner_width:n-h.left-h.right,inner_height:m-h.top-h.bottom};this.column_indent=4;h=this.area.inner_width;n=this.data[0].length;m=null;n+=1;h&&n&&(m=(h-this.column_indent*(n-1))/n,0>m&&(m=0));this.column_width=m;this.axisIndent=2;this.yDomain=this.xDomain=this.svgArea=this.svgLine=this.y=this.x=this.defs=this.svg=!1;this.initGraph()};d.prototype.initGraph=function(){var h=this,m=h.area;h.initGraphCore();h.svg=h.d3node.append("svg").attr("width",m.outer_width).attr("height",m.outer_height);
h.renderBackground();f.each(h.data,function(n,p){n=h.data.length-1-n;h.renderChart(h.data[n],h.charts[n])});h.renderAxis()};d.prototype.initGraphCore=function(){var h=this,m=h.data,n=h.area,p=h.x=d3.time.scale().range([0,n.inner_width]),q=h.y=d3.scale.linear().range([n.inner_height,0]);h.yDomain=function(){var r=d3.min(m,function(v){return d3.min(v,function(t){return t.value})});0<r&&(r=0);var u=d3.max(m,function(v){return d3.max(v,function(t){return t.value+t.y0})});return[r,u]}();h.xDomain=function(){var r=
...
this.$wrapper.find(".js-stage-graph-section"),d=a.stages;if(!b.length||!d.length)return!1;var e=[0,2*d.length],f=function(l){f=function(k){this.$wrapper=k.$wrapper;this.$hint=this.$wrapper.find(".c-hint-wrapper");this.node=this.$wrapper.find(".js-graph")[0];this.d3node=d3.select(this.node);this.show_class="is-shown";console.log(k.data);for(var h=k.data,m=0;m<h.length;m++){var n=h[m];1!==m||n.color||(n.color="#e07d6e")}h=this.charts=h;m=[];for(n=0;n<h.length;n++){for(var p=h[n].data,q=[],r=0;r<p.length;r++){var u=
...
typeof d.options.clear&&d.options.clear.apply(this))});this.input.bind("autocompleteopen",function(g,l){var k=d.input.width();d.ul.find("li.ui-menu-item").each(function(){var h=c(this),m=h.find(".count"),n=h.find(".crm-text");m.insertBefore(n);n.width(Math.max(k-m.outerWidth(!0)-30,0)).data("sep")&&h.css({borderTop:"0px solid #ccc"})});d.ul.width(k)});_other_version_||(this.input.bind("autocompleteopen",function(g,l){d.ul.width(d.input.outerWidth(!0)-5);void 0!==d.offset&&void 0!==d.count&&(d.offset<
d.count?(d.height=d.ul.height(),d.ul.unbind("scroll."+d.ns).bind("scroll."+d.ns,function(){var m=c(this).find(".dummy-end");m.length?c(this).scrollTop()+d.height>=m.position().top&&d.input.trigger("autocompleteload"):d.ul.unbind("scroll."+d.ns)}),g=d.ul.find(".ui-menu-item:first").height(),c('<li class="dummy-end ignore-hover" role="menuitem"><a class="ui-corner-all" style="padding: .2em .4em; line-height: 1.5; display: block;"></a></li>').height(g).appendTo(d.ul),d.ul.css({overflowY:"scroll",overflowX:"hidden",
height:d.height})):(d.ul.unbind("scroll."+d.ns),d.ul.css({overflowY:"",overflowX:""})));d.ul.find(".sep").closest("li").addClass("ignore-hover");var k=d.ul.width(),h="scroll"===d.ul.css("overflowY");d.ul.find("li.ui-menu-item").each(function(){var m=c(this);m.find(".sep").length?m.find(".sep").width(h?k-25:k-10):m.find(".crm-text").width(Math.max(k-m.find(".count").outerWidth(!0)-30,0))})}),this.input.bind("autocompleteload",function(g){!d.loading&&d.offset<d.count&&(d.loading=!0,d.input.addClass("ui-autocomplete-loading"),
c.get(d.options.url,{id:d.ns,offset:d.offset,limit:d.limit,term:d.input.val()},function(l){if("ok"===l.status){var k=l.data.values,h=[];d.ul.find(".ui-menu-item").each(function(){h.push(c(this).data("item.autocomplete"))});for(var m=0;m<k.length;m+=1)h.push(d._formItem(k[m]));d.count=l.data.count;d.response(h);d.offset+=k.length;d.offset>=d.count&&d.ul.find(".dummy-end").remove()}d.loading=!1;d.input.removeClass("ui-autocomplete-loading")},"json"))}));this.input.bind("autocompletefocus",function(g,
...
onOpen:function(m,n){m.find(".datepicker").datepicker();l.start_datetime&&m.find(".datepicker.start").datepicker("setDate",c.datepicker.parseDate("yy-mm-dd",l.start_datetime));l.end_datetime&&m.find(".datepicker.end").datepicker("setDate",c.datepicker.parseDate("yy-mm-dd",l.end_datetime));var p=m.find(".js-apply-action"),q=m.find(".js-cancel-action");p.click(function(r){r.preventDefault();r=m.find(".start").datepicker("getDate");var u=m.find(".end").datepicker("getDate");m.trigger("select",[r,u]);
n.close()});q.click(function(r){r.preventDefault();m.trigger("cancel");n.close()});n.resize()}});this.data("$dialog",c("#"+h));return this.data("$dialog")}.call(this)}})(jQuery);var CRMSettings=function(c){CRMSettings=function(a){this.$wrapper=a.$wrapper;this.$sidebar=a.$sidebar;this.$activeMenuItem=!1;this.initClass()};CRMSettings.prototype.initClass=function(){this.initMenu();this.initToggleSidebar()};CRMSettings.prototype.initToggleSidebar=function(){function a(f,g){0>g.content_uri.indexOf("/settings/")&&c("body").removeClass("noscroll");c(document).off("wa_before_load",a);e.off("click")}var b=this,d=b.$wrapper.find(".sidebar:first"),e=d.find(".js-expand-sidebar");e.length&&
...
b.logo=h}}var b=this,d=!1,e=!1,f=b.$logoWrapper,g=f.find(".js-drop-area"),l=g.find(".js-field"),k=f.find(".js-image");g.on("drop",function(h){h.stopPropagation();h=h.originalEvent.dataTransfer.files;h.length&&(a(h),b.toggleButton(!0))});l.on("change",function(){var h=this.files;h.length&&(a(h),b.toggleButton(!0))});f.on("click",".js-delete-logo",function(h){h.preventDefault();k.attr("src","");if(h=l.attr("name")){var m=c('<input type="hidden" />').attr("name",h).val("delete");l.attr("name","").data("name",
h).after(m)}f.addClass(b.empty_class);b.logo=!1;b.toggleButton(!0)});c(document).on("myDragEnter",function(){c.contains(document,g[0])?g.addClass("is-highlighted"):c(document).off("myDragEnter",b.watcher)});c(document).on("myDragLeave",function(){c.contains(document,g[0])?g.removeClass("is-highlighted"):c(document).off("myDragLeave",b.watcher)});g.on("dragover",function(h){h.preventDefault();d?clearTimeout(d):e||(e=!0,g.addClass("is-hover"));d=setTimeout(function(){d=null;e=!1;g.removeClass("is-hover")},
100)})};CRMSettingsCompanies.prototype.initSubmit=function(){var a=this,b=a.$wrapper.find(".js-form"),d=a.$wrapper.find(".js-errors-place"),e=a.$taxList,f=!1;b.on("submit",function(g){function l(){e.find(".c-tax-option").each(function(m){var n=c(this),p=n.find(".js-type");n=n.find(".js-percent");p.attr("name","company[tax_options]["+m+"][tax_type]");n.attr("name","company[tax_options]["+m+"][tax_percent]")});return b.serializeArray()}function k(m){m=m?m:[];c.each(m,function(n,p){n=p.name;p=p.value;
var q=a.$wrapper.find('[name="'+n+'"]');q.removeClass("error");q.parent().find("span.errormsg").remove();"contact[firstname]"!==n||q.is(":visible")||(q=a.$wrapper.find(".js-contact-autocomplete"));var r=c("<span class='c-error' />").addClass("errormsg").text(p);q.length&&!q.hasClass("error")?(q.parent().append(r),q.addClass("error").one("focus click change",function(){q.removeClass("error");r.remove()})):(d.append(r),b.one("submit",function(){remove(r)}))})}g.preventDefault();if(!f){f=!0;g=function(m){var n=
new FormData;c.each(m,function(p,q){n.append(q.name,q.value)});c.each(a.files,function(p,q){n.append("images["+q.code+"]",q.file)});a.logo&&n.append("logo",a.logo);(m=(m=document.cookie.match(/(?:^|; )_csrf=([^;]*)/))?decodeURIComponent(m[1]):"")&&n.append("_csrf",m);return n}(l());var h=c(a.locales.loading);h.insertAfter(a.$submitButton);c.ajax({url:"?module=settings&action=companiesSave",data:g,dataType:"json",processData:!1,contentType:!1,type:"POST",success:function(m){"ok"===m.status?(c(a.locales.saved_html).insertAfter(a.$submitButton),
a.company_id?c.crm.content.reload():c.crm.content.load(c.crm.app_url+"settings/companies/"+m.data.id+"/")):m.errors&&k(m.errors)}}).always(function(){h.remove();f=!1})}});b.on("keydown keypress keyup","input",function(g){13===g.keyCode&&g.preventDefault()})};CRMSettingsCompanies.prototype.initDeleteCompany=function(){var a=this,b=!1;a.$wrapper.on("click",".js-company-delete",function(d){d.preventDefault();b||(b=!0,c.post("?module=settings&action=companyDeleteDialog",{id:a.company_id},function(e){c.waDialog({html:e})}).always(function(){b=
!1}))})};CRMSettingsCompanies.prototype.toggleButton=function(a){var b=this.$submitButton,d=this.$footer.find(".js-edit-actions");a?(b.addClass("yellow"),d.show()):(b.removeClass("yellow"),d.hide())};CRMSettingsCompanies.prototype.initTemplatesSection=function(){function a(m,n){xhr&&xhr.abort();xhr=c.post("?module=settings&action=companiesRenderParams",{company_id:n,template_id:m},function(p){g.html(p);g.find(".c-color-section").each(function(){b(c(this))});g.find(".js-image-section").each(function(){d(c(this))})}).always(function(){xhr=
!1})}function b(m){var n=m.find(".c-colors"),p=m.find(".js-color-field"),q=n.find(".is-active"),r=function(v){r=function(t){this.$wrapper=t.$wrapper;this.$field=p;this.$icon=this.$wrapper.find(".js-toggle");this.$colorPicker=this.$wrapper.find(".js-color-picker");this.farbtastic=this.is_opened=!1;this.initClass()};r.prototype.initClass=function(){function t(){q.length&&(q.removeClass("is-active"),q=!1)}var w=this;w.farbtastic=v.farbtastic(w.$colorPicker,function(x){w.$field.val()!==x&&(t(),w.$field.val(x).change())});
...
handle:".js-sort-toggle",helper:"clone",items:"> .c-currency",axis:"y",stop:a,onUpdate:a})};CRMSettingsCurrencies.prototype.initShopToggle=function(){function a(){k.set(!1)}function b(){var h=c.waDialog({html:e.copy_shop_currencies_dialog_html}).$wrapper.find("form");h.on("submit",function(m){m.preventDefault();is_locked||(is_locked=!0,m=h.serializeArray(),c.post("?module=settings&action=currenciesShopCopy",m,function(){c.crm.content.reload()}).always(function(){is_locked=!1}))});h.on("click",".js-cancel-button",
a)}function d(h){is_locked||(is_locked=!0,c.post("?module=settings&action=currenciesShopCopy",h?{}:{disable:1},function(m){c.crm.content.reload()}).always(function(){is_locked=!1}))}var e=this,f=e.$wrapper.find(".js-shop-currency-toggle"),g=e.$wrapper.find("#shop-currency-switch");is_locked=!1;if(!f.length)return!1;var l=f.data("use-shop")||!1;g.waSwitch({change:function(h,m){is_locked||(h?l?c.waDialog.confirm({title:'<i class="fas fa-exclamation-triangle smaller state-error"></i> '+e.locales.confirm_shop_title,
...
function(){a.toggleButton(!0)})};CRMSettingsFunnel.prototype.initColorSection=function(){function a(h){var m=g.find(".c-stage .c-ornament .svg-icon"),n=m.length;if(4===h.length||7===h.length){var p=(new c.crm.color(h)).getRange();b.val(p[0]);d.val(p[1]);m.each(function(q){q=p.getColor(Math.floor(100*(q+1)/n));var r=c(this),u=r.find("svg");u.length?u.find("polygon").css("fill",q):r.data("color",q)})}}var b=this.$wrapper.find(".js-start-color-field"),d=this.$wrapper.find(".js-end-color-field"),e=this.$wrapper.find(".c-color-section .c-colors"),
...
(u=q*r);n.val(u)}function h(q){q?(l.addClass("is-extended"),m.focus()):(l.removeClass("is-extended"),m.val(""),n.val(""),b.toggleButton(!0))}var m=l.find(".js-visible-time-field"),n=l.find(".js-hidden-time-field"),p=l.find(".js-time-period-select");l.on("click",".js-show-time",function(){h(!0)});l.on("click",".js-remove-limit",function(){h(!1)});m.on("change",k);p.on("change",k);m.on("keyup",function(){var q=c(this),r=q.val();r=parseInt(r.replace(",","").replace(".",""));isNaN(r)?q.val(""):r?q.val(r):
q.val("")});(function(){var q=n.val();q&&0<parseInt(q)&&(0===parseInt(q)%24?(p.val("24"),m.val(parseInt(q)/24)):p.val("1"))})()}f.find(".js-time-limit-wrapper").each(function(){g(c(this))})}var b=this,d=b.$stagesSection,e=b.$stages;e.on("click",".js-delete-stage",function(f){f.preventDefault();1<b.$stages.find("li").length&&c(this).closest(".c-stage").remove();b.toggleButton(!0)});e.on("keydown keypress keyup","input",function(f){13===f.keyCode&&f.preventDefault()});d.on("click",".js-add-stage",function(f){f.preventDefault();
f=c(b.stage_html);b.$stages.append(f);a(f);b.toggleButton(!0)});e.sortable({distance:10,handle:".js-sort-toggle",helper:"clone",items:"> li",axis:"y",start:function(f,g){},stop:function(f,g){b.toggleButton(!0)}});a(e)};CRMSettingsFunnel.prototype.initSave=function(){function a(){var k=d.$form.serializeArray();d.$stages.find("li").each(function(h){var m=c(this),n=m.data("id");k.push({name:"stages["+h+"][name]",value:m.find(".js-name-field").val()});n&&k.push({name:"stages["+h+"][id]",value:n})});return k}
...
location.href=c.crm.app_url+"settings/field/")})})};a.prototype.save=function(){var b=this,d=b.$form,e=c(".crm-dialog-edit-field-wrapper").find(".js-save"),f=c('<span class="icon loading" style="vertical-align: middle;margin-left: 10px;"><i class="fas fa-spinner fa-spin"></i></span>');c(".loading").remove();e.prop("disabled",!0);var g=!0;d.find(".errormsg").text("");d.find(".state-error").removeClass("state-error");c('[name$="[localized_names]"]').each(function(){var l=c(this),k=l.val();if("string"===
typeof k||k instanceof String)k=k.trim();!k&&0>=l.parents(".template").length&&l.closest("tr").find('[name$="[_disabled]"]:checked').length&&(g=!1,l.addClass("state-error").parent().append(c('<span class="errormsg"></span>').text(b.locales.field_is_required)))});if(!g)return e.attr("disabled",!1),d.one("input, keydown",".state-error",function(){c(this).removeClass("state-error").nextAll(".state-error-hint:first, .errormsg:first").empty()}),!1;f.appendTo(e.parent());return c.post(d.attr("action"),
d.serialize(),function(l){"ok"==l.status&&(c(".loading").remove(),c('<i class="fas fa-check-circle" style="vertical-align: middle;margin-left: 10px;"></i>').appendTo(e.parent()),setTimeout(function(){c.crm.content.reload();b.dialog.close()},1E3));if("ok"!==l.status&&l.errors){e.removeProp("disabled");c(".loading").remove();for(var k=0,h=l.errors.length;k<h;k+=1){var m=l.errors[k];if("string"===typeof m)d.find(".errormsg.crm-common-errors").append(m);else if("object"===typeof m)for(var n in m)if(m.hasOwnProperty(n)){var p=
d.find('[data-error-id="'+n+'"]');p.addClass("state-error");p.nextAll(".state-error-hint:first").text(m[n])}}d.one("input, keydown",".state-error",function(){c(this).removeClass("state-error").nextAll(".state-error-hint:first, .errormsg:first").empty()});d.find("[type=submit]").attr("disabled",!1)}})};a.prototype.initIDAutoFiller=function(){var b,d=this.$form,e=d.find('input[name^="name["][data-main-locale]'),f=d.find('input[name="id_val"]'),g=null;f.on("keydown.check_edited",function(){var l=c(this);
l.data("val",l.val())}).on("keyup.check_edited",function(){var l=c(this);l.val()&&l.val()!=l.data("value")&&(l.off(".check_edited"),l.data("edited",1))});if(!f.prop("disabled")&&!f.data("edited"))d.on("keydown.crm-id-auto-filler",'input[name^="name["]',function(){var l=c(this),k=d.find('[type="submit"]'),h=f.next(".loading");if(l.data("main-locale")||!e.val())f.prop("disabled")||f.data("edited")?d.off(".crm-id-auto-filler"):(k.prop("disabled",!0),h=h.length?h:c('<i class="icon16 loading"></i>'),h.insertAfter(f),
b&&clearTimeout(b),b=setTimeout(function(){var m=function(){g&&(g.abort(),g=null);b&&clearTimeout(b);k.prop("disabled",!1);h.remove()};f.data("edited")?m():g=c.post(c.crm.app_url+"?module=settings&action=fieldTransliterate",d.find('input[name^="name["]').serialize(),function(n){m();"ok"!==n.status||f.data("edited")||f.val(n.data)},"json")},300))})};a.prototype.editSubFields=function(){function b(l){var k=c(".crm-dialog-edit-field-wrapper").find(".js-save");l?k.addClass("yellow").removeAttr("disabled"):
k.removeClass("yellow")}var d=this,e=c(".crm-dialog-edit-field-wrapper"),f=e.find(".subfields-list > .ui-sortable"),g=1;f.sortable({items:".field-row",handle:".js-subfield-sort",axis:"y",update:function(l){b(!0)}});f.on("click","a.js-add-subfield",function(){var l=f.find(".field-row.template");l=l.clone().insertBefore(l).removeClass("template").removeClass("hidden");d.dialog.resize();var k="__"+g;g++;l.find("[name]").each(function(){var h=c(this);h.attr("name",h.attr("name").replace(/%FID%/g,k))});
l.data("fieldId",k);l.find("select.type-selector").change();b(!0);return!1});e.on("click",".js-edit-subfield",function(l){l.preventDefault();c(this).closest("tr").addClass("editor-on").removeClass("editor-off");b(!0)});e.on("click",".js-delete-subfield",function(l){l.preventDefault();var k=c(this).closest("tr");k.hasClass("just-added")&&k.remove();c.crm.confirm.show({title:d.locales.delete_subfield_title,text:d.locales.delete_subfield_text,button:d.locales.delete_subfield_button,onConfirm:function(){k.addClass("editor-off").removeClass("editor-on");
var h=k.find('input:hidden[name$="[_disabled]"]').attr("name").replace("[_disabled]","[_deleted]");c(".js-field-form-edit").append(c('<input type="hidden" name="" value="1">').attr("name",h));k.children().children(":not(label)").remove();k.find("label").addClass("gray").addClass("strike");b(!0)}})});f.on("click","a.add-item",function(){d.dialog.resize()});e.on("change","select.type-selector",function(){var l=c(this),k=l.closest("tr"),h=k.closest("table"),m=k.find(".field-advanced-settings").html('<span class="icon"><i class="fas fa-spinner fa-spin"></i></span>');
...
b){c(b).waSwitch({ready:function(d){var e=d.$wrapper.siblings("label"),f=d.$wrapper.siblings("input"),g=d.$wrapper.find("input");d.$label=e;d.$input=g;d.$input_hidden=f;d.active_text=e.data("active-text");d.inactive_text=e.data("inactive-text")},change:function(d,e){var f=e.$wrapper.closest(".fields-group").find(".params");d?(f.slideDown(),e.$input_hidden.val(1),e.$input.val(1),e.$label.text(e.active_text)):(f.slideUp(),e.$input_hidden.val(0),e.$input.val(0),e.$label.text(e.inactive_text))}})})};
...
a.find(".crm-errors-block").remove()};CRMSettingsSourceIm.prototype.initSubmit=function(){var a=this,b=a.$form,d=a.$wrapper.find(".crm-form-buttons").find(".crm-loading"),e=a.$button,f=b.find(".crm-success-status"),g=c.crm.app_url+"?module=settingsSource&action=save";CRMSettingsSourceIm.clearValidateErrors(b);b.submit(function(l){l.preventDefault();l=b.serializeArray();l.push({name:"id",value:0<a.source.id?a.source.id:a.source.provider});d.show();e.prop("disabled",!0);a.submit_xhr&&a.submit_xhr.abort();
a.submit_xhr=c.post(g,l,null,"json").done(function(k){"ok"===k.status?(a.setFormChanged(!1),f.show().fadeOut(500,function(){k.data.source?c.crm.content.load(c.crm.app_url+"settings/sources/"+k.data.source.id+"/"):c.crm.content.reload()})):CRMSettingsSourceIm.showValidateErrors(b,k.errors||{})}).always(function(){d.hide();e.prop("disabled",!1);a.submit_xhr=null})})};CRMSettingsSourceIm.prototype.initDeleteLink=function(){var a=this;a.$wrapper.find(".crm-delete-source-link").click(function(b){b.preventDefault();
...
f.eq(0).removeClass("gray");f.eq(1).hide()},change:function(e,f){e?f.$label.text(f.active_text):f.$label.text(f.inactive_text)}})})};CRMSettingsSourceIm.clearValidateErrors=function(a){a.find(".state-error").removeClass("state-error");a.find(".crm-errors-block").remove()};CRMSettingsSourceIm.showValidateErrors=function(a,b){var d={};c.each(b||{},function(e,f){"params"===e?c.each(f,function(g,l){d["source[params]["+g+"]"]=c.isArray(l)?l:[l]}):d[e?"source["+e+"]":""]=c.isArray(f)?f:[f]});CRMSettingsSourceIm.clearValidateErrors(a);
c.each(d,function(e,f){var g=c('<div class="crm-errors-block"></div>');e=e?a.find('[name="'+e+'"]'):c();e.addClass("state-error");c.each(f,function(l,k){g.append('<span class="errormsg error">'+k+"</span>")});e.length?e.after(g):a.find(".crm-common-errors-block").append(g)})};return CRMSettingsSourceIm}(jQuery);var CRMSettingsSourceWithContactBlock=function(c){CRMSettingsSourceWithContactBlock=function(a){this.$wrapper=a.$wrapper;this.segments_list_stages=a.segments_list_stages||{};this.init()};CRMSettingsSourceWithContactBlock.prototype.init=function(){this.initAddToSegments()};CRMSettingsSourceWithContactBlock.prototype.initAddToSegments=function(){var a=this.$wrapper,b=this.segments_list_stages,d=a.find(".c-segments-ul-fold-link"),e=a.find(".c-segments-ul");d.click(function(){var f=d.data("state");f=
...
this.source=a.source||{};this.funnel_id=this.source.funnel_id;this.namespace=a.namespace||"";this.class_id=a.class_id||"";this.initClass();this.$wrapper.data("block_object",this)};CRMSettingsSourceResponsibleBlock.prototype.initClass=function(){var a=this,b=a.$wrapper,d=a.$group,e=a.$user,f=a.$user_id,g=a.$user_delete_link,l=a.$user_info,k=f.val(),h=function(){e.show().attr("disabled",!1).attr("required",!0)},m=function(){e.hide().attr("disabled",!0).attr("required",!1)};d.on("change"+a.ns,function(){var n=
c(this),p=n.parents(".crm-fields").find(".js-group-hint");f.val("");"personally"===n.val()?h():(m(),l.hide());"personally"===n.val()||""===n.val()?p.hide():p.show();0<n.val()&&f.val(-n.val());k!=f.val()&&(b.trigger("changeResponsibleUser",[k,f.val()]),k=f.val())});g.on("click"+a.ns,function(n){n.preventDefault();f.val("");h();l.hide()});e.autocomplete({source:c.crm.app_url+"?module=autocomplete&type=user&funnel_id="+a.funnel_id,minLength:0,html:!0,focus:function(){return!1},select:function(n,p){e.val("");
if(0>=p.item.rights)return!1;m();l.show();l.find(".crm-user-icon").css("background-image","url("+p.item.photo_url+")");l.find(".crm-user-name").text(p.item.name).attr("title",p.item.name);f.val(p.item.id);k!=f.val()&&(b.trigger("changeResponsibleUser",[k,f.val()]),k=f.val());return!1}}).data("ui-autocomplete")._renderItem=function(n,p){var q=c("<li />");q.addClass("ui-menu-item-html").append("<div>"+p.value+"</div>").appendTo(n);0<a.funnel_id&&!p.rights&&(q.addClass("is-locked"),q.on("click"+a.ns,
...
a||(a=this.getField("!deal_description"));!a||0>=a.checked||!b.length||(a.redactor?b.data("redactor")?(a=b.redactor("core.editor"))&&0<a.length&&a[0].attr("placeholder",b.attr("placeholder")):c.crm.initWYSIWYG(b,{focus:!1,buttons:["bold","italic","underline","link"],plugins:[],maxHeight:100,minHeight:100}):(b.data("redactor")&&b.redactor("core.destroy"),b.show()))};crmSettingsForm.prototype.bindEvents=function(){var a=this;a.$form.on("change","input,textarea,select",function(b){a.setFormChanged();
b.isTrigger||a.clearValidateErrors()});a.$form.on("keyup","input:text,textarea",function(b){a.setFormChanged();b.isTrigger||a.clearValidateErrors()});a.$form.find(".crm-iframe-code-block-toggle").click(function(){a.$iframe_textarea.toggle()})};crmSettingsForm.prototype.initDealCreateCheckbox=function(){var a=this,b=a.referrer,d=function(e){var f=a.$wrapper.find(".js-responsible-section").data("block_object");e?(a.$available_deal_related_field.removeClass("crm-disabled"),a.$wrapper.find(".js-hint-about-deal-fields").hide(),
...
function(e,f){d(f,!0)});0>=a.form.id&&"settings/sources"===b&&(a.$create_deal_block_wrapper.find(":checkbox").attr("checked",!0).trigger("change"),d(!0,!1))};crmSettingsForm.prototype.setFormChanged=function(a){(a=void 0!==a?a:!0)?this.$button.addClass("yellow"):this.$button.removeClass("yellow");this.changed=a};crmSettingsForm.prototype.initPreviewButton=function(){var a=this.$wrapper,b=a.find(".crm-form-preview-submit-button-caption-input"),d=a.find(".crm-form-preview-submit-button"),e=function(f){f?
(b.show(),d.hide(),b.val(d.val())):(b.hide(),d.show())};d.click(function(){e(!0)});b.blur(function(){d.val(b.val());e(!1)}).keyup(function(f){f.preventDefault();13==f.keyCode?(d.val(b.val()),e(!1)):27==f.keyCode&&e(!1)})};crmSettingsForm.prototype.initFormWidthInput=function(){var a=this.$wrapper,b=a.find(".crm-form-width-input"),d=a.find(".crm-form-preview-block");b.keyup(function(e){13==e.keyCode&&(b.val(Math.max(Math.min(parseInt(b.val(),10)||0,600),200)),d.width(b.val()))})};crmSettingsForm.prototype.initTextEditor=
function(){var a=this,b=a.$form.find('[name="form[params][confirm_mail_body]"]');b.waEditor({focus:!1,buttons:["formatting","bold","italic","link"],plugins:["fontcolor","fontsize","fontfamily"],minHeight:200,callbacks:{keydown:function(d){},change:function(){b.waEditor("sync");a.setFormChanged()}},lang:a.lang})};crmSettingsForm.prototype.initMessagesBlock=function(){var a=this,b=a.$wrapper,d=b.find(".c-settings-messages-block");b=b.find(".js-deal-section");a.renderToVariantSelectors();d.on("loadEditor",
...
a.find(".c-message-to-selector").each(function(){var f=c(this),g=f.find(".c-label");f=f.find(".c-message-to-variants-list");var l=f.find(':checkbox[data-id="client"]');e(l,!d);var k=[];f.find(":checkbox:checked:not(:disabled)").each(function(){var h=c(this).closest("li");k.push(h.find(".c-variant-name-text").text())});k=0<k.length?k.join(", "):g.data("initial_text");g.text(k)})};crmSettingsForm.prototype.getField=function(a,b){var d=this.form.params.fields,e=d.length,f=null;b="undefined"===typeof b?
1:parseInt(b,10)||0;if(0>=b)return null;for(var g=0;g<e;g+=1)if(d[g].id==a&&d[g].checked==b){f=d[g];break}if(!f)return f;f.required=f.required||"";f.captionplace=f.captionplace||"left";f.caption=f.caption||"string"===typeof f.caption?f.caption:f.name;f.placeholder=f.placeholder||"";return f};crmSettingsForm.prototype.getAvailableField=function(a){if(this.available_fields[a]){a=c.extend({},this.available_fields[a]);if("!horizontal_rule"===a.id||"!paragraph"===a.id)a.captionplace="none";return a}return null};
crmSettingsForm.prototype.updateAvailableField=function(a,b){this.available_fields[a]&&c.extend(this.available_fields[a],b)};crmSettingsForm.prototype.addField=function(a,b,d){b=parseInt(b,10)||0;0>=b||!d||(this.deleteField(a,b),d=c.extend({},d,{id:a,checked:b}),this.form.params.fields.push(d))};crmSettingsForm.prototype.deleteField=function(a,b){var d=this.form.params.fields,e=d.length;b=parseInt(b,10)||0;for(var f=[],g=0;g<e;g+=1)d[g].id==a&&d[g].checked==b||f.push(d[g]);b=0;e=f.length;for(g=0;g<
e;g+=1)f[g].id==a&&(b+=1,f[g].checked=b);this.form.params.fields=f};crmSettingsForm.prototype.updateField=function(a,b,d){0>=b||(b=this.getField(a,b),b=c.extend(b,d),this.form.params.fields[a]=b,this.setFormChanged())};crmSettingsForm.prototype.renderField=function(a,b){var d=this.getField(a,b),e=this.$form_fields.find('.crm-form-field:not(.crm-form-field-template)[data-id="'+a+'"][data-checked="'+b+'"]');if(d)if(e.length||(e=this.$form_fields.find('.crm-form-field-template.crm-form-field-template[data-id="'+
a+'"]').clone(),e.removeClass("crm-form-field-template"),e.attr("data-checked",b),e.insertBefore(this.$form_fields.find(".crm-form-field-template:first")),e.show()),"!paragraph"===d.id)e.find(".crm-paragraph").html(d.text||"");else{a=e.find(".crm-caption");b=a.html;var f=(f=d.caption)&&(""+f).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");b.call(a,"<label>"+f+(d.required&&"!captcha"!==d.id?" *":"")+"</label>");e.removeClass("crm-caption-style-none crm-caption-style-above").addClass("crm-caption-style-"+
d.captionplace);e.find(":input:eq(0)").prop("placeholder",d.placeholder||"");"password"===d.id&&e.find(":input:eq(1)").prop("placeholder",d.placeholder_confirm||"");"!deal_description"===d.id&&this.renderDealDescription(d)}else e.remove()};crmSettingsForm.prototype.initFields=function(){var a=this;a.$form_fields.on("click",".crm-caption-col, .crm-input-col",function(b){a.editField(c(this).closest(".crm-form-field"))})};crmSettingsForm.prototype.editField=function(a){var b=a.data("id");a=a.data("checked");
if("!horizontal_rule"!==b){var d={html:"",id:b,checked:a,settingsForm:this};a="!agreement_checkbox"===b?this.$agreement_checkbox_edit_dialog_tempalte.clone():"!paragraph"===b?this.$paragraph_edit_dialog_template.clone():this.$edit_dialog_template.clone();a.removeAttr("data-template").attr("id","crm-settings-dialog-wrapper-"+b);a.appendTo("body");a.show();d.html=a;"!agreement_checkbox"===b?new crmSettingsFormAgreementCheckboxEditDialog(d):"!paragraph"===b?new crmSettingsFormParagraphEditDialog(d):
new crmSettingsFormFieldEditDialog(d)}};crmSettingsForm.prototype.initSubmit=function(){var a=this,b=a.$form,d=b.find(".crm-loading"),e=b.find(".crm-success-status");b.on("keypress","input",function(f){13==f.keyCode&&f.preventDefault()});b.submit(function(f){f.preventDefault();f=b.serializeArray();var g=[];c.each(a.form.params.fields,function(l,k){if(0<k.checked){var h=g.length;c.each(k,function(m,n){"html"!==m&&(g[h]=g[l]||{},g[h][m]=n)})}});f.push({name:"form[params][fields]",value:JSON.stringify(g)});
a.clearValidateErrors();d.show();a.$button.prop("disabled",!0);a.submit_xhr&&a.submit_xhr.abort();a.submit_xhr=c.post(b.attr("action"),f).done(function(l){"ok"!==l.status?(a.showValidateErrors(l.errors||{}),d.hide(),a.$button.prop("disabled",!1)):(e.show().fadeOut(500),c.crm.content.load(c.crm.app_url+"settings/form/"+l.data.form.id).then(function(){d.hide();a.$button.prop("disabled",!1);a.setFormChanged(!1)}))}).fail(function(){a.showValidateErrors({"":a.messages.unknown_server_error});d.hide();
a.$button.prop("disabled",!1)}).always(function(){})})};crmSettingsForm.prototype.clearValidateErrors=function(){var a=this.$form;a.find(".state-error").removeClass("state-error");a.find(".crm-errors-block").remove()};crmSettingsForm.prototype.showValidateErrors=function(a){var b=this.$form,d={};c.each(a||{},function(e,f){"params"===e?c.each(f,function(g,l){d["form[params]["+g+"]"]=c.isArray(l)?l:[l]}):d[e?"form["+e+"]":""]=c.isArray(f)?f:[f]});this.clearValidateErrors();c.each(d,function(e,f){var g=
c('<div class="crm-errors-block"></div>');e=e?b.find('[name="'+e+'"]'):c();e.addClass("state-error");c.each(f,function(l,k){g.append('<span class="errormsg">'+k+"</span>")});e.length?e.after(g):b.find(".crm-common-errors-block").append(g)})};crmSettingsForm.prototype.initDeleteLink=function(){var a=this,b=a.messages||{};a.$delete_link.click(function(d){d.preventDefault();c.crm.confirm.show({title:b.delete_confirm_title,text:b.delete_confirm_text,button:b.delete_confirm_button,onConfirm:function(){var e=
c(".crm-confirm-dialog"),f=e.find(".crm-loading").show(),g=e.find(".js-confirm-dialog").attr("disabled",!0);c.post(c.crm.app_url+"?module=settings&action=formDelete",{id:a.form.id}).always(function(){c.crm.content.load(c.crm.app_url+"settings/form");f.hide();g.attr("disabled",!1)});return!1}})})};crmSettingsForm.prototype.initSortable=function(){function a(){for(var d=b.form.params.fields,e=d.length,f={},g=[],l=b.$form_fields.find(".crm-form-field:not(.crm-form-field-template)"),k=0;k<e;k+=1)f[d[k].id+
"-"+d[k].checked]=k;l.each(function(){var h=c(this),m=h.data("id");h=h.data("checked");g.push(d[f[m+"-"+h]])});b.form.params.fields=g;b.setFormChanged()}var b=this;b.$form_fields.sortable({distance:5,helper:"clone",items:".crm-form-field:not(:hidden)",opacity:.75,handle:".sort",tolerance:"pointer",containment:b.$form_fields,stop:a,onUpdate:a})};crmSettingsForm.prototype.initDeleteFieldLinks=function(){var a=this;a.$wrapper.on("click",".crm-delete-field-link",function(){var b=c(this).closest(".crm-form-field");
...
a=b.getFormField("password"),b.deleteFormField(a),b.renderToVariantSelectors());"company"===d&&b.$form_available_fields.find(".crm-form-field[data-person-enabled=0][data-company-enabled=1]").each(function(){var g=c(this),l=g.data("id");l=b.getFormField(l);b.deleteFormField(l);g.addClass("crm-disabled")});b.setFormChanged()}};crmSettingsForm.prototype.getFormField=function(a,b){var d='.%clz%[data-id="%id%"]%extra%:not(.%not_clz%)';a=[["%clz%","crm-form-field"],["%id%",a],["%extra%",void 0!==b?'[data-id="'+
b+'"]':""],["%not_clz%","crm-form-field-template"]];b=0;for(var e=a.length;b<e;b+=1)d=d.replace(a[b][0],a[b][1]);return this.$form_fields.find(d)};crmSettingsForm.prototype.initAvailableFields=function(){var a=this,b=a.default_checked_fields,d=a.$form_available_fields;d.on("click",".crm-form-field:not(.crm-disabled)",function(e){a.addFormField(c(this))});0>=a.form.id&&0<b.length&&c.each(b,function(e,f){e=d.find('.crm-form-field[data-id="'+f+'"]:not(.crm-disabled)');a.addFormField(e)})};crmSettingsForm.prototype.addFormField=
function(a){if(!(0>=a.length)){var b=a.data("id"),d=this.getAvailableField(b);this.$form_available_fields.hide();var e=d.checked+1;d.is_multi||a.addClass("crm-disabled");this.getField(d.id,e)||this.addField(b,e,d);this.updateAvailableField(b,{checked:e});"email"===b&&(this.$confirmation_enable_text.hide(),this.enableIButton(this.$confirmation_checkbox),this.$confirmation_checkbox.attr("disabled",!1),(a=this.getAvailableField("password"))&&0>=a.checked&&this.$form_available_fields.find('.crm-form-field[data-id="password"]').removeClass("crm-disabled"));
"company"===b&&this.$form_available_fields.find(".crm-form-field.crm-disabled[data-person-enabled=0][data-company-enabled=1]").removeClass("crm-disabled");this.renderField(b,e);"!paragraph"===b&&(b=this.$form_fields.find('.crm-form-field[data-id="'+b+'"][data-checked="'+e+'"]'),this.editField(b));this.setFormChanged();this.renderToVariantSelectors()}};crmSettingsForm.prototype.initAddNewFieldLink=function(){function a(e){c.contains(document,d[0])?27===e.keyCode&&d.is(":visible")&&d.hide():c(document).off("keypress",
a)}function b(e){if(c.contains(document,d[0])){var f=c.contains(d[0],e.target);e=d[0]===e.target;!d.is(":visible")||e||f||d.hide()}else c(document).off("keypress",b)}var d=this.$form_available_fields;this.$wrapper.on("click",".crm-add-new-field-link",function(e){e.stopPropagation();d.toggle()});c(document).on("keydown",a).on("click",b)};crmSettingsForm.prototype.initIButtons=function(){var a=this;a.$wrapper.find("#js-ibutton").each(function(){if(c(this).data("iButtonInit"))return c(this);a.$switch=
c(this).waSwitch({ready:function(b){var d=b.$wrapper.siblings("label");b.$label=d;b.active_text=d.data("active-text");b.inactive_text=d.data("inactive-text")},change:function(b,d){b?(a.$switch.has(a.$confirmation_checkbox).length&&(a.$email_confirm_block.slideDown(300),a.$confirmation_checkbox.attr("checked",!0)),d.$label.text(d.active_text)):(a.$switch.has(a.$confirmation_checkbox).length&&(a.$email_confirm_block.slideUp(300),a.$confirmation_checkbox.attr("checked",!1)),d.$label.text(d.inactive_text))}})})};
crmSettingsForm.prototype.disableIButton=function(){var a=this.$switch.waSwitch("switch");a.set(!1);a.disable(!0)};crmSettingsForm.prototype.enableIButton=function(){this.$switch.waSwitch("switch").disable(!1)};crmSettingsForm.prototype.initCopySmartyHelper=function(){this.$copy.on("click",function(){var a=c(this),b=a.data("content"),d=c("<input>");c("body").append(d);d.val(b).select();document.execCommand("copy");d.remove();a.html('<i class="fas fa-check-circle"></i>');setTimeout(function(){a.html('<i class="fas fa-copy"></i>')},
2E3)})};crmSettingsForm.prototype.initIframeTextarea=function(){var a=this;a.$iframe_textarea.click(function(){a.$iframe_textarea.select()})};crmSettingsForm.prototype.initCancelLink=function(){var a=this.referrer;this.$wrapper.on("click",".js-c-cancel-link",function(b){b.preventDefault();"settings/sources"==a?(c.crm.storage.set("crm/settings/source_tab","form"),c.crm.content.load(c.crm.app_url+"settings/sources/")):c.crm.content.load(c.crm.app_url+"settings/form/")})};crmSettingsForm.prototype.initBlocks=
function(){var a=this.$wrapper,b=a.find(".js-deal-section");a=a.find(".js-responsible-section");b=b.data("block_object");a=a.data("block_object");b.setResponsibleBlock(a)};return crmSettingsForm}(jQuery);var crmSettingsFormFieldEditDialog=function(c){var a=function(b){var d=this,e=b.onClose;b.onClose=function(g,l){e&&e(g,l)};var f=b.onOpen;b.onOpen=function(g,l){f&&f(g,l);d.$wrapper=g;d.$form=d.$wrapper.find("form");d.$button=d.$form.find("[type=submit]");d.id=b.id;d.checked=b.checked;d.settingsForm=b.settingsForm;d.dialog=l;d.initClass()};c.waDialog(b)};a.prototype.initClass=function(){var b=this.dialog.$block,d=this.settingsForm.getField(this.id,this.checked);c.each(d,function(g,l){"id"===g?c(".crm-field-id",
...
c(".dialog").remove()},1E4))},"json").always(function(){b=!1}));return!1}})})};CRMNotificationEdit.prototype.initSender=function(){function a(){return k&&0<k.id?k.transport||"email":h.filter(":checked").first().val()}function b(){"email"===a()?(d("specified"===l.$sender.val()),g()):(f("specified"===l.$sms_sender.val()),e())}function d(m){var n=l.$sender_block.find(".js-specified-sender-name"),p=l.$sender_block.find(".js-specified-sender-email");l.$sender_block.show();l.$sender.attr("disabled",!1);
...
function(){function a(){var l={data:[],errors:[]},k=f.serializeArray();c.each(k,function(h,m){l.data.push(m)});return l}function b(l,k){k=k?k:[];if(l){var h=Object.keys(l);c.each(h,function(m,n){k.push({name:n,value:l[n]})})}c.each(k,function(m,n){m=n.value;var p=e.$form.find('[name="'+n.name+'"]');if(p.length&&!p.hasClass("error")){var q=c("<span />").addClass("errormsg").text(m);e.$wrapper.append(q);p.addClass("error").one("focus click change",function(){p.removeClass("error");q.remove()})}})}function d(l){if(!g){g=
!0;var k=c(e.locales.saving);e.$footer.append(k);c.post("?module=settings&action=notificationsEditSave",l,"json").done(function(h){if("ok"!==h.status)b(h.errors||{});else if(0>=e.notification_id)c.crm.content.load(c.crm.app_url+"settings/notifications/edit/"+h.data.notification.id+"/");else{var m=c(e.locales.saved);e.$footer.append(m);setTimeout(function(){c.contains(document,m[0])&&m.remove()},2E3);c(document).trigger("formIsSaved")}}).always(function(){k.remove();g=!1})}}var e=this,f=e.$form,g=
...
e.abort();f={vaults:b()};var k=c(d.locales.saving);k.appendTo(l);e=c.post("?module=settings&action=vaultsSave",f,function(h){if("ok"==h.status){var m=c(d.locales.saved);m.appendTo(l);setTimeout(function(){c.contains(document,m[0])&&m.remove()},2E3)}}).always(function(){k.remove();e=!1})}function b(){var f=[];d.$list.find(".c-vault-item").each(function(){var g=c(this).data("id");g&&f.push(g)});return f}var d=this,e=!1;d.$list.sortable({distance:10,handle:".js-sort-toggle",helper:"clone",items:"> .c-vault-item",
...
onSuccess:function(){a(e,function(){b.dialog.options.onDelete();b.dialog.close()})}})})};CRMVaultEdit.prototype.initSave=function(){function a(){var h={data:[],errors:[]},m=f.serializeArray();c.each(m,function(n,p){"data[name]"===p.name&&(l=p.value);"data[color]"===p.name&&(k=p.value);h.data.push(p)});return h}function b(h,m){m=m?m:[];if(h){var n=Object.keys(h);c.each(n,function(p,q){m.push({name:q,value:h[q]})})}c.each(m,function(p,q){p=q.value;var r=e.$form.find('[name="'+q.name+'"]');if(r.length&&
!r.hasClass("error")){var u=c("<span />").addClass("errormsg").text(p);e.$wrapper.append(u);r.addClass("error").one("focus click change",function(){r.removeClass("error");u.remove()})}})}function d(h){g||(g=!0,c.post("?module=settings&action=vaultDialogSave",h,function(m){"ok"===m.status?(console.log(e.dialog),e.dialog.options.onSave(m.data.id,l,k),e.dialog.close()):m.errors&&b(m.errors)},"json").always(function(){g=!1}))}var e=this,f=e.$form,g=!1,l="",k="#aaa";f.on("submit",function(h){h.preventDefault();
...
this.$wrapper.find(".js-footer-actions"),b=a.find(".js-submit-button");this.$wrapper.on("change keydown","input, textarea, select",function(){b.addClass("yellow");a.addClass("is-changed")})};CRMSettingsShopWorkflowPage.prototype.initSubmit=function(){function a(){var k={data:[],errors:[]},h=f.serializeArray();c.each(h,function(m,n){k.data.push(n)});return k}function b(k){k&&k[0]||(k=[]);c.each(k,function(h,m){h=m.value;var n=e.$wrapper.find('[name="'+m.name+'"]'),p=c("<span class='c-error' />").addClass("errormsg").text(h);
n.length&&!n.hasClass("error")?(n.parent().append(p),n.addClass("error").one("focus click change",function(){n.removeClass("error");p.remove()})):(g.append(p),f.one("submit",function(){p.remove()}))})}function d(k){if(!l){l=!0;var h=c.crm.app_url+"?module=settings&action=shopWorkflowSave",m=e.$form.find(".c-loading");m.show();c.post(h,k,function(n){"ok"===n.status?c.crm.content.reload():b(n.errors)},"json").always(function(){l=!1;m.hide()})}}var e=this,f=e.$form,g=e.$wrapper.find(".js-errors-place"),
...
keydownCallback:function(b){}});this.dialog.onClose=function(){var b=a.data("redactor");b&&"core"in b&&b.core.destroy()}};return CRMEmailPersonalSettingsDialog}(jQuery);var CRMWebSocket=function(c){CRMWebSocket=function(a){this.app_url=a.app_url||c&&c.crm&&c.crm.app_url;this.channel=a.channel;this.onmessage=a.onmessage;this.onerror=a.onerror;this.ws=null;this.initWS()};CRMWebSocket.prototype.initWS=function(){var a=this;c.get(a.app_url+"?module=ws&action=url&channel="+a.channel,function(b){if("ok"==b.status)console.log(b.data.ws_url),a.ws=new WebSocket(b.data.ws_url),a.ws.onclose=function(){console.log("WebSocket connection closed");a.initWS()},a.ws.onmessage=a.onmessage;
else if("function"===typeof a.onerror)a.onerror("WebSocket connection fail");else console.log("WebSocket connection fail")})};CRMWebSocket.prototype.sendMessage=function(a){try{return this.ws.send(a),!0}catch(b){if("function"===typeof this.onerror)this.onerror(b);else console.log(b);return!1}};CRMWebSocket.prototype.close=function(){this.ws&&(this.ws.onclose=function(){console.log("Close WebSocket")},this.ws.close())};return CRMWebSocket}(jQuery);
