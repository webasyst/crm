...
a.notification_event))return e.show(),!1;e.load(a.site_app_url+"?module=pages&action=help","app=crm&key=notification."+a.notification_event,function(){e.find("#wa-help-wa").remove();e.find("#wa-help-wa-content").remove();e.show();e.find("ul>li.no-tab>p.bold").hide();e.data("loaded."+a.notification_event,!0)});return!1});e.on("click","div.fields a.inline-link",function(b){b.preventDefault();b=d(this).find("i");var c=a.$emailBody;"sms"===a.transport&&(c=a.$smsBody);(c=c.data("wa_editor"))&&c.insert(d.trim(b.text()))})};
