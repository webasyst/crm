...
class crmSegmentIncludeMethod extends crmApiAbstractMethod
{
    ...
    public function execute()
    {
        ...
        if (!is_numeric($segment_id)) {
            throw new waAPIException('invalid_param', 'Invalid segment ID', 400);
        } elseif (
            $segment_id < 1
        ...
        if (!empty($contact_ids)) {
            $counters = $this->getSegmentModel()->addTo($segment_id, $contact_ids);
        }
