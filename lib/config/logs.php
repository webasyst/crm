<?php

return array(
    'contact_add' => array(
        'name' => /*_w*/('added a new contact'),
        'subject' => true,
    ),
    'contact_delete' => array(
        'name' => /*_w*/('deleted contact')
    ),
    'contact_edit' => array(
        'name' => /*_w*/('edited contact'),
        'subject' => true,
    ),
    'contact_merge' => array(
        'name' => /*_w*/('merged contacts')
    ),
    'deal_add' => array(
        'name' => /*_w*/('added deal'),
    ),
    'deal_step' => array(
        'name' => /*_w*/('changed stage'),
    ),
    'deal_addcontact' => array(
        'name' => /*_w*/('added contact to a deal'),
    ),
    'deal_removecontact' => array(
        'name' => /*_w*/('excluded deal participant'),
    ),
    'deal_removeowner' => array(
        'name' => /*_w*/('remove deal owner'),
    ),
    'deal_reopen' => array(
        'name' => /*_w*/('reopened deal'),
    ),
    'deal_won' => array(
        'name' => /*_w*/('closed deal as won'),
    ),
    'deal_lost' => array(
        'name' => /*_w*/('closed deal as lost'),
    ),
    'deal_edit' => array(
        'name' => /*_w*/('edited deal'),
    ),
    'deal_contact_change' => array(
        'name' => /*_w*/('changed deal contact'),
    ),
    'deal_transfer' => array(
        'name' => /*_w*/('changed deal owner'),
    ),
    'reminder_add' => array(
        'name' => /*_w*/('added reminder'),
    ),
    'reminder_update' => array(
        'name' => /*_w*/('updated reminder'),
    ),
    'reminder_delete' => array(
        'name' => /*_w*/('deleted reminder'),
    ),
    'reminder_done' => array(
        'name' => /*_w*/('marked reminder as done'),
    ),
    'reminder_undone' => array(
        'name' => /*_w*/('marked reminder as active'),
    ),
    'note_add' => array(
        'name' => /*_w*/('added note'),
    ),
    'note_edit' => array(
        'name' => /*_w*/('edited note'),
    ),
    'note_delete' => array(
        'name' => /*_w*/('deleted note'),
    ),
    'file_add' => array(
        'name' => /*_w*/('added file'),
    ),
    'file_delete' => array(
        'name' => /*_w*/('deleted file'),
    ),
    'funnel_add' => array(
        'name' => /*_w*/('added funnel'),
    ),
    'funnel_edit' => array(
        'name' => /*_w*/('edited funnel'),
    ),
    'invoice_add' => array(
        'name' => /*_w*/('added invoice'),
    ),
    'invoice_archived' => array(
        'name' => /*_w*/('Invoice archived'),
    ),
    'invoice_cancel' => array(
        'name' => /*_w*/('Invoice restored to draft'),
    ),
    'invoice_delete' => array(
        'name' => /*_w*/('Invoice deleted'),
    ),
    'invoice_issue' => array(
        'name' => /*_w*/('issued invoice'),
    ),
    'invoice_paid' => array(
        'name' => /*_w*/('Invoice paid'),
    ),
    'invoice_paiment_canceled' => array(
        'name' => /*_w*/('Invoice payment canceled'),
    ),
    'invoice_refunded' => array(
        'name' => /*_w*/('Invoice payment refunded'),
    ),
    'invoice_restored' => array(
        'name' => /*_w*/('Invoice restored'),
    ),
    'invoice_updated' => array(
        'name' => /*_w*/('Invoice edited'),
    ),
    'create_signup_form' => array(
        'name' => /*_w*/('created signup form')
    ),
    'form_lead' => array(
        'name' => /*_w*/('filled form'),
        'subject' => true,
    ),
    'message_sent' => array(
        'name' => /*_w*/('sent message'),
    ),
    'call' => array(
        'name' => /*_w*/('called'),
    ),
    'call_in' => array(
        'name' => /*_w*/('incoming call %s sec.'),
    ),
    'call_out' => array(
        'name' => /*_w*/('outgoing call  %s sec.'),
    ),
    'contact_transfer' => array(
        'name' => /*_w*/('changed contact owner'),
    ),
);
