...
class crmDealListAction extends crmBackendViewAction
{
    ...
    public function execute()
    {
        ...
        foreach ($deals as &$d) {
            ...
            $sources = $dm->getDealSources($d['id']);
            if ($sources) {
                $d['source'] = array(
                    'id'    => $sources['id'],
                    'type'  => $sources['type'],
                ...
                if ($sources['type'] == "IM") {
                    $d['source']['icon'] = wa()->getAppStaticUrl('crm/plugins/' . $sources['provider'] . '/img', true) . $sources['provider'].'.png';
                }
            }
    ...
    protected function getListParams()
    {
        ...
        if ($this->view_mode == 'list') {
            $list_params['offset'] = waRequest::post('offset', 0, waRequest::TYPE_INT);
            $list_params['limit'] = 50;
        }

        // Prepare order-by params
        $deal_list_sort = wa()->getUser()->getSettings('crm', 'deal_list_sort');
        $list_sort = explode(' ', $deal_list_sort.' ', 2);
        $sort = waRequest::request('sort', ifempty($list_sort[0], 'create_datetime'), 'string');
        $asc = (int)waRequest::request('asc', trim($list_sort[1]), 'string');
        $list_params['sort'] = $sort;
        $list_params['order'] = $asc ? 'asc' : 'desc';
        if ($deal_list_sort !== $sort.' '.$asc) {
        ...
        $old_tag = wa()->getUser()->getSettings('crm', 'deal_tag_id');
        $tag = waRequest::request('tag', null, waRequest::TYPE_STRING_TRIM);
        if ($tag !== $old_tag && !empty($tag)) {
        ...
        foreach ($deal_fields as $key => $value) {
            $old_fields[$key] = wa()->getUser()->getSettings('crm', 'deal_field_' . $key);
            $fields[$key] = waRequest::request('field-' . $key, null, waRequest::TYPE_STRING_TRIM);
            if ($fields[$key] !== $old_fields[$key] && !empty($fields[$key])) {
        ...
        $reminder_states = waRequest::request('reminder', array(), waRequest::TYPE_STRING_TRIM);
        if ($reminder_states) {
    ...
    {
        $funnel_id = waRequest::request('funnel', wa()->getUser()->getSettings('crm', 'deal_funnel_id'), waRequest::TYPE_STRING_TRIM);
        ...
        if (!$funnel_id || is_numeric($funnel_id)) {
            $old_funnel_id = wa()->getUser()->getSettings('crm', 'deal_funnel_id');

            $deal_fields = $this->getFunnelFields($funnel_id);
            if ($funnel_id !== null) {
                if ($funnel_id != $old_funnel_id) {
