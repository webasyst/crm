 ...
 */
class crmPopup20Controller extends crmJsonController
{
    public $funnels = null;
        ...
        if ($pop_up_disabled != 1) {
            foreach ($reminders as $reminder) {
                ...
                if ($contact_id > 0) {
                    ...
                    $contact_show = '<i class="icon size-16 userpic" style="background-image: url('.$contact->getPhoto('20').');"></i> <a href="'.wa()->getAppUrl('crm').'contact/'.$contact['id'].'" class="notifi-link">'.htmlspecialchars($contact['name']).'</a>';
                } // Related Deal
                elseif ($contact_id < 0) {
                    $dm = new crmDealModel();
                    ...
                    try {
                        ...
                        $color = $stage['color'];
                        $contact_show = '
                        <i class="icon size-16 userpic" title="'.htmlspecialchars($contact['name']).'" style="background-image: url('.$contact->getPhoto('20').');"></i>
                ...
                $html = '<div class="crm-notification-popup reminder" data-item="reminder-'.$reminder_id.'">
                            <div class="c-notifi-head">
                                <i class="icon fas fa-clock"></i>
                                <span class="c-notifi-title">'._w('Reminder').' '.$reminder_time.'</span>
                                <i class="icon fas fa-times c-notify-close" title="'._w('Close').'"></i>
                            </div>
    ...
    {
        return '<span class="icon size-16" title="'.$funnel_name." / ".$stage_name.'"><i class="fas fa-circle" style="color: '.$color.'"></i></span>';
    }
}
