...
class crmCallRedirectController extends crmJsonController
{
    ...
    public function execute()
    {
        ...
        if (!$number_type) {
            throw new waException('No redirect number_type', 404);
        }

        $number = waRequest::post('number', 0, waRequest::TYPE_STRING_TRIM);
        if (!$number) {
            throw new waException('No redirect number', 404);
        }
        ...
        if (!$tplugin) {
            throw new waException('A non-telephony plugin is used', 404);
        }

        $redirect_allowed = $tplugin->isRedirectAllowed($call);

        if (!$redirect_allowed) {
            throw new waException('The telephony plug-in does not provide the ability to redirect a call', 404);
        }
