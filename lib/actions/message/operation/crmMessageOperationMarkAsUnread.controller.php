...
/**
 * Class crmMessageOperationMarkAsUnreadController
 *
 * Controller for mass marking as unread list of messages/conversations
 ...
 */
class crmMessageOperationMarkAsUnreadController extends crmMessageOperationMarkController
{
    public function execute()
    {
        ...
        // mark messages
        $marked_message_ids = $this->markMessagesAsUnread($message_ids, $this->getUserId());
     ...
     * @throws waException
     */
    protected function markMessagesAsUnread($message_ids, $contact_id)
    {
        if (!wa_is_int($contact_id) || $contact_id <= 0) {
            return [];
        }
        $mrm = $this->getMessageReadModel();
        $mrm->setUnread($message_ids, $contact_id);
