...
class crmSettingsFieldSaveController extends crmJsonController
{
    ...
    public function execute()
    {
        ...
        if (!$res[0]) {
            ...
            $this->error(_w("Can't construct field"));
        }
    ...
    {
        return array(
            'id' => trim((string) $this->getRequest()->post('id_val')),
            'ftype' => trim((string) $this->getRequest()->post('ftype')),
            'select_field_value' => trim((string) $this->getRequest()->post('select_field_value')),
            'names' => array_map(function ($_name) {
                    return trim((string) $_name);
                },
    ...
    public function setOptions($options)
    {
        ...
        foreach ($options as $field_id => $opts)
        {
            ...
            foreach ($opts['fields'] as $subfield_id => $params)
            {
                ...
                // for radio/select/etc kind of field
     ...
     */
    protected function createFromOpts($opts, $occupied_keys = array())
    {
