...
class crmMaxPluginMessageBodyWorker
{
    /**
     * @var array
     */
    ...
    /**
     * @param MAX message body $body
     * @param array $options
     * @return string
     */
    public static function parser($body, $options = array())
    {
        $sanitizer = new self($options);
        return $sanitizer->compile(ifset($body['text'], ''), ifset($body['markup'], []));
    }
     ...
     * @return string
     */
    public static function parserCaption($body, $options = array())
    {
        $sanitizer = new self($options);
        return $sanitizer->compile(ifset($body['text'], ''), ifset($body['markup'], []));
    }
        ...
        foreach ($entities as $idx => &$ent)
        {
            switch ($ent['type']) {
                ...
                case 'link':
                    ...
                    break;
                case 'user_mention':
                    $username = substr($content, $ent['from']*2 + 1, $ent['length']*2);
                    $open_tag = 'a href="https://max.ru/'.trim($username).'" target="_blank"';
                    $close_tag = '/a';
                    break;
                case 'monospaced':
                    $open_tag = 'code';
                    ...
                    break;
                case 'heading':
                    $open_tag = 'span class="large bold"';
                    $close_tag = '/span';
                    break;
                case 'blockquote':
                    $open_tag = 'blockquote';
            ...
            for ($i = $idx - 1; $i >= 0; $i--) {
                if ($ent['from'] + $ent['orig_length'] >= $entities[$i]['from'] + $entities[$i]['orig_length']) {
            ...
            $format_str = $open_tag . substr($content, $ent['from']*2, $ent['length']*2). $close_tag;
            if ($ent['type'] === 'pre') {
                $format_str = str_replace($nl_16, $nl_16_subst, $format_str);
            }
            $content = substr_replace($content, $format_str, $ent['from']*2, $ent['length']*2);
        }
    ...
    public function sanitize($content)
    {
        ...
        // Leave only those tags from which MAX will not go mad
